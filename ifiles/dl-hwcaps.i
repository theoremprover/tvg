...
# 40 "dl-hwcaps.c"
  ...
  uint64_t masked = _rtld_local_ro._dl_hwcap & hwcap_mask;
  size_t cnt = platform != 
# 43 "dl-hwcaps.c" 3 4
                          ((void *)0)
# 43 "dl-hwcaps.c"
...
# 61 "dl-hwcaps.c"
    ...
    {
      const Elf64_Phdr *const phdr = _rtld_local_ro._dl_sysinfo_map->l_phdr;
      const Elf64_Word phnum = _rtld_local_ro._dl_sysinfo_map->l_phnum;
      for (uint_fast16_t i = 0; i < phnum; ++i)
 if (phdr[i].p_type == 4)
   {
     const Elf64_Addr start = (phdr[i].p_vaddr
          + _rtld_local_ro._dl_sysinfo_map->l_addr);
     ...
     Elf64_Addr align = phdr[i].p_align;
     if (align < 4)
       align = 4;
     ...
     while ((Elf64_Addr) (note + 1) - start < phdr[i].p_memsz)
       {
  ...
  if (note->type == 2
      ...
      && note->datalen > 2 * sizeof (Elf64_Word) + 2)
    {
      const Elf64_Word *p
        = ((const void *) note
    + (((sizeof (Elf64_Nhdr) + (sizeof "GNU")) + ((align)) - 1) & -((__typeof__ ((sizeof (Elf64_Nhdr) + (sizeof "GNU")) + ((align)) - 1)) (((align))))));
      cnt += *p++;
      ++p;
  ...
  note = ((const void *) note
   + ((((((sizeof (Elf64_Nhdr) + ((note->vendorlen))) + (((align))) - 1) & -((__typeof__ ((sizeof (Elf64_Nhdr) + ((note->vendorlen))) + (((align))) - 1)) ((((align)))))) + (note->datalen)) + ((align)) - 1) & -((__typeof__ (((((sizeof (Elf64_Nhdr) + ((note->vendorlen))) + (((align))) - 1) & -((__typeof__ ((sizeof (Elf64_Nhdr) + ((note->vendorlen))) + (((align))) - 1)) ((((align)))))) + (note->datalen)) + ((align)) - 1)) (((align)))))
                           );
       }
     if (dsocaps != 
# 118 "dl-hwcaps.c" 3 4
...
# 131 "dl-hwcaps.c"
                    )
    {
      ...
      const Elf64_Word mask = ((const Elf64_Word *) dsocaps)[-1];
      _rtld_local_ro._dl_hwcap |= (uint64_t) mask << (48 + 4);




      hwcap_mask |= (uint64_t) mask << (48 + 4);

      ({ __tunable_set_val (glibc_tune_hwcap_mask, & (uint64_t) {hwcap_mask}); });
      ...
      for (const char *p = dsocaps; p < dsocaps + dsocapslen; p += len + 1)
 {
   ...
   if (__builtin_expect ((mask & ((Elf64_Word) 1 << bit)), 1))
     {
       temp[m].str = p;
       temp[m].len = len;
       ++m;
...
# 173 "dl-hwcaps.c"
  ...
  ((void) sizeof ((m == cnt) ? 1 : 0), __extension__ ({ if (m == cnt) ; else __assert_fail ("m == cnt", "dl-hwcaps.c", 184, __extension__ __PRETTY_FUNCTION__); }));
  ...
  if (cnt == 1)
    {
      ...
      result[1].len = 0;
      cp = __builtin_mempcpy ((char *) (result + *sz), temp[0].str, temp[0].len);
      *cp = '/';
      *sz = 2;
...
# 238 "dl-hwcaps.c"
  ...
  if (cnt == 2)
    {
      cp = __builtin_mempcpy (__builtin_mempcpy (cp, temp[1].str, temp[1].len), "/", 1);;
      cp = __builtin_mempcpy (__builtin_mempcpy (cp, temp[0].str, temp[0].len), "/", 1);;
    }
  else
    {
      n = 1 << (cnt - 1);
      do
 {
   ...
   cp = __builtin_mempcpy (__builtin_mempcpy (cp, temp[cnt - 1].str, temp[cnt - 1].len), "/", 1);;


   for (m = cnt - 2; m > 0; --m)
     if ((n & (1 << m)) != 0)
       cp = __builtin_mempcpy (__builtin_mempcpy (cp, temp[m].str, temp[m].len), "/", 1);;
