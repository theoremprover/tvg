...
determine_info (const Elf64_Addr addr, struct link_map *match, Dl_info *info,
  struct link_map **mapp, const Elf64_Sym **symbolp)
{
  ...
  const Elf64_Sym *symtab
    = (const Elf64_Sym *) (match)->l_info[6]->d_un.d_ptr;
  const char *strtab = (const char *) (match)->l_info[5]->d_un.d_ptr;

  Elf64_Word strtabsize = match->l_info[10]->d_un.d_val;

  const Elf64_Sym *matchsym = 
# 44 "dl-addr.c" 3 4
                             ((void *)0)
# 44 "dl-addr.c"
                                 ;
  if (match->l_info[(35 + 0 + 16 + 3 + 12 + (0x6ffffeff - (0x6ffffef5)))] != 
# 45 "dl-addr.c" 3 4
                                             ((void *)0)
# 45 "dl-addr.c"
                                                 )
      ...
      for (Elf_Symndx bucket = 0; bucket < match->l_nbuckets; ++bucket)
 {
       ...
       do
  {
    ...
    if ((symtab[symndx].st_shndx != 0
         || symtab[symndx].st_value != 0)
        && symtab[symndx].st_shndx != 0xfff1
        && ((symtab[symndx].st_info) & 0xf) != 6
        && ((addr) >= (match)->l_addr + (&symtab[symndx])->st_value && ((((&symtab[symndx])->st_shndx == 0 || (&symtab[symndx])->st_size == 0) && (addr) == (match)->l_addr + (&symtab[symndx])->st_value) || (addr) < (match)->l_addr + (&symtab[symndx])->st_value + (&symtab[symndx])->st_size) && ((matchsym) == 
# 64 "dl-addr.c" 3 4
          ((void *)0) 
# 64 "dl-addr.c"
          || (matchsym)->st_value < (&symtab[symndx])->st_value))

        && symtab[symndx].st_name < strtabsize)
      matchsym = (Elf64_Sym *) &symtab[symndx];
...
# 78 "dl-addr.c"
 ...
 symtabend = (symtab
       + ((Elf_Symndx *) (match)->l_info[4]->d_un.d_ptr)[1]);
      else




 symtabend = (const Elf64_Sym *) strtab;

      for (; (void *) symtab < (void *) symtabend; ++symtab)
 if (((((unsigned char) (symtab->st_info)) >> 4) == 1
      || (((unsigned char) (symtab->st_info)) >> 4) == 2)
     && __builtin_expect ((!dl_symbol_visibility_binds_local_p (symtab)), 1)
     && ((symtab->st_info) & 0xf) != 6
     && (symtab->st_shndx != 0
  || symtab->st_value != 0)
     && symtab->st_shndx != 0xfff1
     && ((addr) >= (match)->l_addr + (symtab)->st_value && ((((symtab)->st_shndx == 0 || (symtab)->st_size == 0) && (addr) == (match)->l_addr + (symtab)->st_value) || (addr) < (match)->l_addr + (symtab)->st_value + (symtab)->st_size) && ((matchsym) == 
# 96 "dl-addr.c" 3 4
       ((void *)0) 
# 96 "dl-addr.c"
       || (matchsym)->st_value < (symtab)->st_value))
     && symtab->st_name < strtabsize)
   matchsym = (Elf64_Sym *) symtab;
    }
  ...
  if (matchsym)
    {
      ...
      lookup_t matchl = match;

      info->dli_sname = strtab + matchsym->st_name;
      info->dli_saddr = (void *) ((matchsym) == 
# 113 "dl-addr.c" 3 4
                       ((void *)0) 
# 113 "dl-addr.c"
                       ? 0 : (__builtin_expect (((matchsym)->st_shndx == 0xfff1), 0) ? 0 : ((
# 113 "dl-addr.c" 3 4
                       0
# 113 "dl-addr.c"
                       ) || (matchl) ? (matchl)->l_addr : 0)) + (matchsym)->st_value);
    }
  else
    {
...
_dl_addr (const void *address, Dl_info *info,
   struct link_map **mapp, const Elf64_Sym **symbolp)
{
  const Elf64_Addr addr = ((Elf64_Addr) (address));
  int result = 0;
...
extern __typeof (_dl_addr) __EI__dl_addr __asm__("" "_dl_addr"); extern __typeof (_dl_addr) __EI__dl_addr __attribute__((alias ("" "__GI__dl_addr")));
