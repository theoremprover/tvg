...
# 195 "dl-cache.c"
                  )
    {
      ...
      if (file != ((void *) -1) && cachesize > sizeof *cache
   && memcmp (file, "ld.so-1.7.0", sizeof "ld.so-1.7.0" - 1) == 0)
 {
   size_t offset;
   ...
   offset = (((sizeof (struct cache_file) + cache->nlibs * sizeof (struct file_entry)) + __alignof__ (struct cache_file_new) -1) & (~(__alignof__ (struct cache_file_new) - 1)))
                                                ;

   cache_new = (struct cache_file_new *) ((void *) cache + offset);
   if (cachesize < (offset + sizeof (struct cache_file_new))
       || memcmp (cache_new->magic, "glibc-ld.so.cache" "1.1",
    sizeof "glibc-ld.so.cache" "1.1" - 1) != 0)
     cache_new = (void *) -1;
 }
      else if (file != ((void *) -1) && cachesize > sizeof *cache_new
        && memcmp (file, "glibc-ld.so.cache" "1.1",
     sizeof "glibc-ld.so.cache" "1.1" - 1) == 0)
 {
   cache_new = file;
   cache = file;
...
# 237 "dl-cache.c" 3 4
     ((void *)0)
# 237 "dl-cache.c"
     ) ? 1 : 0), __extension__ ({ if (cache != 
# 237 "dl-cache.c" 3 4
     ((void *)0)
# 237 "dl-cache.c"
     ) ; else __assert_fail ("cache != NULL", "dl-cache.c", 237, __extension__ __PRETTY_FUNCTION__); }));
    }
  ...
  if (cache_new != (void *) -1)
    {
      ...
      platform = _dl_string_platform (_rtld_local_ro._dl_platform);
      if (platform != (uint64_t) -1)
 platform = 1ULL << platform;

      uint64_t hwcap_mask = ({ tunable_id_t id = glibc_tune_hwcap_mask; uint64_t ret; __tunable_get_val (id, &ret, 
# 260 "dl-cache.c" 3 4
                           ((void *)0)
# 260 "dl-cache.c"
                           ); ret; });
      ...
      uint64_t hwcap_exclude = ~((_rtld_local_ro._dl_hwcap & hwcap_mask)
     | (((1ULL << 4) - 1) << 48) | (1LL << 63));
# 276 "dl-cache.c"
      do { left = 0; right = cache_new->nlibs - 1; while (left <= right) { __typeof__ (cache_new->libs[0].key) key; middle = (left + right) / 2; key = cache_new->libs[middle].key; if (! (key < cache_data_size)) { cmpres = 1; break; } cmpres = _dl_cache_libcmp (name, cache_data + key); if (__builtin_expect ((cmpres == 0), 0)) { left = middle; while (middle > 0) { __typeof__ (cache_new->libs[0].key) key; key = cache_new->libs[middle - 1].key; if (! (key < cache_data_size) || _dl_cache_libcmp (name, cache_data + key) != 0) break; --middle; } do { int flags; __typeof__ (cache_new->libs[0]) *lib = &cache_new->libs[middle]; if (middle > left && (! (lib->key < cache_data_size) || (_dl_cache_libcmp (name, cache_data + lib->key) != 0))) break; flags = lib->flags; if (((flags) == 0x303) && (lib->value < cache_data_size)) { if (best == 
# 276 "dl-cache.c" 3 4
     ((void *)0) 
# 276 "dl-cache.c"
     || flags == _rtld_local_ro._dl_correct_cache_id) { if (lib->hwcap & hwcap_exclude) continue; if (_rtld_local_ro._dl_osversion && lib->osversion > _rtld_local_ro._dl_osversion) continue; if (4 && (lib->hwcap & (((1ULL << 4) - 1) << 48)) != 0 && (lib->hwcap & (((1ULL << 4) - 1) << 48)) != platform) continue; best = cache_data + lib->value; if (flags == _rtld_local_ro._dl_correct_cache_id) break; } } } while (++middle <= right); break; } if (cmpres < 0) left = middle + 1; else right = middle - 1; } } while (0);
    }
  else
    {
      ...
      do { left = 0; right = cache->nlibs - 1; while (left <= right) { __typeof__ (cache->libs[0].key) key; middle = (left + right) / 2; key = cache->libs[middle].key; if (! (key < cache_data_size)) { cmpres = 1; break; } cmpres = _dl_cache_libcmp (name, cache_data + key); if (__builtin_expect ((cmpres == 0), 0)) { left = middle; while (middle > 0) { __typeof__ (cache->libs[0].key) key; key = cache->libs[middle - 1].key; if (! (key < cache_data_size) || _dl_cache_libcmp (name, cache_data + key) != 0) break; --middle; } do { int flags; __typeof__ (cache->libs[0]) *lib = &cache->libs[middle]; if (middle > left && (! (lib->key < cache_data_size) || (_dl_cache_libcmp (name, cache_data + lib->key) != 0))) break; flags = lib->flags; if (((flags) == 0x303) && (lib->value < cache_data_size)) { if (best == 
# 288 "dl-cache.c" 3 4
     ((void *)0) 
# 288 "dl-cache.c"
     || flags == _rtld_local_ro._dl_correct_cache_id) { do {} while (0); best = cache_data + lib->value; if (flags == _rtld_local_ro._dl_correct_cache_id) break; } } } while (++middle <= right); break; } if (cmpres < 0) left = middle + 1; else right = middle - 1; } } while (0);
    }
...
# 297 "dl-cache.c"
  ...
  char *temp;
  temp = __builtin_alloca (strlen (best) + 1);
  strcpy (temp, best);
