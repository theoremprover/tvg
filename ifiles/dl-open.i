...
extern void __init_misc (int, char **, char **) __attribute__ ((visibility ("hidden")));


extern __typeof (__profile_frequency) __profile_frequency __attribute__ ((visibility ("hidden")));


extern int __libc_multiple_libcs __attribute__ ((visibility ("hidden")));
# 36 "dl-open.c" 2
...
# 23 "../sysdeps/unix/sysv/linux/not-cancel.h"
# 1 "../include/fcntl.h" 1
...
# 24 "../sysdeps/unix/sysv/linux/not-cancel.h" 2
...
add_to_global (struct link_map *new)
{
  ...
  for (cnt = 0; cnt < new->l_searchlist.r_nlist; ++cnt)
    if (new->l_searchlist.r_list[cnt]->l_global == 0)
      ++to_add;
# 86 "dl-open.c"
  struct link_namespaces *ns = &_rtld_local._dl_ns[new->l_ns];
...
# 98 "dl-open.c"
 ...
 = _rtld_local._dl_ns[new->l_ns]._ns_main_searchlist->r_list;
...
# 129 "dl-open.c"
 ...
 _rtld_local._dl_wait_lookup_done ();
...
struct link_map *
_dl_find_dso_for_object (const Elf64_Addr addr)
{
  struct link_map *l;


  for (Lmid_t ns = 0; ns < _rtld_local._dl_nns; ++ns)
    for (l = _rtld_local._dl_ns[ns]._ns_loaded; l != 
# 168 "dl-open.c" 3 4
                                           ((void *)0)
# 168 "dl-open.c"
                                               ; l = l->l_next)
   ...
   && (l->l_contiguous
       || _dl_addr_inside_object (l, (Elf64_Addr) addr)))
 {
   ((void) sizeof ((ns == l->l_ns) ? 1 : 0), __extension__ ({ if (ns == l->l_ns) ; else __assert_fail ("ns == l->l_ns", "dl-open.c", 173, __extension__ __PRETTY_FUNCTION__); }));
   return l;
 }
  return 
# 176 "dl-open.c" 3 4
...
extern __typeof (_dl_find_dso_for_object) __EI__dl_find_dso_for_object __asm__("" "_dl_find_dso_for_object"); extern __typeof (_dl_find_dso_for_object) __EI__dl_find_dso_for_object __attribute__((alias ("" "__GI__dl_find_dso_for_object")));;
...
# 194 "dl-open.c"
                                  )
    {
      ...
      call_map = _rtld_local._dl_ns[0]._ns_loaded;

      struct link_map *l = _dl_find_dso_for_object ((Elf64_Addr) caller_dlopen);
...
# 222 "dl-open.c"
    ...
    {
      ((void) sizeof ((mode & 0x00004) ? 1 : 0), __extension__ ({ if (mode & 0x00004) ; else __assert_fail ("mode & RTLD_NOLOAD", "dl-open.c", 224, __extension__ __PRETTY_FUNCTION__); }));
      return;
    }
  ...
  if (__builtin_expect ((mode & 0x01000), 0))
    new->l_flags_1 |= 0x00000008;

  if (__builtin_expect ((mode & 0x40000000), 0))
  ...
  if (__builtin_expect ((new->l_searchlist.r_list != 
# 242 "dl-open.c" 3 4
     ((void *)0)
# 242 "dl-open.c"
     ), 0))
 ...
 _dl_debug_printf ("opening file=%s [%lu]; direct_opencount=%u\n\n",
     new->l_name, new->l_ns, new->l_direct_opencount);
      ...
      if ((mode & 0x00100) && new->l_global == 0)
 (void) add_to_global (new);

      ((void) sizeof ((_dl_debug_initialize (0, args->nsid)->r_state == RT_CONSISTENT) ? 1 : 0), __extension__ ({ if (_dl_debug_initialize (0, args->nsid)->r_state == RT_CONSISTENT) ; else __assert_fail ("_dl_debug_initialize (0, args->nsid)->r_state == RT_CONSISTENT", "dl-open.c", 254, __extension__ __PRETTY_FUNCTION__); }));
...
# 260 "dl-open.c"
                               , 0, 0,
         mode & (0x80000000 | 0x00008 | 0x08000000));


  for (unsigned int i = 0; i < new->l_searchlist.r_nlist; ++i)
    if (new->l_searchlist.r_list[i]->l_real->l_versions == 
# 265 "dl-open.c" 3 4
                                                          ((void *)0)
# 265 "dl-open.c"
                                                              )
  ...
  if (__builtin_expect ((_rtld_local_ro._dl_naudit > 0), 0))
    {
      struct link_map *head = _rtld_local._dl_ns[new->l_ns]._ns_loaded;
...
# 280 "dl-open.c"
  ...
  int reloc_mode = mode & 0x08000000;
  if (_rtld_local_ro._dl_lazy)
    reloc_mode |= mode & 0x00001;
...
# 344 "dl-open.c"
         ), 0))
 {
   ...
   struct link_map *old_profile_map = _rtld_local._dl_profile_map;

   _dl_relocate_object (l, l->l_scope, reloc_mode | 0x00001, 1);

   if (old_profile_map == 
# 355 "dl-open.c" 3 4
                         ((void *)0) 
# 355 "dl-open.c"
                              && _rtld_local._dl_profile_map != 
# 355 "dl-open.c" 3 4
                                                       ((void *)0)
# 355 "dl-open.c"
                                                           )
     {
       ...
       _rtld_local._dl_profile_map->l_flags_1 |= 0x00000008;
     }
 }
      else
...
# 393 "dl-open.c"
   ...
   if (__builtin_expect ((cnt + 1 >= imap->l_scope_max), 0))
    ...
    && imap->l_scope_max < (sizeof (imap->l_scope_mem) / sizeof (imap->l_scope_mem[0])))
  {
    new_size = (sizeof (imap->l_scope_mem) / sizeof (imap->l_scope_mem[0]));
...
# 460 "dl-open.c"
 ...
 _dl_show_scope (imap, from_scope);
  ...
  if (any_tls && __builtin_expect (++_rtld_local._dl_tls_generation == 0, 0))
    do { _dl_dprintf (2, "TLS generation counter wrapped!  Please report this."); _exit (127); } while (1)
                                                       ;
   ...
   && imap->l_tls_blocksize > 0)
 {
   ...
   _rtld_local._dl_init_static_tls (imap);
   ((void) sizeof ((imap->l_need_tls_init == 0) ? 1 : 0), __extension__ ({ if (imap->l_need_tls_init == 0) ; else __assert_fail ("imap->l_need_tls_init == 0", "dl-open.c", 493, __extension__ __PRETTY_FUNCTION__); }));
 }
    }
...
# 534 "dl-open.c"
  ...
  if (__builtin_expect ((nsid == -1), 0))
    {

      for (nsid = 1; 16 > 1 && nsid < _rtld_local._dl_nns; ++nsid)
 if (_rtld_local._dl_ns[nsid]._ns_loaded == 
# 543 "dl-open.c" 3 4
                                  ((void *)0)
# 543 "dl-open.c"
                                      )
...
# 551 "dl-open.c"
      ...
      else if (nsid == _rtld_local._dl_nns)
 {
   (void) ((_rtld_local._dl_ns[nsid]._ns_unique_sym_table.lock) = (__rtld_lock_recursive_t) {{ { 0, 0, 0, 0, PTHREAD_MUTEX_RECURSIVE_NP, 0, 0, { 0, 0 } } }});
   ++_rtld_local._dl_nns;
 }
  ...
  else if (__builtin_expect ((nsid != 0 && nsid != -2), 0)
    && (__builtin_expect ((nsid < 0 || nsid >= _rtld_local._dl_nns), 0)




        || 16 == 1
        || _rtld_local._dl_ns[nsid]._ns_nloaded == 0
        || _rtld_local._dl_ns[nsid]._ns_loaded->l_auditing))
    _dl_signal_error (
# 574 "dl-open.c" 3 4
                     22
# 574 "dl-open.c"
...
# 581 "dl-open.c"
  ...
  if (__builtin_expect ((exception.errstring != 
# 596 "dl-open.c" 3 4
     ((void *)0)
# 596 "dl-open.c"
     ), 0))
...
# 612 "dl-open.c"
      ...
      ((void) sizeof ((_dl_debug_initialize (0, args.nsid)->r_state == RT_CONSISTENT) ? 1 : 0), __extension__ ({ if (_dl_debug_initialize (0, args.nsid)->r_state == RT_CONSISTENT) ; else __assert_fail ("_dl_debug_initialize (0, args.nsid)->r_state == RT_CONSISTENT", "dl-open.c", 615, __extension__ __PRETTY_FUNCTION__); }));


      _rtld_local._dl_rtld_unlock_recursive (&(_rtld_local._dl_load_lock).mutex);


      _dl_signal_exception (errcode, &exception, 
# 621 "dl-open.c" 3 4
                                                ((void *)0)
# 621 "dl-open.c"
                                                    );
  ...
  ((void) sizeof ((_dl_debug_initialize (0, args.nsid)->r_state == RT_CONSISTENT) ? 1 : 0), __extension__ ({ if (_dl_debug_initialize (0, args.nsid)->r_state == RT_CONSISTENT) ; else __assert_fail ("_dl_debug_initialize (0, args.nsid)->r_state == RT_CONSISTENT", "dl-open.c", 624, __extension__ __PRETTY_FUNCTION__); }));
...
_dl_show_scope (struct link_map *l, int from)
{
  _dl_debug_printf ("object=%s [%lu]\n",
      ((l->l_name)[0] ? (l->l_name) : (_dl_argv[0] ?: "<main program>")), l->l_ns);
  if (l->l_scope != 
# 638 "dl-open.c" 3 4
                   ((void *)0)
# 638 "dl-open.c"
                       )
    for (int scope_cnt = from; l->l_scope[scope_cnt] != 
# 639 "dl-open.c" 3 4
                                                       ((void *)0)
# 639 "dl-open.c"
                                                           ; ++scope_cnt)
 ...
 for (unsigned int cnt = 0; cnt < l->l_scope[scope_cnt]->r_nlist; ++cnt)
   if (*l->l_scope[scope_cnt]->r_list[cnt]->l_name)
    ...
    l->l_scope[scope_cnt]->r_list[cnt]->l_name);
     ...
     _dl_debug_printf_c (" %s", (_dl_argv[0] ?: "<program name unknown>"));
