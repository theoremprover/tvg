...
# 99 "/home/flaig/EB_2.1/build/tmp/work/corei7-64-poky-linux/glibc/2.28-r0/recipe-sysroot-native/usr/lib/x86_64-poky-linux/gcc/x86_64-poky-linux/8.3.0/include/stdarg.h" 3 4
...
# 1 "../sysdeps/unix/sysv/linux/bits/uio-ext.h" 1
# 26 "../sysdeps/unix/sysv/linux/bits/uio-ext.h"
...
extern ssize_t process_vm_writev (pid_t __pid, const struct iovec *__lvec,
      ...
      unsigned long int __flags)
  __attribute__ ((__nothrow__ ));
# 51 "../sysdeps/unix/sysv/linux/bits/uio-ext.h"

# 169 "../misc/sys/uio.h" 2
...
# 26 "../sysdeps/unix/sysv/linux/not-cancel.h" 2
...
_dl_sysdep_read_whole_file (const char *file, size_t *sizep, int prot)
{
  void *result = ((void *) -1);
  struct stat64 st;
  int fd = __open64_nocancel (file, 00 | 02000000);
  if (fd >= 0)
    {
...
_dl_debug_vdprintf (int fd, int tag_p, const char *fmt, va_list arg)
{
   ...
   if (pid == 0)
     {
       char *p;
       pid = __getpid ();
       ((void) sizeof ((pid >= 0 && sizeof (pid_t) <= 4) ? 1 : 0), __extension__ ({ if (pid >= 0 && sizeof (pid_t) <= 4) ; else __assert_fail ("pid >= 0 && sizeof (pid_t) <= 4", "dl-misc.c", 98, __extension__ __PRETTY_FUNCTION__); }));
  ...
  *--p = ' ';
   ...
   ((void) sizeof ((niov < 64) ? 1 : 0), __extension__ ({ if (niov < 64) ; else __assert_fail ("niov < NIOVMAX", "dl-misc.c", 107, __extension__ __PRETTY_FUNCTION__); }));
   iov[niov].iov_len = 12;
 ...
 ++fmt;
      ...
      ((void) sizeof ((niov < 64) ? 1 : 0), __extension__ ({ if (niov < 64) ; else __assert_fail ("niov < NIOVMAX", "dl-misc.c", 120, __extension__ __PRETTY_FUNCTION__); }));
...
# 202 "dl-misc.c"
       ...
       if (prec != -1)
  iov[niov].iov_len = ((((size_t) prec)<(iov[niov].iov_len))?((size_t) prec):(iov[niov].iov_len));
       ++niov;
       break;
     ...
     default:
       ((void) sizeof ((! "invalid format specifier") ? 1 : 0), __extension__ ({ if (! "invalid format specifier") ; else __assert_fail ("! \"invalid format specifier\"", "dl-misc.c", 216, __extension__ __PRETTY_FUNCTION__); }));
     }
   ++fmt;
 }
      else if (*fmt == '\n')
...
# 249 "dl-misc.c"
                    ;
  _dl_debug_vdprintf (_rtld_local_ro._dl_debug_fd, 1, fmt, arg);
  ...
  while (1)
   ...
   if (*nptr >= 'a' && *nptr <= 'f')
     ...
     break;
      ...
      if (result >= ((18446744073709551615UL) - digval) / base)
 {
   if (endptr != 
# 425 "dl-misc.c" 3 4
                ((void *)0)
