...
# 23 "../sysdeps/unix/sysv/linux/lowlevellock-futex.h"
...
# 24 "../sysdeps/unix/sysv/linux/lowlevellock-futex.h" 2
...
extern int __lll_trylock_elision (int *lock, short *adapt_count)
  ;
# 34 "../sysdeps/nptl/libc-lockP.h" 2
...
# 21 "cache.c" 2
# 1 "../include/inttypes.h" 1

# 1 "../sysdeps/generic/inttypes.h" 1
# 34 "../sysdeps/generic/inttypes.h"
typedef int __gwchar_t;
# 266 "../sysdeps/generic/inttypes.h"
...
# 290 "../sysdeps/generic/inttypes.h"
...
# 432 "../sysdeps/generic/inttypes.h"

# 3 "../include/inttypes.h" 2


# 22 "cache.c" 2
# 1 "../include/libgen.h" 1
# 1 "../misc/libgen.h" 1
# 23 "../misc/libgen.h"
      ...
      uint64_t hwcap, const char *key)
{
  ...
  switch (flag & 0x00ff)
    ...
    case 0x0002:
    case 0x0003:
      fputs (flag_descr[flag & 0x00ff], stdout);
      break;
    default:
      fputs (__dcgettext (_libc_intl_domainname, "unknown", 5), stdout);
      break;
    }
  switch (flag & 0xff00)
    {
    ...
    case 0:
      break;
    default:
      printf (",%d", flag & 0xff00);
      break;
    }
  if (hwcap != 0)
    printf (", hwcap: %#.16" "l" "x", hwcap);
  if (osversion != 0)
    {
 ...
 [6] = "Unknown OS"
      ...
      printf (__dcgettext (_libc_intl_domainname, ", OS ABI: %s %d.%d.%d", 5),
       __dcgettext (_libc_intl_domainname, abi_tag_os[os > (sizeof abi_tag_os / sizeof abi_tag_os[0] - 1) ? (sizeof abi_tag_os / sizeof abi_tag_os[0] - 1) : os], 5),
       (osversion >> 16) & 0xff,
       (osversion >> 8) & 0xff,
       osversion & 0xff);
    }
...
{
  int fd = open (cache_name, 00);
  if (fd < 0)
    error (1, (*__errno_location ()), __dcgettext (_libc_intl_domainname, "Can't open cache file %s\n", 5), cache_name);
...
# 175 "cache.c" 3 4
           ((void *)0)
# 175 "cache.c"
               , st.st_size, 0x1, 0x02, fd, 0);
  if (cache == ((void *) -1))
    error (1, (*__errno_location ()), __dcgettext (_libc_intl_domainname, "mmap of cache file failed.\n", 5));

  size_t cache_size = st.st_size;
  if (cache_size < sizeof (struct cache_file))
    error (1, 0, __dcgettext (_libc_intl_domainname, "File is not a cache file.\n", 5));
...
# 183 "cache.c"
  ...
  if (memcmp (cache->magic, "ld.so-1.7.0", sizeof "ld.so-1.7.0" - 1))
    {

      cache_new = (struct cache_file_new *) cache;

      if (memcmp (cache_new->magic, "glibc-ld.so.cache", sizeof "glibc-ld.so.cache" - 1)
   || memcmp (cache_new->version, "1.1",
        sizeof "1.1" - 1))
 error (1, 0, __dcgettext (_libc_intl_domainname, "File is not a cache file.\n", 5));
      format = 1;
    ...
    {
      size_t offset = (((sizeof (struct cache_file) + (cache->nlibs * sizeof (struct file_entry))) + __alignof__ (struct cache_file_new) -1) & (~(__alignof__ (struct cache_file_new) - 1)))
   ...
   (offset + sizeof (struct cache_file_new)))
 {
   ...
   if (memcmp (cache_new->magic, "glibc-ld.so.cache",
        sizeof "glibc-ld.so.cache" - 1) == 0
       && memcmp (cache_new->version, "1.1",
    sizeof "1.1" - 1) == 0)
     {
       cache_data = (const char *) cache_new;
  ...
  if (format == 0)
    {
      printf (__dcgettext (_libc_intl_domainname, "%d libs found in cache `%s'\n", 5), cache->nlibs, cache_name);
...
# 316 "cache.c"
 ...
 + cache_entry_old_count * sizeof (struct file_entry);
      ...
      memcpy (file_entries->magic, "ld.so-1.7.0", sizeof "ld.so-1.7.0" - 1);

      file_entries->nlibs = cache_entry_old_count;
    }

  struct cache_file_new *file_entries_new = 
# 340 "cache.c" 3 4
                                           ((void *)0)
# 340 "cache.c"
                                               ;
 ...
 + cache_entry_count * sizeof (struct file_entry_new);
      ...
      memcpy (file_entries_new->magic, "glibc-ld.so.cache",
       sizeof "glibc-ld.so.cache" - 1);
      memcpy (file_entries_new->version, "1.1",
       sizeof "1.1" - 1);
  ...
  size_t pad = (((file_entries_size) + __alignof__ (struct cache_file_new) -1) & (~(__alignof__ (struct cache_file_new) - 1))) - file_entries_size;
  ...
  for (idx_old = 0, idx_new = 0, entry = entries; entry != 
# 379 "cache.c" 3 4
                                                          ((void *)0)
# 379 "cache.c"
                                                              ;
 ...
 ++idx_old;
  ...
  int fd = open (temp_name, 0100|01|01000|0400000,
   0400|0200);
  if (fd < 0)
    error (1, (*__errno_location ()), __dcgettext (_libc_intl_domainname, "Can't create temporary cache file %s", 5),
    temp_name);
 ...
 error (1, (*__errno_location ()), __dcgettext (_libc_intl_domainname, "Writing of cache data failed", 5));
    }
  if (opt_format != 0)
    {
 ...
 error (1, (*__errno_location ()), __dcgettext (_libc_intl_domainname, "Writing of cache data failed", 5));
  ...
  if (chmod (temp_name, ((0400 >> 3) >> 3)|(0400 >> 3)|0400|0200))
    error (1, (*__errno_location ()),
    __dcgettext (_libc_intl_domainname, "Changing access rights of %s to %#o failed", 5), temp_name,
    ((0400 >> 3) >> 3)|(0400 >> 3)|0400|0200);
  ...
  if (rename (temp_name, cache_name))
    error (1, (*__errno_location ()), __dcgettext (_libc_intl_domainname, "Renaming of %s to %s failed", 5), temp_name,
    cache_name);
...
{
  char magic[sizeof "glibc-ld.so.auxcache-1.0" - 1];
  uint32_t nlibs;
  uint32_t len_strings;
  struct aux_cache_file_entry libs[0];
    ...
    unsigned int *osversion, char **soname)
{
  ...
  id.ctime = (uint64_t) stat_buf->st_ctim.tv_sec;
...
load_aux_cache (const char *aux_cache_name)
{
  ...
  struct stat64 st;
  if (__fxstat64 (1, fd, &st) < 0 || st.st_size < sizeof (struct aux_cache_file))
    {
      close (fd);
      init_aux_cache ();
      return;
...
# 706 "cache.c" 3 4
           ((void *)0)
# 706 "cache.c"
               , aux_cache_size, 0x1, 0x02, fd, 0);
  if (aux_cache == ((void *) -1)
      || aux_cache_size < sizeof (struct aux_cache_file)
      || memcmp (aux_cache->magic, "glibc-ld.so.auxcache-1.0", sizeof "glibc-ld.so.auxcache-1.0" - 1)
      || aux_cache_size != (sizeof(struct aux_cache_file) +
...
# 729 "cache.c" 3 4
     ((void *)0) 
# 729 "cache.c"
          : aux_cache_data + aux_cache->libs[i].soname,
    0);
...
# 751 "cache.c"
  ...
  memcpy (file_entries->magic, "glibc-ld.so.auxcache-1.0", sizeof "glibc-ld.so.auxcache-1.0" - 1);
...
# 785 "cache.c"
  ...
  char *dir = (__extension__ ({ const char *__old = (aux_cache_name); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); }));
  dir = dirname (dir);

  struct stat64 st;
  if (__xstat64 (1, dir, &st) < 0)
    {
      if (mkdir (dir, 0700) < 0)
 goto out_fail;
    }
