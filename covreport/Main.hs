...
colourSrc SrcFile{..} srctext = (font ! [ face "courier" ]) $ simpleTable [] [] [ [
	...
	colouring _ buf [] = stringToHtml buf
	colouring (cnt@Counter{..}:cnts) buf rest@((l,c,i,_):_) | lineC==l, columnC==c =
		stringToHtml buf +++
	...
	colouring cnt buf ((_,_,_,char):rest) = colouring cnt (buf++[char]) rest
