...
_Dcomplex (csqrt)(_Dcomplex x)
 ...
 if (erry == 1)
  ...
  return (_Cbuild(0.0,
   (((_Dval *)(char *)&(im))->_Sh[3] & ((unsigned short)0x8000)) ? -_Inf._Double
    : _Inf._Double));
 else
  return (_Cbuild(_Inf._Double,
   (((_Dval *)(char *)&(im))->_Sh[3] & ((unsigned short)0x8000)) ? -_Zero
    : 0.0));
 ...
 if (xexp == 0)
  ...
  {
  double remag = ldexp(re < 0.0 ? -re : re, -xexp);

  rho = ldexp(sqrt(2.0 * (remag + rho)),
   xexp / 2 - 1);
  if (0.0 <= re)
   return (_Cbuild(rho, im / (2.0 * rho)));
  else if ((((_Dval *)(char *)&(im))->_Sh[3] & ((unsigned short)0x8000)))
