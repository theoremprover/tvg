...
_Lcomplex (csqrtl)(_Lcomplex x)
 ...
 if (erry == 1)
  ...
  return (_LCbuild(0.0L,
   (((_Lval *)(char *)&(im))->_Sh[3] & ((unsigned short)0x8000)) ? -_LInf._Long_double
    : _LInf._Long_double));
 else
  return (_LCbuild(_LInf._Long_double,
   (((_Lval *)(char *)&(im))->_Sh[3] & ((unsigned short)0x8000)) ? -_LZero
    : 0.0L));
 ...
 if (xexp == 0)
  ...
  {
  long double remag = ldexpl(re < 0.0L ? -re : re, -xexp);

  rho = ldexpl(sqrtl(2.0L * (remag + rho)),
   xexp / 2 - 1);
  if (0.0L <= re)
   return (_LCbuild(rho, im / (2.0L * rho)));
  else if ((((_Lval *)(char *)&(im))->_Sh[3] & ((unsigned short)0x8000)))
