...
long double _WStoldx(const wchar_t *s, wchar_t **endptr, long pten, int *perr)
 {
 long double x;
 long lo[((53 + 27) / 28) + 1];
 const wchar_t *s0 = s;
 ...
 if ((code &= ~8) == 1)
  {
  const int nlo = _WStoflt(s0, s, endptr, lo, ((53 + 27) / 28));
  int i;
  ...
  {
  const int nlo = _WStoxflt(s0, s, endptr, lo, ((53 + 27) / 28));
  int i;
