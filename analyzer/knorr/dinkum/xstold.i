...
long double _Stoldx(const char *s, char **endptr, long pten,
 ...
 extern long double _LDtentox(long double, long, int *);

 if (perr != 0)
  *perr = 0;
 if ((code &= ~8) == 1)
  {
  const int nlo = _Stoflt(s0, s, endptr, lo, ((53 + 27) / 28));
  int i;

  if (nlo == 0)
   x = 0.0L;
  else
   for (i = 1, x = (long double)lo[1]; i < nlo; )
    x = x * 1e9L + (long double)lo[++i];

  x = _LDtentox(x, pten + lo[0], perr);
  }
 else if (code == 2)
  {
  const int nlo = _Stoxflt(s0, s, endptr, lo, ((53 + 27) / 28));
  int i;
  ...
  if (nlo == 0)
   ...
   for (i = 1, x = (long double)lo[1]; i < nlo; )
    x = x * 268435456.0L + (long double)lo[++i];

  _LDscale(&x, lo[0]);
  x = _LDtentox(x, pten, perr);
  }
 else if (code == 3)
