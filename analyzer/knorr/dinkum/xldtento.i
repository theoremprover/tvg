...
static short dmul(long double *px, long double y)
 {
 short xexp;

 _LDunscale(&xexp, px);
 *px *= y;
 return (_LDscale(px, xexp));
 }

long double _LDtentox(long double x, long n, int *perr)
 {
 long double factor;
 short errx;
 size_t i;
 ...
 if (n < 0)
  ...
  for (i = 0; 0 < nu && i < npows; nu >>= 1, ++i)
   if (nu & 1)
    factor *= *(pows + i);
  errx = dmul(&x, 1.0L / factor);
  if (errx < 0 && 0 < nu)
   for (factor = 1.0L / *(pows + npows); 0 < nu; --nu)
