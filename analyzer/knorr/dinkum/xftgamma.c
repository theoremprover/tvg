...
static const struct Approx approx0[] = {
 {67.0F, 97.0F / 4020.0F, s10, ((sizeof (s10) / sizeof ((s10)[0])) - 1), c10, ((sizeof (c10) / sizeof ((c10)[0])) - 1)},
 };
static float stirling(float *px, short *pex)
 {
 int i;
 float x = *px;
 ...
 *px = _FPoly(y, approx0[i].den, approx0[i].dsize)
  / _FPoly(y, approx0[i].num, approx0[i].nsize);
 *px /= rootxx * expf(-x);
 return (0.0F);
  }
 }
