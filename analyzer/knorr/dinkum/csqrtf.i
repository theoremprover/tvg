...
_Fcomplex (csqrtf)(_Fcomplex x)
 ...
 if (erry == 1)
  ...
  return (_FCbuild(0.0F,
   (((_Fval *)(char *)&(im))->_Sh[1] & ((unsigned short)0x8000)) ? -_FInf._Float
    : _FInf._Float));
 else
  return (_FCbuild(_FInf._Float,
   (((_Fval *)(char *)&(im))->_Sh[1] & ((unsigned short)0x8000)) ? -_FZero
    : 0.0F));
 ...
 if (xexp == 0)
  ...
  {
  float remag = ldexpf(re < 0.0F ? -re : re, -xexp);

  rho = ldexpf(sqrtf(2.0F * (remag + rho)),
   xexp / 2 - 1);
  if (0.0F <= re)
   return (_FCbuild(rho, im / (2.0F * rho)));
  else if ((((_Fval *)(char *)&(im))->_Sh[1] & ((unsigned short)0x8000)))
