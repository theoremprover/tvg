...
static const float lnhalf = -0.69314718055994530941723212145817658F;
static const float lnone_half = 0.40546510810816438197801311546434915F;
...
float (expm1f)(float x)
 ...
 case 0:
  return (x);
 case 1:
  return ((((_Fval *)(char *)&(x))->_Sh[1] & ((unsigned short)0x8000)) ? -1.0F : x);
 default:
  if (lnhalf < x && x < lnone_half)
   return (((((s[0] * x + s[1]) * x + s[2]) * x + s[3]) / ((c[0] * x + c[1]) * x + c[2])) * x);
  else
   return (expf(x) - 1.0F);
  }
 }
