...
static double scale = (double)(1 << 6);

static double lnbias[][4] = {
 {
...
double (_Pow)(double x, double y, short *pex)
 ...
 if (erry == 0 && y == 0.0
  ...
  if (errx == 2)
   ...
   if (!(((_Dval *)(char *)&(x))->_Sh[3] & ((unsigned short)0x8000)))
    return ((((_Dval *)(char *)&(y))->_Sh[3] & ((unsigned short)0x8000)) ? 0.0 : _Inf._Double);
   else if (!(((_Dval *)(char *)&(y))->_Sh[3] & ((unsigned short)0x8000)))
   ...
   if (!(((_Dval *)(char *)&(y))->_Sh[3] & ((unsigned short)0x8000)))
    return (xexp <= 0 ? 0.0 : _Inf._Double);
   else
    return (xexp <= 0 ? _Inf._Double : 0.0);
  else
   if (!(((_Dval *)(char *)&(y))->_Sh[3] & ((unsigned short)0x8000)))
    return (erry == 0 && _Dint(&yi, -1) < 0 && (((_Dval *)(char *)&(x))->_Sh[3] & ((unsigned short)0x8000))
     ? -_Zero : 0.0);
   else
    {
    _Feraise(0x04);
 ...
 else if ((((_Dval *)(char *)&(x))->_Sh[3] & ((unsigned short)0x8000)) && erry < 0)
  {
  _Feraise(0x01);
  return (_Nan._Double);
  }
 ...
 if (xexp == 1 && x < rthalf)
  ...
  {
  k = (int)(x * scale + 0.5) - (1 << (6 - 1));
  bias = (double)k / scale + 0.5;
  }
 ...
 x -= bias;
 x1 = z * (w * _Logpoly(w) - x + 2.0 * (1.0 - bias));

 yi = (x + x1) / ln2;
 yi += plna[0] + (double)xexp;
  }
 ...
 if (z < -maxexp)
  ...
  if (plna == 0)
   ...
   z += (y * ((double)xexp + plna[0]) - (double)zexp) * ln2;
   }
  errx = -1;
  }
 else if (maxexp < z)
