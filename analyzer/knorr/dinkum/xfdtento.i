...
static short dmul(float *px, float y)
 {
 short xexp;

 _FDunscale(&xexp, px);
 *px *= y;
 return (_FDscale(px, xexp));
 }

float _FDtentox(float x, long n, int *perr)
 {
 float factor;
 short errx;
 size_t i;
 ...
 if (n < 0)
  ...
  for (i = 0; 0 < nu && i < npows; nu >>= 1, ++i)
   if (nu & 1)
    factor *= *(pows + i);
  errx = dmul(&x, 1.0F / factor);
  if (errx < 0 && 0 < nu)
   for (factor = 1.0F / *(pows + npows); 0 < nu; --nu)
