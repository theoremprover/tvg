...
double _Xp_getw(double *p, int n)
 {
 if (n == 0)
  return (0.0);
 else if (n == 1 || p[0] == 0.0 || 1 < n && p[1] == 0.0)
  return (p[0]);
 else if (n == 2)
  return (p[0] + p[1]);
 else
...
double *_Xp_addh(double *p, int n, double x0)
 ...
 if (n == 0)
  ...
  if (errx == 2 || (errx = _Dtest(&p[0])) <= 0)
   p[0] = x0;
  else if (errx == 2 || (((_Dval *)(char *)&(x0))->_Sh[3] & ((unsigned short)0x8000)) == (((_Dval *)(char *)&(p[0]))->_Sh[3] & ((unsigned short)0x8000)))
   ;
  else
   {
   _Feraise(0x01);
...
double *_Xp_movx(double *p, int n, double *q)
 {
 memcpy(p, q, n * sizeof (double));
 return (p);
 }

double *_Xp_addx(double *p, int n, double *q, int m)
 {
 int k;
...
double *_Xp_subx(double *p, int n, double *q, int m)
 {
 int k;
...
double *_Xp_mulx(double *p, int n, double *q, int m, double *ptemp2)
 {
 if (n == 0 || m == 0)
  ;
