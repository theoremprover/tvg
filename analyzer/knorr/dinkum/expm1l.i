...
static const long double lnhalf = -0.69314718055994530941723212145817658L;
static const long double lnone_half = 0.40546510810816438197801311546434915L;
...
long double (expm1l)(long double x)
 ...
 case 0:
  return (x);
 case 1:
  return ((((_Lval *)(char *)&(x))->_Sh[3] & ((unsigned short)0x8000)) ? -1.0L : x);
 default:
  if (lnhalf < x && x < lnone_half)
   return (((((((s[0] * x + s[1]) * x + s[2]) * x + s[3]) * x + s[4]) * x + s[5]) / (((((c[0] * x + c[1]) * x + c[2]) * x + c[3]) * x + c[4]) * x + c[5])) * x);
  else
   return (expl(x) - 1.0L);
  }
 }
