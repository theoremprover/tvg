...
float _WStofx(const wchar_t *s, wchar_t **endptr, long pten, int *perr)
 {
 float x;
 long lo[((24 + 27) / 28) + 1];
 const wchar_t *s0 = s;
 ...
 extern float _FDtentox(float, long, int *);

 if (perr != 0)
  *perr = 0;
 if ((code &= ~8) == 1)
  {
  const int nlo = _WStoflt(s0, s, endptr, lo, ((24 + 27) / 28));
  int i;

  if (nlo == 0)
   x = 0.0F;
  else
   for (i = 1, x = (float)lo[1]; i < nlo; )
    x = x * 1e9F + (float)lo[++i];

  x = _FDtentox(x, pten + lo[0], perr);
  }
 else if (code == 2)
  {
  const int nlo = _WStoxflt(s0, s, endptr, lo, ((24 + 27) / 28));
  int i;
  ...
  if (nlo == 0)
   ...
   for (i = 1, x = (float)lo[1]; i < nlo; )
    x = x * 268435456.0F + (float)lo[++i];

  _FDscale(&x, lo[0]);
  x = _FDtentox(x, pten, perr);
  }
 else if (code == 3)
  x = _FInf._Float;
 else if (code == 4)
