...
float _FXp_getw(float *p, int n)
 {
 if (n == 0)
  return (0.0F);
 else if (n == 1 || p[0] == 0.0F || 1 < n && p[1] == 0.0F)
  return (p[0]);
 else if (n == 2)
  return (p[0] + p[1]);
 else
...
float *_FXp_addh(float *p, int n, float x0)
 ...
 if (n == 0)
  ...
  if (errx == 2 || (errx = _FDtest(&p[0])) <= 0)
   p[0] = x0;
  else if (errx == 2 || (((_Fval *)(char *)&(x0))->_Sh[1] & ((unsigned short)0x8000)) == (((_Fval *)(char *)&(p[0]))->_Sh[1] & ((unsigned short)0x8000)))
   ;
  else
   {
   _Feraise(0x01);
...
float *_FXp_movx(float *p, int n, float *q)
 {
 memcpy(p, q, n * sizeof (float));
 return (p);
 }

float *_FXp_addx(float *p, int n, float *q, int m)
 {
 int k;
...
float *_FXp_subx(float *p, int n, float *q, int m)
 {
 int k;
...
float *_FXp_mulx(float *p, int n, float *q, int m, float *ptemp2)
 {
 if (n == 0 || m == 0)
  ;
