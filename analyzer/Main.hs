	...
	let
		...
		searchcondpoint stmt = do
			add_branches <- case stmt of
				CWhile cond _ _ _        -> lift $ createBranches (makeWhileBranchName cond) cond
				CCase cond _ _           -> return [ (Branch (lineColNodeInfo stmt) 1 False (makeCaseBranchName cond),true_cexpr) ]
				CDefault _ _             -> return [ (Branch (lineColNodeInfo stmt) 1 False makeDefaultBranchName,true_cexpr) ]
				CFor _ (Just cond) _ _ _ -> lift $ createBranches (makeForBranchName cond) cond
				CFor _ Nothing _ _ _     -> error $ "searchcondpoint: for(_,Nothing,_) not implemented!"
				CIf cond _ _ _           -> lift $ createBranches (makeIfBranchName cond) cond
				_                        -> return []
			modify (add_branches++)
			return stmt
		...
		searchexprcondpoint expr = do
			add_branches <- case expr of
				CCond cond _ _ _ -> lift $ createBranches (makeCondBranchName cond) cond
				_                -> return []
			modify (add_branches++)
			return expr
...
wrapGoto (a,cond) = CBlockStmt $ CGotoPtr (CBinary CEqOp cond (CConst $ CStrConst (CString (show a) True) undefNode) undefNode) undefNode
...
handleDecision :: CExpr -> (CExpr -> String) -> Int -> Trace -> CovVecM ([(Branch,CExpr)],Int)
handleDecision cond makebranchname forks trace = do
	...
	case mb_annotation of
		...
		Just (ns,num_reached) | length ns > num_reached && ns!!num_reached /= 12 -> do
			printLogV 2 $ "Recognized annotation " ++ show (ns!!num_reached) ++ " to " ++ (render.pretty) real_cond ++
...
unfoldTraces1M :: Maybe Type -> Bool -> Int -> [Env] -> Trace -> [([CBlockItem],Bool)] -> CovVecM UnfoldTracesRet
unfoldTraces1M mb_ret_type toplevel forks envs trace bstss@((CBlockStmt stmt : rest,breakable) : rest2) =
	logWrapper [ren "unfoldTraces1M",ren mb_ret_type,ren toplevel,ren forks,ren $ take 2 envs,ren trace,'\n':ren bstss] $ do
		case stmt of
	
			CCompound _ cbis _ -> unfoldTracesM mb_ret_type toplevel forks ([]:envs) trace ((cbis,False) : (rest,breakable) : rest2)
	
			CLabel _ cstat _ _ -> unfoldTracesM mb_ret_type toplevel forks envs trace (((CBlockStmt cstat : rest, breakable)) : rest2)
			...
			CSwitch condexpr (CCompound [] cbis _) switch_ni -> do
				...
				let
					...
					collect_stmts branchnum notconds (CBlockStmt cdefault@(CDefault def_stmt default_ni) : rest) =
						...
						wrapGoto (Just $ Branch (lineColNodeInfo cdefault) branchnum False "default",notconds) :
							CBlockStmt def_stmt :
							( for rest $ \case
			...
			CBreak ni -> do
				...
				unfoldTracesM mb_ret_type toplevel forks new_envs trace new_bstss

			CIf cond then_stmt mb_else_stmt ni -> do
				(branches_to_follow,forks') <- handleDecision cond makeIfBranchName forks trace
				forkUnfoldTraces toplevel branches_to_follow $ \ (branch,branch_cond) -> do
					...
					unfoldTracesM mb_ret_type toplevel (forks+1) envs trace ( (rest ++ cbstmts,breakable) : rest2 )

{-	 mb_ret_type toplevel forks envs trace then_stmt mb_else_stmt insertconds ( (rest,breakable) : rest2 ) (branches_to_follow,forks')
				let (real_cond,mb_annotation) = recognizeAnnotation cond
				all_branches <- createBranches (makeIfBranchName real_cond) real_cond
...
-}
			...
			CExpr (Just (CUnary unaryop expr ni_op)) ni | unaryop `elem` (map fst unaryops) -> do
				...
				unfoldTracesM mb_ret_type toplevel forks envs trace ( (CBlockStmt stmt' : rest,breakable) : rest2 )
				where
			...
			CGotoPtr wrapped ni -> do
				...
				transids cond (Just _BoolTypes) trace $ \ (envs',cond',trace') -> do
					unfoldTracesM mb_ret_type toplevel forks envs' (Condition mb_branch cond' : trace') ( (rest,breakable) : rest2 )
	...
	--				return (Just default_ns,"No annotation, trying " ++ show default_ns)
					...
					translateExprM forks envs toplevel real_cond (Just _BoolTypes) [] >>= \case
						[(_,cond,[])] -> do
							let
		...
		transids expr mb_ty trace cont = logWrapper ["transids",ren expr,ren mb_ty,ren trace,"<cont>"] $ do
			additional_envs_expr_traces :: [([Env],CExprWithType,Trace)] <- translateExprM forks envs toplevel expr mb_ty trace
...
unfoldTraces1M mb_ret_type toplevel forks envs@(env:restenvs) trace ( (cblockitem@(CBlockDecl decl@(CDecl typespecs triples _)) : rest, breakable) : rest2 ) =
	logWrapper [ren "unfoldTraces1M CBlockDecl",ren mb_ret_type,ren toplevel,ren forks,ren $ take 2 envs,ren trace,'\n':ren cblockitem] $ do
		...
		unfoldTracesM mb_ret_type toplevel forks ((concat newenvs ++ env) : restenvs) (concat newitems ++ trace) ((concat initializerss ++ rest,breakable):rest2)

unfoldTraces1M mb_ret_type toplevel@False forks envs trace cbss@[([],_)] =
	logWrapper [ren "unfoldTraces1M END_NOT_TOPLEVEL",ren mb_ret_type,ren toplevel,ren forks,ren $ take 2 envs,ren trace,'\n':ren cbss] $ do
		return $ Left [(envs,trace)]

unfoldTraces1M mb_ret_type toplevel forks envs@(_:restenvs) trace cbss@(([],_):rest2) =
	logWrapper [ren "unfoldTraces1M LEAVE_SCOPE",ren mb_ret_type,ren toplevel,ren forks,ren $ take 2 envs,ren trace,'\n':ren cbss] $ do
...
transcribeExprM envs mb_target_ty expr = do
	...
	annotateTypesAndCastM envs cexpr mb_target_ty = logWrapper ["annotateTypesAndCastM",ren $ take 1 envs,ren cexpr,ren mb_target_ty] $ do
...
type CallOrTernaryIfs = [ Either (Ident,[CExpr],NodeInfo) (CBlockItem,CStat,CStat,([(Branch,CExpr)],Int)) ]
...
translateExprM :: Int -> [Env] -> Bool -> CExpr -> Maybe Types -> Trace -> CovVecM [([Env],CExprWithType,Trace)]
translateExprM forks envs toplevel expr0 mb_target_ty trace = logWrapper ["translateExprM",ren $ take 2 envs,ren toplevel,ren expr0,ren mb_target_ty, ren trace] $ do
	let
		...
		to_call_or_ternaryifs ccond@(CCond cond (Just true_expr) false_expr ni) = do
			branches_to_follow_exprs <- lift $ handleDecision cond makeCondBranchName forks trace
			let
			...
			let
				...
				decl_cbi = CBlockDecl $ type2Decl var_ident ni ty
				then_stmt = var ≔ true_expr
				else_stmt = var ≔ false_expr

			-- Replace the ccond by the new variable "var"
			modify ( Right (decl_cbi,then_stmt,else_stmt,branches_to_follow_exprs) : )
			return var
...
{-
forkBranches :: Maybe Type -> Bool -> Int -> [Env] -> Trace -> CStat -> Maybe CStat -> Bool -> [([CBlockItem],Bool)] -> ([(Branch,CExpr)],Int) -> CovVecM UnfoldTracesRet
forkBranches mb_ret_type toplevel forks envs trace then_stmt mb_else_stmt insertconds ( (rest,breakable) : rest2 ) (branches_to_follow,forks') = do
	forkUnfoldTraces toplevel branches_to_follow $ \ (branch,branch_cond) -> do
		let
			-- Only insert a condition if the CIf was not generated by TVG (for translation of while, e.g.)
			branchcbi = if insertconds then [] else [wrapGoto (Just branch,branch_cond)]
			cbstmts = branchcbi ++ case isElseBranch branch of
				False -> [CBlockStmt then_stmt]
				True  -> case mb_else_stmt of
					Nothing -> []
...
([(Branch,CExpr)],Int)
-}
	create_combinations envs expr trace subs (Right (decl_cbi,then_stmt,else_stmt,(branches_to_follow_exprs,forks)) : rest) = do
		Left envs_ternaryiftraces <- forkUnfoldTraces toplevel branches_to_follow_exprs $ \ (branch,branchcond) -> do
			let
				cbstmts = wrapGoto (Just branch,branchcond) : case isElseBranch branch of
					False -> [CBlockStmt then_stmt]
					True  -> [CBlockStmt else_stmt]
			unfoldTracesM Nothing False forks envs [] [ (decl_cbi : cbstmts,False) ]
		concatForM envs_ternaryiftraces $ \ (envs',ternaryif_trace) -> do
			create_combinations envs' expr (ternaryif_trace++trace) subs rest

			
{-
		Left envs_ternaryiftraces <- forkBranches Nothing False forks envs trace then_stmt (Just else_stmt)
			True [ ([decl_cbi],False) ] branches_to_follow_exprs
		concatForM envs_ternaryiftraces $ \ (envs',ternaryif_trace) -> do
			printLogV 1 $ "XXXXXXXXXXXXXXX length=" ++ show (length envs_ternaryiftraces)
			create_combinations envs' expr (ternaryif_trace++trace) subs rest
-}
