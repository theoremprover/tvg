...
wrapGoto :: (CNode a) => a -> (Maybe Branch,CExpr) -> CBlockItem
wrapGoto cnode (mb_branch,cond) = CBlockStmt $ CGotoPtr (CBinary CEqOp cond (CConst $ CStrConst (CString (show mb_branch) True) undefNode) undefNode) (nodeInfo cnode)
...
unfoldTraces1M mb_ret_type toplevel forks envs trace bstss@((CBlockStmt stmt : rest,breakable) : rest2) =
	logWrapper [ren "unfoldTraces1M",ren mb_ret_type,ren toplevel,ren forks,ren $ take 2 envs,ren trace,'\n':ren bstss] $ do
		case stmt of
			...
			CSwitch condexpr (CCompound [] cbis _) switch_ni -> do
				...
				let
					...
					collect_stmts branchnum notconds (CBlockStmt cdefault@(CDefault def_stmt default_ni) : rest) =
						...
						wrapGoto cdefault (Just $ Branch (lineColNodeInfo cdefault) branchnum False makeDefaultBranchName,notconds) :
							CBlockStmt def_stmt :
							( for rest $ \case
					...
					collect_stmts branchnum notconds (CBlockStmt ccase@(CCase caseexpr stmt case_ni) : rest) = [
						CBlockStmt $ CIf (CBinary CEqOp cond_var caseexpr case_ni)
							(makeCompound $
								wrapGoto caseexpr (Just $ Branch (lineColNodeInfo ccase) branchnum False ("case " ++ (render.pretty) caseexpr),cond) :
								CBlockStmt stmt :
								filtercases rest)
			...
			CIf cond then_stmt mb_else_stmt ni -> do
				...
				forkUnfoldTraces toplevel branches_to_follow $ \ (branch,branch_cond) -> do
					let
						...
						branchcbi = if ni==undefNode then [] else [wrapGoto cond (Just branch,branch_cond)]
			...
			CWhile cond body False ni -> do
				...
				unroll n =
					concat ( replicate n [ wrapGoto cond (Just $ Branch (lineColNodeInfo cond) 1 False "",cond), CBlockStmt body ] ) ++
					[ wrapGoto cond (Just $ Branch (lineColNodeInfo cond) 2 True "",not_c cond) ]
	...
	--				return (Just default_ns,"No annotation, trying " ++ show default_ns)
					...
					translateExprM envs toplevel real_cond (Just _BoolTypes) trace 0 >>= \case
						[(_,cond,[])] -> do
							let
		...
		to_call_or_ternaryifs ccond@(CCond cond (Just true_expr) false_expr ni) = do
			...
			let
				...
				cbiss = for branches_to_follow $ \ (branch,branch_cond) ->
					decl : wrapGoto cond (Just branch,branch_cond) : case isElseBranch branch of
