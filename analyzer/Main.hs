...
createBranchesWithAnno cond makebranchname trace = do
	...
	printLogV 0 $ "XXX real_cond=" ++ (render.pretty) real_cond ++ " at " ++ showLocation (lineColNodeInfo cond)
...
--		printLogV 0 $ showTrace trace
		case stmt of
			...
			CIf cond then_stmt mb_else_stmt ni → do
				...
				printLogV 0 $ "XXX CIf ( "  ++ (render.pretty) cond ++ " )"
				printLogV 0 $ "XXX CIf at " ++ showLocation (lineColNodeInfo ni) ++ " branches =" ++ show (map (numBranch.fst) branches_to_follow)
