# 1 "/arm-libs/library-src/dinkum/source/./xwctomb.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwctomb.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./xwctomb.c" 2
# 1 "/toolchain/arm/include/limits.h" 1 3
# 4 "/arm-libs/library-src/dinkum/source/./xwctomb.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xwctomb.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./xwctomb.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./xwctomb.c" 2
...
# 48 "/arm-libs/library-src/dinkum/source/./xwctomb.c"
  ...
  for (; ; )
   ...
   if (16 <= state
    || (stab = pwcstate->_Tab[state]) == 0
    || (int) _Getmbcurmax() <= nout
    || (16 * 0xff) <= ++limit
    || (code = stab[wc & 0xff]) == 0)
    {
    ( _Errno) = 0x0058;
   ...
   if (code & 0x1000)
    wc = (wchar_t)(wc << 8
     | (0xff & wc >> 8 * (sizeof (wchar_t) - 1)));
   if (code & 0x2000)
    {
    if ((s[nout++] = (char)(code & 0x00ff ? code : wc)) == '\0')
     leave = 1;
    limit = 0;
    }
   if (code & 0x4000 || leave)
    {
    pst->_State = state;
    return (nout);
    }
...
int _Wctomb(char *s, wchar_t wc, mbstate_t *pst)
 {
 _Statab *pmbstate = ((_Tls_setup__Mbstate && _Tls_setup__Mbstate()), (&(_Mbstate)));
 _Statab *pwcstate = pmbstate == 0 || s == 0 ? 0
  : ((_Tls_setup__Wcstate && _Tls_setup__Wcstate()), (&(_Wcstate)));
