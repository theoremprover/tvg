# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/memmove_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/memmove_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/memmove_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/memmove_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/memmove_s.c" 2
...
errno_t (memmove_s)(void *s1, rsize_t maxsize,
 ...
 if (s1 == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("memmove_s: bad destination argument", sizeof ("memmove_s: bad destination argument")));
 else if (s2 == 0 || ((_Sizet)(-1) >> 1) < n || maxsize < n)
  {
  memset(s1, '\0', maxsize);
  return (_Fail_s("memmove_s: bad argument", sizeof ("memmove_s: bad argument")));
  }
 else
  {
  char *cs1 = (char *)s1;
  ...
  if (cs2 < cs1 && cs1 < cs2 + n)
   for (cs1 += n, cs2 += n; 0 < n; --n)
    *--cs1 = *--cs2;
  else
   for (; 0 < n; --n)
    *cs1++ = *cs2++;
