# 1 "/arm-libs/library-src/dinkum/source/./remquof.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./remquof.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./remquof.c" 2
...
static void _F_Remquo_subtract(float *px, float *py)
 {
 short xexp, yexp;
...
float (remquof)(float x, float y, int *pquo)
 {
 const short errx = _FDtest(&x);
 const short erry = _FDtest(&y);
 ...
 if (0 <= errx || 0 <= erry)
  ...
  for (t = y, _FDunscale(&ychar, &t), n = 0, intpart = 0; ; )
   {
   short xchar;

   t = x;
   if (n < 0 || _FDunscale(&xchar, &t) == 0
...
# 4 "/arm-libs/library-src/dinkum/source/./remquof.c" 2
