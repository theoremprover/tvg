# 1 "/arm-libs/library-src/dinkum/source/./xfhypot.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfhypot.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfhypot.c" 2
...
float _FHypot(float x, float y, int *pexp)
 {
 short xexp, yexp;
 short errx = _FDunscale(&xexp, &x);
 short erry = _FDunscale(&yexp, &y);
 ...
 *pexp = 0;
 if (errx == 1 || erry == 1)
  ...
  ((*_FPmsw(&(y))) &= ~((unsigned short)0x8000));
  ...
  else if (x == 0.0F || xexp < yexp || (xexp == yexp && x < y))
   {
   short texp = xexp;
   float tmp = x;
  ...
  if ((*pexp = (xexp / 4) * 2) == 0)
   *pexp = 2;
  _FDscale(&x, xexp - *pexp);
  _FDscale(&y, yexp - *pexp);
  ...
  if (z == x)
   ...
   {
   const float qv = ((x) / (y));
   z = x + ((y) / (qv + sqrtf(qv * qv + 1.0F)));
   }
  else
   {
   static const float root2 =
    1.4142135623730950488016887242096982F;
   static const float oneplusroot2high = 2.4142F;
   static const float oneplusroot2low =
    0.0000135623730950488016887242096980785698F;
   const float qv = ((z) / (y));
   const float rv = (qv + 2.0F) * qv;
   const float sv = ((rv) / (root2 + sqrtf(rv + 2.0F)))
...
# 4 "/arm-libs/library-src/dinkum/source/./xfhypot.c" 2
