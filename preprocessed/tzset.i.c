# 1 "/arm-libs/library-src/dinkum/source/./tzset.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./tzset.c" 2
...
time_t _Tzoff(void);
# 3 "/arm-libs/library-src/dinkum/source/./tzset.c" 2
...
void (tzset)()
 {
 time_t tod = time(0);
 time_t t1 = mktime(gmtime(&tod));
 struct tm *st2 = localtime(&tod);
 time_t t2 = mktime(st2);

 timezone = (long)difftime(t1, t2);
 daylight = 0 <= st2->tm_isdst ? 1 : 0;

 st2->tm_isdst = 0;
 if (strftime(tz0, 8, "%Z", st2) == 0)
  tz0[0] = '\0';
 st2->tm_isdst = 1;
 if (strftime(tz1, 8, "%Z", st2) == 0)
  tz1[0] = '\0';
 }
