# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/asctime_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/asctime_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/asctime_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/asctime_s.c" 2
...
time_t _Tzoff(void);
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/asctime_s.c" 2


extern _Tinfo _CTinfo;

errno_t (asctime_s)(char *s, rsize_t maxsize,
 const struct tm *t)
 {
 if (s == 0 || maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("asctime_s: bad destination argument", sizeof ("asctime_s: bad destination argument")));
 else if (maxsize < 26 || t == 0
  || t->tm_wday < 0 || 6 < t->tm_wday
  || t->tm_mon < 0 || 11 < t->tm_mon
  || t->tm_mday < 1 || 31 < t->tm_mday
  || t->tm_hour < 0 || 59 < t->tm_hour
  || t->tm_min < 0 || 59 < t->tm_min
  || t->tm_sec < 0 || 59 < t->tm_sec
  || t->tm_year < 0 - 1900 || 9999 - 1900 < t->tm_year)
  {
  s[0] = '\0';
  return (_Fail_s("asctime_s: bad argument", sizeof ("asctime_s: bad argument")));
  }

 return (_CStrftime(s, maxsize, "%c\n", t, &_CTinfo) == 0
  ? 0x0022 : 0);
 }
