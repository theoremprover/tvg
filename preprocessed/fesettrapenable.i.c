# 1 "/arm-libs/library-src/dinkum/source/./fesettrapenable.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fesettrapenable.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fesettrapenable.c" 2
# 28 "/arm-libs/library-src/dinkum/source/./fesettrapenable.c"
int (fesettrapenable)(fexcept_t enable)
 {
 if ((enable & (0x02 | 0x10 | 0x01 | 0x04 | 0x08)) != enable)
  return (1);
 else
  {
# 45 "/arm-libs/library-src/dinkum/source/./fesettrapenable.c"
  fenv_t env;

  fegetenv(&env);
  env = env & ~((0x02 | 0x10 | 0x01 | 0x04 | 0x08) << 8)
   | ((enable & (0x02 | 0x10 | 0x01 | 0x04 | 0x08)) << 8);
  fesetenv(&env);
# 88 "/arm-libs/library-src/dinkum/source/./fesettrapenable.c"
  return (0);
  }
 }
