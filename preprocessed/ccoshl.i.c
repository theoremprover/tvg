# 1 "/arm-libs/library-src/dinkum/source/./ccoshl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./ccoshl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./ccoshl.c" 2
...
_Lcomplex (ccoshl)(_Lcomplex x)
 ...
 if (errx == 2)
  ...
  _Feraise(0x01);
  if (errx == 0)
   return (_LCbuild(_LNan._Long_double,
    im < 0.0L ? -_LZero : 0.0L));
  else if (errx == 1)
  ...
  if (errx == 0)
   return (_LCbuild(_LNan._Long_double, re));
  else if (errx == 1)
   return (_LCbuild(_LInf._Long_double,
    _LNan._Long_double));
  else
  ...
  if (erry == 0)
   ...
   return (_LCbuild(_LInf._Long_double * cosl(im),
    _LInf._Long_double * sinl(im)));
 else
  return (_LCbuild(_LCosh(re, cosl(im)),
   _LSinh(re, sinl(im))));
 }
# 4 "/arm-libs/library-src/dinkum/source/./ccoshl.c" 2
