# 1 "/arm-libs/library-src/dinkum/source/./xtgamma.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xtgamma.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xtgamma.c" 2
...
double _Pow(double, double, short *);
const double _Gamma_big = 54.0;
# 48 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
static const double s[] = {
 0.00001192292740826975,
 0.00011794182945081212,
 0.00101959460145355905,
 0.00756060680498438352,
 0.01926527211745248008,
 0.12554212337999338034,
 0.54544333054825790736,
 0.43251944860403390826,
 };

static const double c[] = {
 0.00002670630520841925,
 -0.00142562700733949340,
 0.01184309540813817347,
 -0.00913626688892627011,
 -0.18643116397504167435,
 0.29357730424461011830,
 1.02302619254637107038,
 };
...
static const Approx_53 approx0[] =
 ...
 14, 2,
  {
 0.16614314518760217485e-1,
 -0.13965707272626176936e-2,
 -0.49824254383538848853e-2,
 0.10583487432607028510e-2,
 0.21489456425337538895e-2,
 -0.95896447695592953043e-3,
 -0.14455262959800639857e-2,
 0.14401123536802324852e-2,
 0.16474099822547439696e-2,
 -0.51877751839995419056e-2,
 -0.80784377037000113812e-3,
 0.15488179113127575577,
 1.96594700928271786568,
 2.50896364305534390989,
  },
  {
 0.70023705660120274726,
 1.0,
  },
 };
# 389 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
static double _Tgamma_stirling(double *px, short *pex)
 {
 int i;
 double x = *px;
 double rootxx = _Pow(x, x - 0.5, pex);
 ...
 *pex = -*pex;
 for (i = 0; i < sizeof (approx0) / sizeof (approx0[0]) - 1; ++i)
  ...
  {
 double y = (1.0 / (x)) - approx0[i].mid;

 *px = ((_Poly(y, approx0[i].den, approx0[i].dsize - 1)) / (_Poly(y, approx0[i].num, approx0[i].nsize - 1)));
...
double _Tgamma(double *px, short *pex)
 ...
 if (54.0 <= x)
  return (_Tgamma_stirling(px, pex));
 else
  {
  if (x < -0.5)
   {
   ...
   *pex = 0;
   ...
   {
   for (y = 1.0; 2.5 < x; y = ((y) / (x)))
    x -= 1.0;
   x -= 2.0;
   _Dunscale(pex, &y);
   }
  *px = y;
  return (((_Poly(x, s, sizeof (s) / sizeof (s[0]) - 1)) / (_Poly(x, c, sizeof (c) / sizeof (c[0]) - 1))) * x);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./xtgamma.c" 2
