# 1 "/arm-libs/library-src/dinkum/source/./casinh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./casinh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./casinh.c" 2
...
_Dcomplex (casinh)(_Dcomplex x)
 ...
 if (errx == 2)
  ...
  if (erry == 1)
   {
   u = re;
   v = im < 0.0 ? -piby4 : piby4;
   }
  else
   {
   u = re;
   v = ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -_Zero : 0.0;
   }
 else if (erry == 1)
  {
  u = im;
  ...
  {
  const _Dcomplex w = csqrt(_Cbuild(1.0 - im, re));
  const _Dcomplex z = csqrt(_Cbuild(1.0 + im, -re));
  ...
  v = atan2(im, creal(((w) * (z))));
  }
 return (_Cbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./casinh.c" 2
