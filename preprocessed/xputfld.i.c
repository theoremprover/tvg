# 1 "/arm-libs/library-src/dinkum/source/./xputfld.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xputfld.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xputfld.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xputfld.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xputfld.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./xputfld.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./xputfld.c" 2
# 18 "/arm-libs/library-src/dinkum/source/./xputfld.c"
int _Putfld(_Pft *px, va_list *pap, char code, char *ac)
 ...
 case 'c':
  ...
  if (px->qual != 'l')
   ...
   for (; 0 <= px->argno; --px->argno)
    ch = (char)__builtin_va_arg(*pap, int);
   ac[px->n0++] = ch;
   }
  else
   {
  ...
  for (; 0 <= px->argno; --px->argno)
   ...
   wi = __builtin_va_arg(*pap, wint_t);
 ...
 case 'd': case 'i':
  ...
  for (; 0 <= px->argno; --px->argno)
   px->v.li = px->qual == 'l' ? __builtin_va_arg(*pap, long)
    : px->qual == 'q' ? __builtin_va_arg(*pap, _Longlong)
    : px->qual == 'j' ? __builtin_va_arg(*pap, intmax_t)
    : px->qual == 't' ? __builtin_va_arg(*pap, ptrdiff_t)
    : px->qual == 'z' ? (ptrdiff_t)__builtin_va_arg(*pap, size_t)
  ...
  if (px->v.li < 0)
   ac[px->n0++] = '-';
  else if (px->flags & 0x02)
   ac[px->n0++] = '+';
  else if (px->flags & 0x01)
   ac[px->n0++] = ' ';
  px->s = &ac[px->n0];
  _Litob(px, code);
  break;
 ...
 case 'x': case 'X':
  ...
  for (; 0 <= px->argno; --px->argno)
   px->v.uli = px->qual == 'l' ? __builtin_va_arg(*pap, unsigned long)
    : px->qual == 'q' ? __builtin_va_arg(*pap, _ULonglong)
    : px->qual == 'j' ? __builtin_va_arg(*pap, uintmax_t)
    : px->qual == 't' ? __builtin_va_arg(*pap, ptrdiff_t)
    : px->qual == 'z' ? __builtin_va_arg(*pap, size_t)
    : __builtin_va_arg(*pap, unsigned int);
  if (px->qual == 'h')
   px->v.uli = (unsigned short)px->v.li;
  else if (px->qual == 'b')
   px->v.uli = (unsigned char)px->v.li;
  else if (px->qual == 't' || px->qual == 'z')
   px->v.uli = (size_t)px->v.uli;
  if (px->flags & 0x08 && px->v.uli != 0
   && (code == 'x' || code == 'X'))
 ...
 case 'a': case 'A':
  ...
  for (; 0 <= px->argno; --px->argno)
   px->v.ld = px->qual == 'L'
    ? __builtin_va_arg(*pap, long double) : __builtin_va_arg(*pap, double);
  if ((*_LPmsw(&(px->v.ld)) & ((unsigned short)0x8000)))
   ac[px->n0++] = '-';
  else if (px->flags & 0x02)
   ac[px->n0++] = '+';
  else if (px->flags & 0x01)
 ...
 case 'n':
  if (px->secure)
   return ((-1) - _Fail_s("printf_s: %n disallowed", sizeof ("printf_s: %n disallowed")));

  switch (px->qual)
   {
   case 'b':
    {signed char *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, signed char *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (signed char)px->nchar; }
    break;

   case 'q':
    {_Longlong *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, _Longlong *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (_Longlong)px->nchar; }
    break;

   case 'j':
    {intmax_t *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, intmax_t *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (intmax_t)px->nchar; }
    break;

   case 't':
    {ptrdiff_t *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, ptrdiff_t *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (ptrdiff_t)px->nchar; }
    break;

   case 'z':
    {size_t *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, size_t *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (size_t)px->nchar; }
    break;

   case 'h':
    {short *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, short *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (short)px->nchar; }
    break;

   case 'l':
    {long *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, long *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (long)px->nchar; }
    break;

   default:
    {int *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, int *); if (p == 0) return ((-1) - _Fail_s("printf: bad %n argument", sizeof ("printf: bad %n argument"))); *p = (int)px->nchar; }
   }
  break;
 ...
 case 'p':
  ...
  for (; 0 <= px->argno; --px->argno)
   pv = __builtin_va_arg(*pap, void *);
  px->v.uli = (_ULonglong)((char *)pv - (char *)0);
  if (vpsize == sizeof (unsigned long))
   px->v.uli &= 0xffffffffUL;
  px->s = &ac[px->n0];
  _Litob(px, 'x');
   }
  break;
 ...
 case 's':
  if (px->qual != 'l')
   ...
   for (; 0 <= px->argno; --px->argno)
    px->s = __builtin_va_arg(*pap, char *);
   if (px->s == 0)
    return ((-1) - _Fail_s("printf_s: bad %s argument", sizeof ("printf_s: bad %s argument")));

   px->n1 = (int)(px->prec < 0 ? strlen(px->s)
   ...
   for (; 0 <= px->argno; --px->argno)
    ws1 = __builtin_va_arg(*pap, wchar_t *);
   if (ws1 == 0)
    return ((-1) - _Fail_s("printf_s: bad %ls argument", sizeof ("printf_s: bad %ls argument")));
