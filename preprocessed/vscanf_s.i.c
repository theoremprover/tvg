# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vscanf_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vscanf_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/vscanf_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/vscanf_s.c" 2
...
int (vscanf_s)(const char * fmt, va_list ap)
 {
 int ans;

 if (fmt == 0)
  return (_Fail_s("vscanf_s: bad argument", sizeof ("vscanf_s: bad argument")), (-1));

 (void)0;
 ans = _Scanf(&scin, (& _Stdin), fmt, ap, 1);
 (void)0;
 return (ans);
 }
