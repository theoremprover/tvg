# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vfwprintf_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vfwprintf_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/vfwprintf_s.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/vfwprintf_s.c" 2
...
static void *prout(void *arg, const wchar_t *buf, size_t n)
 ...
 for (; 0 < n; --n, ++buf)
  if (fputwc(*buf, str) == (( wint_t)(-1)))
   return (0);
 return (str);
 }

int (vfwprintf_s)(FILE * str, const wchar_t * fmt,
 va_list ap)
 {
 int ans;

 if (str == 0 || fmt == 0)
  return (_Fail_s("vfwprintf_s: bad argument", sizeof ("vfwprintf_s: bad argument")), (-1));

 (void)0;
 ans = _WPrintf(&prout, str, fmt, ap, 1);
 (void)0;
 return (ans);
 }
