# 1 "/arm-libs/library-src/dinkum/source/./xmakesta.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xmakesta.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xmakesta.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xmakesta.c" 2
...
int _Makestab(_Linfo *p, const _Locitem *q, const char *s)
 ...
 if (*s != ',')
  lo = stno, stno = 0;
 else if (q->_Code != L_STATE || 16 <= stno
 ...
 if (((s = _Locsum(s, &val)) == 0 && inc == 0)
  || (s != 0 && *s != '\0'))
  return (0);
 uspp = &(*(unsigned short * *)((char *)p + q->_Offset))
  + (int)(stno & 0xf);
 if (q->_Code == L_TABLE)
  usp = ((*(short * *)((char *)p + q->_Offset)) != (*(short * *)((char *)&_Clocale + q->_Offset))) ? *uspp : 0;
 else
  usp = *uspp != 0 && (*uspp)[-1] ? *uspp : 0;
 if (usp == 0)
  {
  if ((usp = (unsigned short *)malloc(((0xff + 2) * sizeof (short)))) == 0)
   return (0);
  if ((*(short * *)((char *)p + q->_Offset)) != 0)
   memcpy(usp, (*(short * *)((char *)p + q->_Offset)) - 1, ((0xff + 2) * sizeof (short)));
  else
   memset(usp, 0, ((0xff + 2) * sizeof (short)));
  if (q->_Code == L_STATE)
   usp[0] = 1;
  *uspp = ++usp;
  }
