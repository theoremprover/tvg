# 1 "/arm-libs/library-src/dinkum/source/./cprojf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cprojf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cprojf.c" 2
...
_Fcomplex (cprojf)(_Fcomplex x)
 {
 float re = crealf(x);
 float im = cimagf(x);
 const short err_re = _FDtest(&re);
 const short err_im = _FDtest(&im);

 return (err_re != 1 && err_im != 1 ? x
  : _FCbuild(_FInf._Float, ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -_FZero : 0.0F));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cprojf.c" 2
