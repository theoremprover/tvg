# 1 "/arm-libs/library-src/dinkum/source/./expm1.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./expm1.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./expm1.c" 2
...
static const double s[] = {
 0.00000259766943573417,
 0.00011305370687423613,
 0.00109066792970778349,
 0.02784564909329514297,
 0.03926190614812718186,
 0.92389869353018596046,
 };

static const double c[] = {
 -0.00001757061517257875,
 0.00060499786991627820,
 -0.00956033110459000699,
 0.08520625381341306649,
 -0.42268744061696587854,
 0.92389869353018596365,
 };
# 109 "/arm-libs/library-src/dinkum/source/./xxexpm1.h"
static const double ln3_2 = 0.40546510810816438197801311546434914;
static const double ln2 = 0.69314718055994530941723212145817657;
static const double ln3 = 1.09861228866810969139524523692252573;
static const double ln4 = 1.38629436111989061883446424291635315;
static const double ln5 = 1.60943791243410037460075933322618765;

static double _Expm1_approx(double x)
 {
...
double (expm1)(double x)
 ...
 case 1:
  return (((*_Pmsw(&(x))) & ((unsigned short)0x8000)) ? -1.0 : x);

 default:
  if (-_Eps._Double <= x && x <= _Eps._Double)
   return (x);
  else if (x < -ln2)
   return (exp(x) - 1.0);
  else if (x < ln3_2)
   return (_Expm1_approx(x));
  else if (x < ln3)
   return (2.0 * _Expm1_approx(x - ln2) + 1.0);
  else if (x < ln5)
   return (4.0 * _Expm1_approx(x - ln4) + 3.0);
  else
   return (exp(x) - 1.0);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./expm1.c" 2
