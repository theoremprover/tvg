# 1 "/arm-libs/library-src/dinkum/source/./xisdst.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xisdst.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xisdst.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xisdst.c" 2
...
time_t _Tzoff(void);
# 6 "/arm-libs/library-src/dinkum/source/./xisdst.c" 2
...
static dst_t olddst = 0; static int (*_Tls_setup_olddst)(void) = 0;
static rules_t rules = 0; static _Once_t _Tls_once_rules = 0; static void _Tls_dtor_rules(void) { free_tls(&(rules)); } static void _Tls_reg_rules(void) { _Atexit(_Tls_dtor_rules); } static int _Tls_setupx_rules(void) { if (*(&_Tls_once_rules) == 0) (_Tls_reg_rules)(), *(&_Tls_once_rules) = 2; return (1); } static int (*_Tls_setup_rules)(void) = _Tls_setupx_rules;

int _Isdst(const struct tm *t)
 {
 Dstrule *pr;
 dst_t *polddst = ((_Tls_setup_olddst && _Tls_setup_olddst()), (&(olddst)));
 rules_t *prules = ((_Tls_setup_rules && _Tls_setup_rules()), (&(rules)));
 _Tinfo *ptimes = ((_Tls_setup__Times && _Tls_setup__Times()), (&(_Times)));
 ...
 const int wd0 = (365L * t->tm_year + d0 + 1) % 7 + 14;
