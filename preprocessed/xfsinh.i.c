# 1 "/arm-libs/library-src/dinkum/source/./xfsinh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfsinh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfsinh.c" 2
...
static const float ln3by2 = 0.54930614433405484569762261846126284F;
static const float xbig = (float)(0.308 * (24 + 1));

static float _FSinh_small(float x)
 {
 const float z = x * x;
 const float ch = 1.0F + z * (c[0] * z + c[1]);

 x += x * z * (s[0]);
 return (((x) / (ch)));
 }

float _FSinh(float x, float y)
 {
...
# 4 "/arm-libs/library-src/dinkum/source/./xfsinh.c" 2
