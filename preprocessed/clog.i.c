# 1 "/arm-libs/library-src/dinkum/source/./clog.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./clog.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./clog.c" 2
...
_Dcomplex (clog)(_Dcomplex x)
 ...
 if (errx == 2)
  {
  if (erry == 1)
   return (_Cbuild(_Inf._Double, re));
  else
   return (_Cbuild(re, re));
  }
 else if (erry == 2)
  {
  if (errx == 1)
   return (_Cbuild(_Inf._Double, im));
  else
   return (_Cbuild(im, im));
  }

 theta = atan2(im, re);

 if (errx == 1 || erry == 1)
  return (_Cbuild(_Inf._Double, theta));
 else
  {
  ...
  if (re == 0.0)
   ...
   _Dint(&re0, (53 - 1) / 2);
   _Dint(&im0, (53 - 1) / 2);
   norm1 = (re0 * re0 - 1.0) + im0 * im0;
   norm1 += (re - re0) * (re + re0) + (im - im0) * (im + im0);
   return (_Cbuild(0.5 * log1p(norm1), theta));
   }
  else
   {
   double eps = ((im) / (re));

   return (_Cbuild(log(re)
    + 0.5 * log1p(eps * eps), theta));
   }
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./clog.c" 2
