# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpfile_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpfile_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpfile_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpfile_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpfile_s.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpfile_s.c" 2


errno_t (tmpfile_s)(FILE * * pstr)
 {
 FILE *str;
 char fn[16], *s;
 errno_t errcode;
 ...
 if (pstr == 0)
  return (_Fail_s("tmpfile_s: bad argument", sizeof ("tmpfile_s: bad argument")));
 else if ((errcode = tmpnam_s(fn, 16)) != 0)
  return (errcode);
 else
  {
  str = fopen(fn, "wb+");
  ...
  *pstr = str;
  if (str == 0)
   return (_Fail_s("tmpfile_s: can't create file", sizeof ("tmpfile_s: can't create file")));
  else
   return (0);
  }
 }
