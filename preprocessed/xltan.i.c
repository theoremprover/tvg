# 1 "/arm-libs/library-src/dinkum/source/./xltan.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xltan.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xltan.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxtan.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxtan.h" 2


unsigned int _LQuad(long double *px, int retcode);
# 25 "/arm-libs/library-src/dinkum/source/./xxxtan.h"
static const long double c[] = {
 -0.00020844956382258822L,
 0.02334489464693293380L,
 -0.46161689768996201755L,
 };
static const long double s[] = {
 -0.00000748373924372997L,
 0.00280592875035233052L,
 -0.12828356435663158978L,
 };
# 79 "/arm-libs/library-src/dinkum/source/./xxxtan.h"
static long double _LTan_approx(long double x, unsigned int invert)
 {
 const long double z = x * x;
 const long double xd = 1.0L + z * ((c[0] * z + c[1]) * z + c[2]);
...
long double (_LTan)(long double x, int retcode)
 {
 switch (_LDtest(&x))
  {
 case 2:
 ...
 default:
   {
  unsigned int invert = _LQuad(&x, retcode) & 0x1;
  ...
  if (x < _LRteps._Long_double)
   ...
   if (invert)
    ...
    _LDunscale(&xexp, &x);
    x = -(1.0L / (x));
    if (0 < _LDscale(&x, -xexp))
     _Feraise(0x04);

    invert = 0;
    }
   }
# 146 "/arm-libs/library-src/dinkum/source/./xxxtan.h"
  else
   {
   x = _LTan_approx(x, invert);
   invert = 0;
   }
...
# 4 "/arm-libs/library-src/dinkum/source/./xltan.c" 2
