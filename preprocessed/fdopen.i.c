# 1 "/arm-libs/library-src/dinkum/source/./fdopen.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fdopen.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fdopen.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./fdopen.c" 2


FILE *(fdopen)(signed char fd, const char *mods)
 {
 if (fd < 0 || 20 <= fd)
  return (0);
 ...
 if (_Files[fd] == 0)
  ...
  if ((_Files[fd] = (FILE *)malloc(sizeof (FILE))) == 0)
   return (0);
  _Files[fd]->_Mode = 0x80;
  _Files[fd]->_Idx = (unsigned char)fd;
  }
 else if (_Files[fd]->_Mode == 0)
  {
  _Files[fd]->_Mode = (unsigned short)~0x80;
  }

 if ((_Files[fd]->_Mode & (0x1 | 0x2)) != 0)
  return (_Files[fd]);
 else
  return (_Foprep(0, mods, _Files[fd], fd, 0));
 }
