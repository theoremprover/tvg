# 1 "/arm-libs/library-src/dinkum/source/./atanhf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./atanhf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./atanhf.c" 2
...
float (atanhf)(float x)
 ...
 default:
  ...
  if (1.0F < x)
   ...
   {
   _Feraise(0x02);
   return (neg ? -_FInf._Float : _FInf._Float);
   }
  else if (-_FRteps._Float < x && x < _FRteps._Float)
   {
   if (neg)
    ((*_FPmsw(&(x))) ^= ((unsigned short)0x8000));
   return (x);
   ...
   {
   y = 0.5F * log1pf(2.0F * ((x) / ((1.0F - x))));

   return (neg ? -y : y);
   }
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./atanhf.c" 2
