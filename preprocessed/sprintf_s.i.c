# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/sprintf_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/sprintf_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/sprintf_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/sprintf_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/sprintf_s.c" 2


typedef struct _Spstuff
 {
 char *s;
 size_t size;
 int fail;
...
int (sprintf_s)(char * s, rsize_t size,
 const char * fmt, ...)
 {
 int ans;
 _Spstuff x;
 ...
 if (s == 0 || size == 0 || ((_Sizet)(-1) >> 1) < size)
  return (_Fail_s("sprintf_s: bad argument", sizeof ("sprintf_s: bad argument")), 0);
 else if (fmt == 0)
  {
  *s = '\0';
  return (_Fail_s("sprintf_s: bad format argument", sizeof ("sprintf_s: bad format argument")), 0);
  }
 ...
 if (x.fail)
  return (_Fail_s("sprintf_s: buffer overflow", sizeof ("sprintf_s: buffer overflow")), 0);
 else if (ans < (-1))
  {
  *s = '\0';
  return (0);
