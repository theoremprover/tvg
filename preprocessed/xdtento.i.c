# 1 "/arm-libs/library-src/dinkum/source/./xdtento.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xdtento.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xdtento.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxdtent.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxdtent.h" 2
# 36 "/arm-libs/library-src/dinkum/source/./xxxdtent.h"
static const double tenth[] =
 {
 (double)(6710886.0 / 67108864.0L),
 (double)(26843545.0 / (67108864.0L * 67108864.0L)),
 (double)(40265318.0 / (67108864.0L * 67108864.0L) / 67108864.0L),
 (double)(26843545.0 / (67108864.0L * 67108864.0L) / (67108864.0L * 67108864.0L)),
 };
# 70 "/arm-libs/library-src/dinkum/source/./xxxdtent.h"
double _Dtento(double *xpx, long n, int *perr)
 {
 double xpf[4];
 double x;

 if (n == 0 || xpx[0] == 0.0)
  return (_Xp_getw(xpx, 4));
 ...
 if (0 < n)
  ...
  {
  n = -n;
  _Xp_movx(xpf, 4, tenth);
  _Xp_ldexpx(xpx, 4, (4 * (53 / 2)));
  }
 ...
 for (; ; )
  ...
  if (n == 0)
   break;
  _Xp_movx(xpw, 4, xpf);
  _Xp_mulx(xpf, 4, xpw, 4, xpt);
  }

 if (xpf[0] < 1.0)
  _Xp_ldexpx(xpx, 4, -(4 * (53 / 2)));
 x = _Xp_getw(xpx, 4);
 if (x == 0.0 || x == _Inf._Double || x == -_Inf._Double)
  {
  ( _Errno) = 0x0022;
  if (perr != 0)
   *perr |= 1;
...
# 4 "/arm-libs/library-src/dinkum/source/./xdtento.c" 2
