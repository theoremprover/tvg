# 1 "/arm-libs/library-src/dinkum/source/./nexttowardl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./nexttowardl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./nexttowardl.c" 2


long double (nexttowardl)(long double x, long double y)
 {
 _Lval *px = (_Lval *)(char *)&x;
 short errx = _LDtest(&x);

 if (errx == 2)
  ;
 else if (_LDtest(&y) == 2 || x == y)
  x = y;
 else if (errx == 1)
  {

  px->_Sh[3] = (px->_Sh[3] & ((unsigned short)0x8000)) | (((unsigned short)((1 << (15 - 4)) - 1)) << 4) - 1;
  px->_Sh[2] = 0xffff;
  px->_Sh[1] = 0xffff;
  px->_Sh[0] = 0xffff;
# 39 "/arm-libs/library-src/dinkum/source/./nexttowardl.c"
  }
 else if (errx == 0)
  {

  px->_Sh[3] = x < y ? 0 : ((unsigned short)0x8000);
  px->_Sh[0] = 1;
# 54 "/arm-libs/library-src/dinkum/source/./nexttowardl.c"
  }
 else if ((x < y && 0.0L < x) || (y < x && x < 0.0L))
  {

  if ((++px->_Sh[0] & 0xffff) == 0
   && (++px->_Sh[1] & 0xffff) == 0
   && (++px->_Sh[2] & 0xffff) == 0
   && (++px->_Sh[3] & ~((unsigned short)0x8000)) == ((unsigned short)((1 << (15 - 4)) - 1)) << 4)
   _Feraise(0x04);
# 86 "/arm-libs/library-src/dinkum/source/./nexttowardl.c"
  }


 else if ((--px->_Sh[0] & 0xffff) == 0xffff
  && (--px->_Sh[1] & 0xffff) == 0xffff
  && (--px->_Sh[2] & 0xffff) == 0xffff)
  --px->_Sh[3];
 if ((px->_Sh[3] & ((unsigned short)(0x7fff & ~((unsigned short)((1 << 4) - 1))))) == 0)
  _Feraise(0x08);
# 126 "/arm-libs/library-src/dinkum/source/./nexttowardl.c"
 return (x);
 }
