# 1 "/arm-libs/library-src/dinkum/source/./expm1l.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./expm1l.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./expm1l.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxexpm1.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxexpm1.h" 2
# 32 "/arm-libs/library-src/dinkum/source/./xxexpm1.h"
static const long double s[] = {
 0.00000259766943573417L,
 0.00011305370687423613L,
 0.00109066792970778349L,
 0.02784564909329514297L,
 0.03926190614812718186L,
 0.92389869353018596046L,
 };

static const long double c[] = {
 -0.00001757061517257875L,
 0.00060499786991627820L,
 -0.00956033110459000699L,
 0.08520625381341306649L,
 -0.42268744061696587854L,
 0.92389869353018596365L,
 };
# 109 "/arm-libs/library-src/dinkum/source/./xxexpm1.h"
static const long double ln3_2 = 0.40546510810816438197801311546434914L;
static const long double ln2 = 0.69314718055994530941723212145817657L;
static const long double ln3 = 1.09861228866810969139524523692252573L;
static const long double ln4 = 1.38629436111989061883446424291635315L;
static const long double ln5 = 1.60943791243410037460075933322618765L;

static long double _LExpm1_approx(long double x)
 {
 return ((((((((s[0] * x + s[1]) * x + s[2]) * x + s[3]) * x + s[4]) * x + s[5]) * x) / ((((((c[0] * x + c[1]) * x + c[2]) * x + c[3]) * x + c[4]) * x + c[5]))));
 }
...
long double (expm1l)(long double x)
 ...
 case 1:
  return (((*_LPmsw(&(x))) & ((unsigned short)0x8000)) ? -1.0L : x);

 default:
  if (-_LEps._Long_double <= x && x <= _LEps._Long_double)
   return (x);
  else if (x < -ln2)
   return (expl(x) - 1.0L);
  else if (x < ln3_2)
   return (_LExpm1_approx(x));
  else if (x < ln3)
   return (2.0L * _LExpm1_approx(x - ln2) + 1.0L);
  else if (x < ln5)
   return (4.0L * _LExpm1_approx(x - ln4) + 3.0L);
  else
   return (expl(x) - 1.0L);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./expm1l.c" 2
