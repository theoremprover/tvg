# 1 "/arm-libs/library-src/dinkum/source/./catanhl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./catanhl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./catanhl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcatanh.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxcatanh.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxcatanh.h" 2
...
static const long double big = 3.35e153L;
# 21 "/arm-libs/library-src/dinkum/source/./xxcatanh.h"
static const long double piby2 = 1.5707963267948966192313216916397514L;

_Lcomplex (catanhl)(_Lcomplex x)
 {
 long double re = creall(x);
 long double im = cimagl(x);
 short errx = _LDtest(&re);
 short erry = _LDtest(&im);
 long double u, v;
 ...
 if (errx == 2)
  ...
  if (errx == 1 || errx == 0)
   {
   u = ((*_LPmsw(&(re))) & ((unsigned short)0x8000)) ? -_LZero : 0.0L;
   v = im;
   }
  else
   {
  ...
  {
  u = re < 0.0L ? -_LZero : 0.0L;
  v = ((*_LPmsw(&(im))) & ((unsigned short)0x8000)) ? -piby2 : piby2;
  }
 else
  {
  long double magim = ((*_LPmsw(&(im))) & ((unsigned short)0x8000)) ? -im : im;
  int neg;

  if (re < 0.0L)
   {
   neg = 1;
   ((*_LPmsw(&(re))) ^= ((unsigned short)0x8000));
   }
  else
   {
   neg = 0;
   ((*_LPmsw(&(im))) ^= ((unsigned short)0x8000));
   }
  ...
  if (big < re)
   ...
   {
   long double refrom1 = 1.0L - re;
   long double imeps2 = magim * magim;

   u = 0.25L * log1pl(((4.0L * re) / ((refrom1 * refrom1 + imeps2))));

   v = 0.5L * atan2l(2.0L * im,
    refrom1 * (1.0L + re) - imeps2);
   }
  else if (im == 0.0L)
   {
   _Feraise(0x02);
   u = _LInf._Long_double;
   v = im;
   }
  else
   {
   u = logl(((sqrtl(sqrtl(4.0L + im * im))) / (sqrtl(magim))));

   v = 0.5L * (piby2 + atan2l(magim, 2.0L));
   ((*_LPmsw(&(v))) &= ~((unsigned short)0x8000));
   }

  if (neg)
   ((*_LPmsw(&(u))) ^= ((unsigned short)0x8000));
  else
   ((*_LPmsw(&(v))) ^= ((unsigned short)0x8000));
  }
 return (_LCbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./catanhl.c" 2
