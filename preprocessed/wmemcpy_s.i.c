# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wmemcpy_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wmemcpy_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/wmemcpy_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/wmemcpy_s.c" 2


errno_t (wmemcpy_s)(wchar_t * s1, rsize_t maxsize,
 const wchar_t * s2, rsize_t n)
 {
 if (s1 == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("wmemcpy_s: bad destination argument", sizeof ("wmemcpy_s: bad destination argument")));
 else if (s2 == 0 || ((_Sizet)(-1) >> 1) < n || maxsize < n
  || (s1 < s2 + n && s2 < s1 + maxsize))
  {
  wmemset(s1, L'\0', maxsize);
  return (_Fail_s("wmemcpy_s: bad argument", sizeof ("wmemcpy_s: bad argument")));
  }
