# 1 "/arm-libs/library-src/dinkum/source/./xfdtento.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfdtento.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfdtento.c" 2
...
# 47 "/arm-libs/library-src/dinkum/source/./xxxdtent.h"
static const float tenth[] =
 {
 (float)(409.0F / 4096.0L),
 (float)(2457.0F / (4096.0L * 4096.0L)),
 (float)(2457.0F / (4096.0L * 4096.0L) / 4096.0L),
 (float)(2457.0F / (4096.0L * 4096.0L) / (4096.0L * 4096.0L)),
 };
# 70 "/arm-libs/library-src/dinkum/source/./xxxdtent.h"
float _FDtento(float *xpx, long n, int *perr)
 {
 float xpf[4];
 float x;

 if (n == 0 || xpx[0] == 0.0F)
  return (_FXp_getw(xpx, 4));

 if (0 < n)
  _FXp_setw(xpf, 4, 10.0);
 else
  {
  n = -n;
  _FXp_movx(xpf, 4, tenth);
  _FXp_ldexpx(xpx, 4, (4 * (24 / 2)));
  }
 ...
 for (; ; )
  ...
  if (n == 0)
   break;
  _FXp_movx(xpw, 4, xpf);
  _FXp_mulx(xpf, 4, xpw, 4, xpt);
  }

 if (xpf[0] < 1.0F)
  _FXp_ldexpx(xpx, 4, -(4 * (24 / 2)));
 x = _FXp_getw(xpx, 4);
 if (x == 0.0F || x == _FInf._Float || x == -_FInf._Float)
  {
  ( _Errno) = 0x0022;
  if (perr != 0)
   *perr |= 1;
...
# 4 "/arm-libs/library-src/dinkum/source/./xfdtento.c" 2
