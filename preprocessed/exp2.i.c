# 1 "/arm-libs/library-src/dinkum/source/./exp2.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./exp2.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./exp2.c" 2
...
static const double ln2 = 0.69314718055994530941723212145817658;
...
double (exp2)(double x)
 ...
 case 1:
  return (((*_Pmsw(&(x))) & ((unsigned short)0x8000)) ? 0.0 : x);

 case 0:
  return (1.0);

 default:
  if (x <= (double)(-0x7fffffffL - 1))
   return (0.0);
  else if ((double)0x7fffffffL <= x)
   return (_Inf._Double);
  else
   {
   xexp = (long)x;
...
# 4 "/arm-libs/library-src/dinkum/source/./exp2.c" 2
