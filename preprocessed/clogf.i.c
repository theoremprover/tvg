# 1 "/arm-libs/library-src/dinkum/source/./clogf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./clogf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./clogf.c" 2
...
_Fcomplex (clogf)(_Fcomplex x)
 ...
 if (errx == 2)
  {
  if (erry == 1)
   return (_FCbuild(_FInf._Float, re));
  else
   return (_FCbuild(re, re));
  }
 else if (erry == 2)
  {
  if (errx == 1)
   return (_FCbuild(_FInf._Float, im));
  else
   return (_FCbuild(im, im));
  }

 theta = atan2f(im, re);

 if (errx == 1 || erry == 1)
  return (_FCbuild(_FInf._Float, theta));
 else
  {
  ...
  if (re == 0.0F)
   ...
   _FDint(&re0, (24 - 1) / 2);
   _FDint(&im0, (24 - 1) / 2);
   norm1 = (re0 * re0 - 1.0F) + im0 * im0;
   norm1 += (re - re0) * (re + re0) + (im - im0) * (im + im0);
   return (_FCbuild(0.5F * log1pf(norm1), theta));
   }
  else
   {
   float eps = ((im) / (re));

   return (_FCbuild(logf(re)
    + 0.5F * log1pf(eps * eps), theta));
   }
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./clogf.c" 2
