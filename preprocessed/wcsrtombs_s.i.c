# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c" 2
# 1 "/toolchain/arm/include/limits.h" 1 3
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsrtombs_s.c" 2


errno_t (wcsrtombs_s)(size_t * psize,
 ...
 if (psize == 0 || pwcs == 0 || *pwcs == 0 || pst == 0
  || (s == 0 && 0 < maxsize)
  || (s != 0
  ...
  if (psize != 0)
   *psize = (size_t)(-1);
  return (_Fail_s("wcsrtombs_s: bad argument", sizeof ("wcsrtombs_s: bad argument")));
  }
 ...
 for (ps = s; 0 < n && 0 < maxsize;
  nc += i, ++wcs, s += i, n -= i, maxsize -= i)
  {
  char *t;
  mbstate_t mbstsave = *pst;

  if ( _Getmbcurmax() <= n && _Getmbcurmax() <= maxsize)
   t = s;
  else
   t = buf;
  ...
  if (s == t)
   ;
  else if (n < (size_t)i || maxsize < (size_t)i)
   {
   *pst = mbstsave;
   maxsize = 0;
   break;
 ...
 *pwcs = wcs;
  ...
  *psize = nc;
  ...
  s[0] = '\0';
  *psize = (size_t)-1;
  return (_Fail_s("wcstombs_s: buffer too small", sizeof ("wcstombs_s: buffer too small")));
  }
 }
