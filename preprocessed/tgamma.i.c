# 1 "/arm-libs/library-src/dinkum/source/./tgamma.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./tgamma.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./tgamma.c" 2
...
double (tgamma)(double x)
 ...
 case 0:
  if (x == 0.0)
   {
   _Feraise(0x02);
   return (((*_Pmsw(&(x))) & ((unsigned short)0x8000)) ? -_Inf._Double : _Inf._Double);
   }
  else if (x < 0.0)
   {
   _Feraise(0x01);
 ...
 default:
  if (x <= -174.0)
   ...
   if (x - z < -1.0)
    y = -y;
   z = ((pi) / ((-x * _Sinx(pi * y, 0, 0) * (-x - 1.0))));
   z = ((z) / ((1.0 + _Tgamma(&w, &xexp))));
   z *= w;

   switch (_Dscale(&z, xexp))
    {
   case 0:
    _Feraise(0x08);
    break;
   ...
   {
   short xexp;
   double y = _Tgamma(&x, &xexp);

   x = ((1.0 + y) / (x));
   switch (_Dscale(&x, -xexp))
    {
   case 0:
    _Feraise(0x08);
    break;
...
# 4 "/arm-libs/library-src/dinkum/source/./tgamma.c" 2
