# 1 "/arm-libs/library-src/dinkum/source/./setvbuf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./setvbuf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./setvbuf.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./setvbuf.c" 2


int (setvbuf)(FILE * str, char * abuf,
 ...
 if (str->_Mode & 0x40)
  free(str->_Buf), str->_Mode &= ~0x40;
 str->_Mode = (unsigned short)((str->_Mode & ~(0x400 | 0x800)) | mode);
 str->_Buf = buf;
 str->_Bend = buf + size;
 str->_Next = buf;
