# 1 "/arm-libs/library-src/dinkum/source/./xwfrprep.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwfrprep.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xwfrprep.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 4 "/arm-libs/library-src/dinkum/source/./xwfrprep.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xwfrprep.c" 2
...
int _WFrprep(FILE *str)
 ...
 if (str->_Next < str->_WRend)
  return (1);
 else if (str->_Mode & 0x100)
  return (0);
 else if ((str->_Mode & (0x1 | 0x2000 | 0x4000)) != 0x1)
  {
  str->_Mode |= str->_Mode & 0x4000
   ? 0x200 : 0x200 | 0x8000;
  return (-1);
  }
 if ((str->_Mode & (0x800 | 0x400)) != 0 || str->_Buf != &str->_Cbuf)
  ;
 else if ((str->_Buf = (unsigned char *)malloc(512)) == 0)
  {
  str->_Buf = &str->_Cbuf;
 ...
 str->_WWend = str->_Buf;
  {
 int n = (int)read(((str)->_Handle), str->_Buf, str->_Bend - str->_Buf);

 if (n < 0)
  {
  str->_Mode |= 0x200 | 0x8000;
  return (-1);
  }
 else if (n == 0)
  {
  str->_Mode =
   (unsigned short)((str->_Mode & ~0x1000) | 0x100 | 0x8000);
  return (0);
  }
 else
  {
  str->_Mode |= 0x1000 | 0x8000;
  str->_WRend += n;
  return (1);
  }
  }
