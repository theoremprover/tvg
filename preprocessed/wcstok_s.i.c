# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcstok_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcstok_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/wcstok_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/wcstok_s.c" 2


static int wcsspn_s(const wchar_t *s1, const wchar_t *s2, size_t *psize)
 {
 const wchar_t *sc1, *sc2;
 size_t n = *psize;

 for (sc1 = s1; 0 < n && *sc1 != L'\0'; --n, ++sc1)
  for (sc2 = s2; ; ++sc2)
   if (*sc2 == L'\0')
    {
    *psize = sc1 - s1;
...
static size_t wcscspn_s(const wchar_t *s1, const wchar_t *s2, size_t *psize)
 {
 const wchar_t *sc1, *sc2;
 size_t n = *psize;
...
wchar_t *(wcstok_s)(wchar_t * s1, rsize_t *psize1,
 const wchar_t * s2, wchar_t ** pbuf1)
 {
 wchar_t *sbegin, *send;
 size_t n = *psize1;

 if (psize1 == 0 || ((_Sizet)(-1) >> 1) < *psize1 || s2 == 0 || pbuf1 == 0)
  return (_Fail_s("wcstok_s: bad argument", sizeof ("wcstok_s: bad argument")), (wchar_t *)0);

 sbegin = s1 != 0 ? s1 : *pbuf1;
 if (wcsspn_s(sbegin, s2, &n))
  return (_Fail_s("wcstok_s: missing token", sizeof ("wcstok_s: missing token")), (wchar_t *)0);
 sbegin += n;
 *psize1 -= n;
 if (*sbegin == L'\0')
  return ((wchar_t *)0);
 ...
 n = *psize1;
 if (wcscspn_s(sbegin, s2, &n))
  return (_Fail_s("wcstok_s: missing delimiter", sizeof ("wcstok_s: missing delimiter")), (wchar_t *)0);
 send = sbegin + n;
 *psize1 -= n;
 if (*send != L'\0')
  {
