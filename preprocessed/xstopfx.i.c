# 1 "/arm-libs/library-src/dinkum/source/./xstopfx.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xstopfx.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xstopfx.c" 2
...
int isblank(int);
# 4 "/arm-libs/library-src/dinkum/source/./xstopfx.c" 2
...
int _Stopfx(const char **ps, char **endptr)
 ...
 for (; (_Ctype[(int)((unsigned char)*s)] & (0x40|0x04|0x100)); ++s)
  ;
 if (*s == '-')
  code = 8, ++s;
 else if (*s == '+')
 ...
 if (*s == 'n' || *s == 'N')
  ...
  if ((*++s != 'a' && *s != 'A') || (*++s != 'n' && *s != 'N'))
   ...
   if (*q == '(')
    {
    for (; (_Ctype[(int)((unsigned char)*++q)] & (0x20|0x10|0x02|0x200)) || *q == '_'; )
     ;
    if (*q == ')')
     s = ++q;
    }
  ...
  if (*s1 == '.')
   ++s1;
  if (!(_Ctype[(int)((unsigned char)*s1)] & 0x01))
   code |= 1;
  else
   s += 2, code |= 2;
  }
