# 1 "/arm-libs/library-src/dinkum/source/./catanh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./catanh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./catanh.c" 2
...
static const double big = 3.35e153;
# 21 "/arm-libs/library-src/dinkum/source/./xxcatanh.h"
static const double piby2 = 1.5707963267948966192313216916397514;

_Dcomplex (catanh)(_Dcomplex x)
 {
 double re = creal(x);
 double im = cimag(x);
 ...
 if (errx == 2)
  ...
  if (errx == 1 || errx == 0)
   {
   u = ((*_Pmsw(&(re))) & ((unsigned short)0x8000)) ? -_Zero : 0.0;
   v = im;
   }
  else
   {
  ...
  {
  u = re < 0.0 ? -_Zero : 0.0;
  v = ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -piby2 : piby2;
  }
 else
  {
  double magim = ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -im : im;
  int neg;

  if (re < 0.0)
   {
   neg = 1;
   ((*_Pmsw(&(re))) ^= ((unsigned short)0x8000));
   }
  else
   {
   neg = 0;
   ((*_Pmsw(&(im))) ^= ((unsigned short)0x8000));
   }
  ...
  if (big < re)
   ...
   {
   double refrom1 = 1.0 - re;
   double imeps2 = magim * magim;

   u = 0.25 * log1p(((4.0 * re) / ((refrom1 * refrom1 + imeps2))));

   v = 0.5 * atan2(2.0 * im,
    refrom1 * (1.0 + re) - imeps2);
   }
  else if (im == 0.0)
   {
   _Feraise(0x02);
   ...
   {
   u = log(((sqrt(sqrt(4.0 + im * im))) / (sqrt(magim))));

   v = 0.5 * (piby2 + atan2(magim, 2.0));
   ((*_Pmsw(&(v))) &= ~((unsigned short)0x8000));
   }

  if (neg)
   ((*_Pmsw(&(u))) ^= ((unsigned short)0x8000));
  else
   ((*_Pmsw(&(v))) ^= ((unsigned short)0x8000));
  }
 return (_Cbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./catanh.c" 2
