# 1 "/arm-libs/library-src/dinkum/source/./fwide.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fwide.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fwide.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 6 "/arm-libs/library-src/dinkum/source/./fwide.c" 2
...
int (fwide)(FILE *str, int mode)
 ...
 (void)0;
 if (mode < 0 && !(str->_Mode & 0x8000))
  str->_Mode |= 0x4000;
 else if (0 < mode && !(str->_Mode & 0x4000))
  str->_Mode |= 0x8000;
 ans = str->_Mode & 0x4000 ? -1
  : str->_Mode & 0x8000 ? 1 : 0;
 (void)0;
 return (ans);
 }
