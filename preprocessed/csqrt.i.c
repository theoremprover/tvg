# 1 "/arm-libs/library-src/dinkum/source/./csqrt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./csqrt.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./csqrt.c" 2
...
_Dcomplex (csqrt)(_Dcomplex x)
 ...
 if (erry == 1)
  ...
  if (re < 0.0)
   return (_Cbuild(im, re));
  else
   return (x);
 else if (re < 0.0)
  return (_Cbuild(0.0,
   ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -_Inf._Double
    : _Inf._Double));
 else
  return (_Cbuild(_Inf._Double,
   ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -_Zero
    : 0.0));

 rho = _Hypot(re, im, &xexp);
 if (xexp == 0)
  return (_Cbuild(rho, im * rho));
 else
  {
  double remag = re;

  ((*_Pmsw(&(remag))) &= ~((unsigned short)0x8000));
  remag = ldexp(remag, -xexp);

  rho = ldexp(sqrt(2.0 * (remag + rho)),
   xexp / 2 - 1);
  if (0.0 <= re)
   return (_Cbuild(rho, ((im) / ((2.0 * rho)))));
  else if (((*_Pmsw(&(im))) & ((unsigned short)0x8000)))
   return (_Cbuild(((-im) / ((2.0 * rho))), -rho));
  else if (im == 0.0)
   return (_Cbuild(0.0, rho));
  else
   return (_Cbuild(((im) / ((2.0 * rho))), rho));
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./csqrt.c" 2
