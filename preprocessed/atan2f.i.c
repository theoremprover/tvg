# 1 "/arm-libs/library-src/dinkum/source/./atan2f.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./atan2f.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./atan2f.c" 2
...
static float _FAtan2_divide(float y, float x)
 {
 float ans = ((y) / (x));
...
float (atan2f)(float y, float x)
 ...
 if (errx <= 0 && erry <= 0)
  ...
  if (x < y)
   z = _FAtan2_divide(x, y), hex ^= 0x2;
  else if (errx < 0)
   z = _FAtan2_divide(y, x);
  else
   z = 0.0F;
  }
 else if (erry == 2)
  ...
  {
  z = errx == erry ? 1.0F : 0.0F;
  hex = ((*_FPmsw(&(y))) & ((unsigned short)0x8000)) ? 0x8 : 0x0;
  if (((*_FPmsw(&(x))) & ((unsigned short)0x8000)))
   hex ^= 0x6;
  if (erry == 1)
   hex ^= 0x2;
...
# 4 "/arm-libs/library-src/dinkum/source/./atan2f.c" 2
