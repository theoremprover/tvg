# 1 "/arm-libs/library-src/dinkum/source/./fputwc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fputwc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fputwc.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./fputwc.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 5 "/arm-libs/library-src/dinkum/source/./fputwc.c" 2
...
wint_t (fputwc)(wchar_t c, FILE *str)
 ...
 for (n = -1; n != 0; )
  ...
  m = (int)(str->_WWend - str->_Next);
  if (m == 0)
   m = 1;

  if (0 < n)
   ;
  else if ((int) _Getmbcurmax() <= m)
  ...
  else if ((n = _Wctomb(s, c, &str->_Wstate)) < 0)
   {
   (void)0;
   return ((( wint_t)(-1)));
   }
