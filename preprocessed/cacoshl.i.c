# 1 "/arm-libs/library-src/dinkum/source/./cacoshl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cacoshl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cacoshl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcacosh.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxcacosh.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxcacosh.h" 2
...
# 21 "/arm-libs/library-src/dinkum/source/./xxcacosh.h"
static const long double pi = 3.1415926535897932384626433832795029L;
static const long double pi3by4 = 2.3561944901923449288469825374596271L;
 ...
 if (errx == 2)
  ...
  if (erry == 1)
   ...
   if (re < 0.0L)
    ...
    v = 0.0L;
  if (((*_LPmsw(&(im))) & ((unsigned short)0x8000)))
   v = -v;
  }
 else if (erry == 1)
  {
  ...
  {
  const _Lcomplex w = csqrtl(_LCbuild(re - 1.0L, -im));
  const _Lcomplex z = csqrtl(_LCbuild(re + 1.0L, im));
  const long double wr = creall(w);
  const long double wi = cimagl(w);
  const long double zr = creall(z);
  const long double zi = cimagl(z);
  int neg;
  ...
  v = 2.0L * atan2l(
   cimagl(csqrtl(_LCbuild(re - 1.0L, im))), zr);
  if (neg)
   ((*_LPmsw(&(v))) ^= ((unsigned short)0x8000));
  }
 return (_LCbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cacoshl.c" 2
