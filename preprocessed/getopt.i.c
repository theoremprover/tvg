# 1 "/arm-libs/library-src/dinkum/source/./getopt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./getopt.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./getopt.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./getopt.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./getopt.c" 2



static const char *optnext = 0;
...
int (getopt)(int argc, char *const argv[], const char *fmt)
 {
 optopt = '\0';
 optarg = 0;
 ...
 if (optnext == 0 && (argv[optind][0] != '-' || argv[optind][1] == '\0'))
  return (-1);
 else if (optnext == 0 && strcmp(argv[optind], "--") == 0)
  {
  ++optind;
  return (-1);
  }
  ...
  if (*s == '\0')
   ...
   if (s[1] != ':')
    ...
    if (opterr != 0 && *fmt != ':')
     fprintf((& _Stderr), "missing option argument for: %c\n",
      optopt);
    }
   }

  if (optnext != 0 && *++optnext == '\0')
   {
   ++optind;
   optnext = 0;
   }
