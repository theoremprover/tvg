# 1 "/arm-libs/library-src/dinkum/source/./xlsin.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlsin.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlsin.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xlsin.c" 2

# 1 "/arm-libs/library-src/dinkum/source/./xxxsin.h" 1




unsigned int _LQuad(long double *px, int quads);
# 22 "/arm-libs/library-src/dinkum/source/./xxxsin.h"
static const long double c[] = {
 0.00000000206374484196L,
 -0.00000027555365134677L,
 0.00002480157946764225L,
 -0.00138888888730525966L,
 0.04166666666651986722L,
 -0.49999999999999547304L,
 };
# 80 "/arm-libs/library-src/dinkum/source/./xxxsin.h"
static const long double s[] = {
 0.00000000015893606014L,
 -0.00000002505069049138L,
 0.00000275573131527032L,
 -0.00019841269827816117L,
 0.00833333333331908278L,
 -0.16666666666666612594L,
 };
# 124 "/arm-libs/library-src/dinkum/source/./xxxsin.h"
long double _LSinx(long double x, unsigned int qoff, int quads)
 {
 switch (_LDtest(&x))
  {
 case 2:
 ...
 default:
  qoff += _LQuad(&x, quads);
  if (-_LRteps._Long_double < x && x < _LRteps._Long_double)
   {
   if ((qoff & 0x1) != 0)
    x = 1.0L;
   }
   ...
   if ((qoff & 0x1) != 0)
    x = 1.0L + w * (((((c[0] * w + c[1]) * w + c[2]) * w + c[3]) * w + c[4]) * w + c[5]);
   else
...
long double _LSincos(long double x, long double *pcos)
 {
 switch (_LDtest(&x))
  {
 case 2:
 ...
 case 1:
  *pcos = _LNan._Long_double;
  _Feraise(0x01);
  return (_LNan._Long_double);

 default:
   {
  unsigned int qoff = _LQuad(&x, 0);
...
long double _LSin(long double x, unsigned int qoff)
 {
 return (_LSinx(x, qoff, 0));
 }
# 6 "/arm-libs/library-src/dinkum/source/./xlsin.c" 2
