# 1 "/arm-libs/library-src/dinkum/source/./conjf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./conjf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./conjf.c" 2
...
_Fcomplex (conjf)(_Fcomplex x)
 {
 float ans[2];

 ans[0] = crealf(x);
 ans[1] = cimagf(x);
 ((*_FPmsw(&(ans[1]))) ^= ((unsigned short)0x8000));
 return (*(_Fcomplex *)&ans[0]);
 }
# 4 "/arm-libs/library-src/dinkum/source/./conjf.c" 2
