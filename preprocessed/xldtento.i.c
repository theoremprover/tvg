# 1 "/arm-libs/library-src/dinkum/source/./xldtento.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xldtento.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xldtento.c" 2
...
static const long double tenth[] =
 {
 ...
 (long double)(26843545.0L / (67108864.0L * 67108864.0L) / (67108864.0L * 67108864.0L)),
 };
# 70 "/arm-libs/library-src/dinkum/source/./xxxdtent.h"
long double _LDtento(long double *xpx, long n, int *perr)
 {
 long double xpf[4];
 long double x;

 if (n == 0 || xpx[0] == 0.0L)
  return (_LXp_getw(xpx, 4));

 if (0 < n)
  _LXp_setw(xpf, 4, 10.0);
 else
  {
  n = -n;
  _LXp_movx(xpf, 4, tenth);
  _LXp_ldexpx(xpx, 4, (4 * (53 / 2)));
  }
 ...
 for (; ; )
  ...
  if (n == 0)
   break;
  _LXp_movx(xpw, 4, xpf);
  _LXp_mulx(xpf, 4, xpw, 4, xpt);
  }

 if (xpf[0] < 1.0L)
  _LXp_ldexpx(xpx, 4, -(4 * (53 / 2)));
 x = _LXp_getw(xpx, 4);
 if (x == 0.0L || x == _LInf._Long_double || x == -_LInf._Long_double)
  {
  ( _Errno) = 0x0022;
  if (perr != 0)
   *perr |= 1;
...
# 4 "/arm-libs/library-src/dinkum/source/./xldtento.c" 2
