# 1 "/arm-libs/library-src/dinkum/source/./cpowl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cpowl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cpowl.c" 2
...
_Lcomplex (cpowl)(_Lcomplex x, _Lcomplex y)
 ...
 else if (!(creall(x) == 0.0L)
  && !(cimagl(y) == 0.0L))
  return (cexpl(((y) * (logl(creall(x))))));
 else
  return (_LCbuild(powl(creall(x), creall(y)),
   0.0L));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cpowl.c" 2
