# 1 "/arm-libs/library-src/dinkum/source/./gets.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./gets.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./gets.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./gets.c" 2
...
char *(gets)(char *buf)
 ...
 (void)0;
 if (((& _Stdin)->_Mode & 0x4000) != 0)
  for (; (& _Stdin)->_Rback < (& _Stdin)->_Back + sizeof ((& _Stdin)->_Back); )
   {
   *s = *(& _Stdin)->_Rback++;
   if (*s++ == '\n')
    {
 ...
 for (; ; )
  {
  if ((& _Stdin)->_Rsave != 0)
   (& _Stdin)->_Rend = (& _Stdin)->_Rsave, (& _Stdin)->_Rsave = 0;
  if ((& _Stdin)->_Next < (& _Stdin)->_Rend)
   ;
  else if (_Frprep((& _Stdin)) < 0)
   {
   (void)0;
   return (0);
   }
  else if ((& _Stdin)->_Mode & 0x100)
   break;

   {
  unsigned char *s1
   = (unsigned char *)memchr((void *)(& _Stdin)->_Next,
    '\n', (& _Stdin)->_Rend - (& _Stdin)->_Next);
  size_t m = (s1 ? s1 + 1 : (& _Stdin)->_Rend) - (& _Stdin)->_Next;

  memcpy(s, (& _Stdin)->_Next, m);
  s += m; (& _Stdin)->_Next += m;
  if (s1 != 0)
   {
   s[-1] = '\0';
