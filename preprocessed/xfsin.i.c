# 1 "/arm-libs/library-src/dinkum/source/./xfsin.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfsin.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfsin.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xfsin.c" 2
...
unsigned int _FQuad(float *px, int quads);
...
# 70 "/arm-libs/library-src/dinkum/source/./xxxsin.h"
...
float _FSinx(float x, unsigned int qoff, int quads)
 {
 switch (_FDtest(&x))
  {
 case 2:
 ...
 default:
  qoff += _FQuad(&x, quads);
  if (-_FRteps._Float < x && x < _FRteps._Float)
   {
   if ((qoff & 0x1) != 0)
    x = 1.0F;
   }
   ...
   if ((qoff & 0x1) != 0)
    x = 1.0F + w * ((c[0] * w + c[1]) * w + c[2]);
   else
    x += x * w * ((s[0] * w + s[1]) * w + s[2]);
   }
...
float _FSincos(float x, float *pcos)
 {
 switch (_FDtest(&x))
  {
 case 2:
...
float _FSin(float x, unsigned int qoff)
 {
 return (_FSinx(x, qoff, 0));
 }
# 6 "/arm-libs/library-src/dinkum/source/./xfsin.c" 2
