# 1 "/arm-libs/library-src/dinkum/source/./csinhl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./csinhl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./csinhl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcsinh.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxcsinh.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxcsinh.h" 2


_Lcomplex (csinhl)(_Lcomplex x)
 {
 ...
 if (errx == 2)
  if (erry == 0)
   return (_LCbuild(_LNan._Long_double, im));
  else
  ...
  if (erry == 0)
   return (_LCbuild(re, im));
  else
   return (_LCbuild(re * cosl(im),
    _LInf._Long_double * sinl(im)));
 else
  return (_LCbuild(_LSinh(re, cosl(im)),
   _LCosh(re, sinl(im))));
 }
# 4 "/arm-libs/library-src/dinkum/source/./csinhl.c" 2
