# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/freopen_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/freopen_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/freopen_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/freopen_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/freopen_s.c" 2





errno_t (freopen_s)(FILE * * pstr,
 const char * filename,
 const char * mods,
 FILE * str)
 {
 unsigned short mode;
 int exc_code = 0;
# 41 "/arm-libs/library-src/dinkum/source/./c_ext1/freopen_s.c"
 if (pstr == 0)
  return (_Fail_s("fopen_s: bad FILE ** argument", sizeof ("fopen_s: bad FILE ** argument")));
 *pstr = 0;

 if (filename == 0 || mods == 0 || str == 0)
  return (_Fail_s("fopen_s: bad argument", sizeof ("fopen_s: bad argument")));


 if (*mods == 'u' && (*++mods == 'a' || *mods == 'w'))
  exc_code = 'u';

 (void)0;
 mode = (unsigned short)(str->_Mode & 0x80);
 str->_Mode &= ~0x80;
 fclose(str);
 str->_Mode = mode;

 (void)0;
 str = _Foprep(filename, mods, str, (-1), exc_code);
 (void)0;

 (void)0;

 *pstr = str;
 return (str == 0 ? 0x0022 : 0);
 }
