# 1 "/arm-libs/library-src/dinkum/source/./xwputstr.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwputstr.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xwputstr.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 4 "/arm-libs/library-src/dinkum/source/./xwputstr.c" 2
...
int _WPutstr(_WPft *px, const char *pc)
 ...
 for (stat = 0; 0 < m; pc += n, ++px->n0, --m)
  ...
  if ((n = _Mbtowc(&pac[px->n0], pc, 0x7fffffff,
   &mbst)) < 0)
   {
   stat = (-1);
   break;
