# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c" 2
# 1 "/toolchain/arm/include/limits.h" 1 3
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./c_ext1/wcrtomb_s.c" 2


errno_t (wcrtomb_s)(size_t * psize,
 char * s, rsize_t maxsize,
 wchar_t wchar, mbstate_t * pst)
 {
 char buf[8];
 int status;
 ...
 if (psize == 0 || pst == 0
  || (s == 0 && 0 < maxsize)
  || (s != 0 && (maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize)))
  {
  if (s != 0 && 0 < maxsize && maxsize <= ((_Sizet)(-1) >> 1))
   s[0] = '\0';
  if (psize != 0)
   *psize = (size_t)(-1);
  return (_Fail_s("wcrtomb_s: bad argument", sizeof ("wcrtomb_s: bad argument")));
  }

 if (s == 0)
  status = _Wctomb(buf, L'\0', pst);
 else if ( _Getmbcurmax() <= maxsize)
  status = _Wctomb(s, wchar, pst);
 else
  {
  status = _Wctomb(buf, wchar, pst);
  if (status <= 0)
   ;
  else if ((size_t)status <= maxsize)
   memcpy(s, buf, status);
  else
   {
   *psize = (size_t)(-1);
   return (_Fail_s("wcrtomb_s: buffer too small", sizeof ("wcrtomb_s: buffer too small")));
   }
  }
