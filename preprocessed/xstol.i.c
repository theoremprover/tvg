# 1 "/arm-libs/library-src/dinkum/source/./xstol.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xstol.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xstol.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./xstol.c" 2
...
# 8 "/arm-libs/library-src/dinkum/source/./xstol.c" 2
...
long _Stolx(const char * s, char ** endptr,
 ...
 else if ((sign == '+' && 0x7fffffffL < x)
  || (sign == '-' && 0 - (unsigned long)(-0x7fffffffL - 1) < x))
  {
  ( _Errno) = 0x0022;
  if (perr != 0)
   *perr = 1;
  return (sign == '-' ? (-0x7fffffffL - 1) : 0x7fffffffL);
  }
 else
