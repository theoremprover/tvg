# 1 "/arm-libs/library-src/dinkum/source/./malloc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./malloc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./malloc.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./malloc.c" 2
# 54 "/arm-libs/library-src/dinkum/source/./malloc.c"
_Altab _Aldata = {0};
size_t _Size_block = {512};
static _Cell **findmem(size_t size)
 ...
 for (; ; )
  ...
  q->_Size = bs;
  (void)0;
  free((char *)q + (__builtin_offsetof(_Cell, _Next) + ((1 << 3U) - 1) & ~((1 << 3U) - 1)));
   }
  }
 }
...
void *(malloc)(size_t size_arg)
 {
 _Cell *q, **qb;
 size_t size = (size_arg + ((__builtin_offsetof(_Cell, _Next) + ((1 << 3U) - 1) & ~((1 << 3U) - 1)) + ((1 << 3U) - 1))) & ~((1 << 3U) - 1);
 ...
 q = *qb;
 if (q->_Size - (sizeof (_Cell) + ((1 << 3U) - 1) & ~((1 << 3U) - 1)) < size)
  *qb = q->_Next;
 else
  {
 ...
 (void)0;
 (void)0;
 return ((char *)q + (__builtin_offsetof(_Cell, _Next) + ((1 << 3U) - 1) & ~((1 << 3U) - 1)));
 }
