# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/strerror_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/strerror_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/strerror_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/strerror_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/strerror_s.c" 2
...
errno_t (strerror_s)(char *s, rsize_t maxsize,
 ...
 if (s == 0 || maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("strerror_s: bad argument", sizeof ("strerror_s: bad argument")));

 pmes = _Strerror(errcode, 0);
 n = strlen(pmes);
 ...
 if (n < maxsize)
  ...
  {
  memcpy(s, pmes, maxsize - 1);
  s[maxsize - 1] = '\0';
  if (3 < maxsize)
   memcpy(s - 4 + maxsize, "...", 3);
  return (0);
  }
 }
