# 1 "/arm-libs/library-src/dinkum/source/./asinhf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./asinhf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./asinhf.c" 2
...
static const float ln2 = 0.69314718055994530941723212145817658F;
...
float (asinhf)(float x)
 ...
 default:
  ...
  if (x < _FEps._Float)
   y = x;
  else if (x < ((2.0F) / (_FEps._Float)))
   y = log1pf(x
    + x * ((x) / ((1.0F + sqrtf(x * x + 1.0F)))));
  else
   y = logf(x) + ln2;
...
# 4 "/arm-libs/library-src/dinkum/source/./asinhf.c" 2
