# 1 "/arm-libs/library-src/dinkum/source/./xlatan.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlatan.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlatan.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxatan.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxatan.h" 2
# 37 "/arm-libs/library-src/dinkum/source/./xxxatan.h"
static const long double c[] = {
 0.00506666138256980774L,
 0.18786627197308930065L,
 1.05290797687252613439L,
 1.84896092990243571481L,
 };
static const long double s[] = {
 0.06383198950003615800L,
 0.63658766690479336192L,
 1.51562759656910323406L,
 };
# 95 "/arm-libs/library-src/dinkum/source/./xxxatan.h"
static const long double a[8] = {
 0.0L,
 0.52359877559829887307710723054658382L,
 1.57079632679489661923132169163975145L,
 1.04719755119659774615421446109316763L,
 1.57079632679489661923132169163975145L,
 2.09439510239319549230842892218633526L,
 3.14159265358979323846264338327950287L,
 2.61799387799149436538553615273291908L,
 };

static const long double rt3 = 1.7320508075688772935274463415058725L;
static const long double fold = 0.26794919243112270647255365849412763L;
static const long double piby4 = 0.78539816339744830961566084581987572L;

long double _LAtan(long double x, int idx)
 {
 int idx7;

 if (x == 1.0L)
  x = piby4;
# 126 "/arm-libs/library-src/dinkum/source/./xxxatan.h"
 else
  {
  if (fold < x)
   {
  ...
  if (x < -_LRteps._Long_double || _LRteps._Long_double < x)
   {
   const long double z = x * x;
    const long double eps = z * (((c[0] * z + c[1]) * z + c[2]) * z + c[3]);
    const long double denom = (1.0L / (1.0L + eps));

    x += x * z * ((s[0] * z + s[1]) * z + s[2]);
    x -= x * eps * denom;
   }
  }
...
# 4 "/arm-libs/library-src/dinkum/source/./xlatan.c" 2
