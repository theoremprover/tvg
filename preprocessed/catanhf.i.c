# 1 "/arm-libs/library-src/dinkum/source/./catanhf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./catanhf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./catanhf.c" 2
...
static const float big = 4.61e18F;
# 21 "/arm-libs/library-src/dinkum/source/./xxcatanh.h"
static const float piby2 = 1.5707963267948966192313216916397514F;

_Fcomplex (catanhf)(_Fcomplex x)
 {
 float re = crealf(x);
 float im = cimagf(x);
 short errx = _FDtest(&re);
 short erry = _FDtest(&im);
 float u, v;
 ...
 if (errx == 2)
  ...
  if (errx == 1 || errx == 0)
   {
   u = ((*_FPmsw(&(re))) & ((unsigned short)0x8000)) ? -_FZero : 0.0F;
   v = im;
   }
  else
   {
  ...
  {
  u = re < 0.0F ? -_FZero : 0.0F;
  v = ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -piby2 : piby2;
  }
 else
  {
  float magim = ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -im : im;
  int neg;

  if (re < 0.0F)
   {
   neg = 1;
   ((*_FPmsw(&(re))) ^= ((unsigned short)0x8000));
   }
  else
   {
   neg = 0;
   ((*_FPmsw(&(im))) ^= ((unsigned short)0x8000));
   }
  ...
  if (big < re)
   ...
   {
   float refrom1 = 1.0F - re;
   float imeps2 = magim * magim;

   u = 0.25F * log1pf(((4.0F * re) / ((refrom1 * refrom1 + imeps2))));

   v = 0.5F * atan2f(2.0F * im,
    refrom1 * (1.0F + re) - imeps2);
   }
  else if (im == 0.0F)
   {
   _Feraise(0x02);
   ...
   {
   u = logf(((sqrtf(sqrtf(4.0F + im * im))) / (sqrtf(magim))));

   v = 0.5F * (piby2 + atan2f(magim, 2.0F));
   ((*_FPmsw(&(v))) &= ~((unsigned short)0x8000));
   }

  if (neg)
   ((*_FPmsw(&(u))) ^= ((unsigned short)0x8000));
  else
   ((*_FPmsw(&(v))) ^= ((unsigned short)0x8000));
  }
 return (_FCbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./catanhf.c" 2
