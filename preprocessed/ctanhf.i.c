# 1 "/arm-libs/library-src/dinkum/source/./ctanhf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./ctanhf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./ctanhf.c" 2
...
_Fcomplex (ctanhf)(_Fcomplex x)
 ...
 if (errx == 1)
  return (_FCbuild(re < 0.0F ? -1.0F : 1.0F,
   ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -_FZero : 0.0F));
 else if (erry == 2)
  ...
  return (_FCbuild(_FNan._Float, _FNan._Float));
 else
  {
  const float tv = tanf(im);
  const float sv = sinhf(re);
  const float bv = sv * (1.0F + tv * tv);
  const float dv = 1.0F + bv * sv;
  ...
  if (dv == _FInf._Float)
   ...
   return (_FCbuild(
    ((sqrtf(1.0F + sv * sv) * bv) / (dv)),
    ((tv) / (dv))));
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./ctanhf.c" 2
