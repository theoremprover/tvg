# 1 "/arm-libs/library-src/dinkum/source/./xlexp.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlexp.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlexp.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xlexp.c" 2

# 1 "/arm-libs/library-src/dinkum/source/./xxxexp.h" 1
# 23 "/arm-libs/library-src/dinkum/source/./xxxexp.h"
static const long double c[] = {
 0.00099173235263350450L,
 0.11110779924116564678L,
 };
static const long double s[] = {
 0.00003304120783105597L,
 0.01388723295391837963L,
 0.49999999999999998664L,
 };
# 99 "/arm-libs/library-src/dinkum/source/./xxxexp.h"
static const long double c1 = (long double)(46516319.0L / 67108864.0L);
static const long double c23 = (long double)(0.87218081361711894970L / 67108864.0L);

static const long double hugexp = (int)(((unsigned short)0x7fff) * 900L / 1000);
static const long double invln2 = 1.4426950408889634073599246810018922L;

short _LExp(long double *px, long double y, long eoff)
 {
 short errx = _LDtest(px);
 short erry = _LDtest(&y);
  ...
  *px = _LInf._Long_double * y;
  ...
  long xexp = (long)(g + (g < 0.0L ? - 0.5L : 0.5L));

  g = xexp;
  g = (long double)((*px - g * c1) - g * c23);
  if (-_LEps._Long_double < g && g < _LEps._Long_double)
   *px = y;
  else
   {
   const long double z = g * g;
   const long double ch = z * (c[0] * z + c[1]);
   const long double sh = g * ((s[0] * z + s[1]) * z + s[2]);
   const long double den = (1.0L / (1.0L + (ch - sh)));

   *px = (den + (ch + sh) * den) * y;
   }

  switch (errx = _LDscale(px, (long)xexp + eoff))
   {
  case 0:
   _Feraise(0x08);
   break;
...
# 6 "/arm-libs/library-src/dinkum/source/./xlexp.c" 2
