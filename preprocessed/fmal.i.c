# 1 "/arm-libs/library-src/dinkum/source/./fmal.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fmal.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fmal.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxfma.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxfma.h" 2
...
long double (fmal)(long double x, long double y, long double z)
 ...
 long double ans;
 long double xp = x, yp = y, zp = z;
# 22 "/arm-libs/library-src/dinkum/source/./xxfma.h"
 if ((errx = _LDunscale(&expx, &xp)) == 2)
  ans = x;
 else if ((erry = _LDunscale(&expy, &yp)) == 2)
  ans = y;
 else if ((errz = _LDunscale(&expz, &zp)) == 2)
  ans = z;
 else if (errx == 1 || erry == 1)
  if (errx == 0 || erry == 0
   || (errz == 1 && (((*_LPmsw(&(x))) & ((unsigned short)0x8000)) ^ ((*_LPmsw(&(y))) & ((unsigned short)0x8000))) != ((*_LPmsw(&(z))) & ((unsigned short)0x8000))))
   {
   _Feraise(0x01);
   ans = _LNan._Long_double;
   }
  else if (errz == 1)
   ans = z;
  else if ((((*_LPmsw(&(x))) & ((unsigned short)0x8000)) ^ ((*_LPmsw(&(y))) & ((unsigned short)0x8000))) == 0)
   ans = _LInf._Long_double;
  else
   ans = -_LInf._Long_double;
  ...
  if (2 * 53 < exp_diff)
   ...
   xp = (xp < 0.0L) ^ (yp < 0.0L)
    ? -2.2250738585072014e-308L : 2.2250738585072014e-308L;
   _LXp_setw(xpx, 4, xp);
   }
  else
   {
   if (exp_diff < -2 * 53)
    ...
    _LDscale(&zp, exp_diff);
   _LXp_setw(xpz, 2, zp);

   _LXp_setw(xpx, 4, xp);
   _LXp_setw(xpy, 2, yp);
   _LXp_mulh(xpx, 4, xpy[0]);
   if (xpy[1] != 0.0L)
    {
    long double xpw[4];

    _LXp_setw(xpw, 4, xp);
    _LXp_mulh(xpw, 4, xpy[1]);
    _LXp_addx(xpx, 4, xpw, 4);
    }
   }
  _LXp_addx(xpx, 4, xpz, 2);
  ans = _LXp_getw(xpx, 4);
  if (exp_prod != 0)
   _LDscale(&ans, exp_prod);

  if (_LDtest(&ans) != 1)
   ;
  else if (0.0L < ans)
   ans = 1.7976931348623157e+308L + 1.7976931348623157e+308L;
  else
   ans = -(1.7976931348623157e+308L + 1.7976931348623157e+308L);
  }
 return (ans);
 }
# 4 "/arm-libs/library-src/dinkum/source/./fmal.c" 2
