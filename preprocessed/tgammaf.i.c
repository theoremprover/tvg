# 1 "/arm-libs/library-src/dinkum/source/./tgammaf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./tgammaf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./tgammaf.c" 2
...
float (tgammaf)(float x)
 ...
 case 0:
  if (x == 0.0F)
   {
   _Feraise(0x02);
   return (((*_FPmsw(&(x))) & ((unsigned short)0x8000)) ? -_FInf._Float : _FInf._Float);
   }
  else if (x < 0.0F)
   {
   _Feraise(0x01);
 ...
 default:
  if (x <= -38.0F)
   ...
   if (x - z < -1.0F)
    y = -y;
   z = ((pi) / ((-x * _FSinx(pi * y, 0, 0) * (-x - 1.0F))));
   z = ((z) / ((1.0F + _FTgamma(&w, &xexp))));
   z *= w;

   switch (_FDscale(&z, xexp))
    {
   case 0:
    _Feraise(0x08);
    break;
   ...
   {
   short xexp;
   float y = _FTgamma(&x, &xexp);

   x = ((1.0F + y) / (x));
   switch (_FDscale(&x, -xexp))
    {
   case 0:
    _Feraise(0x08);
    break;
...
# 4 "/arm-libs/library-src/dinkum/source/./tgammaf.c" 2
