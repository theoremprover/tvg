# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vprintf_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vprintf_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/vprintf_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/vprintf_s.c" 2
...
int (vprintf_s)(const char * fmt, va_list ap)
 {
 int ans;

 if (fmt == 0)
  return (_Fail_s("vprintf_s: bad argument", sizeof ("vprintf_s: bad argument")), (-1));

 (void)0;
 ans = _Printf(&prout, (& _Stdout), fmt, ap, 1);
 (void)0;
 return (ans);
 }
