# 1 "/arm-libs/library-src/dinkum/source/./csinh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./csinh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./csinh.c" 2
...
_Dcomplex (csinh)(_Dcomplex x)
 ...
 if (errx == 2)
  ...
  if (erry == 0)
   ...
   return (_Cbuild(re * cos(im),
    _Inf._Double * sin(im)));
 else
  return (_Cbuild(_Sinh(re, cos(im)),
   _Cosh(re, sin(im))));
 }
# 4 "/arm-libs/library-src/dinkum/source/./csinh.c" 2
