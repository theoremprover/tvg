# 1 "/arm-libs/library-src/dinkum/source/./xstof.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xstof.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xstof.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xstof.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xstof.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcctype.h" 1
# 6 "/arm-libs/library-src/dinkum/source/./xstof.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./xstof.c" 2


float _Stofx(const char *s, char **endptr, long pten,
 int *perr)
...
# 22 "/arm-libs/library-src/dinkum/source/./xxstod.h"
 ...
 extern float _FDtento(float *, long, int *);

 if (perr != 0)
  *perr = 0;
 if ((code &= ~8) == 1)
  {
  const int nlo = _Stoflt(s0, s, endptr, lo, 1);
  float xpx[4], xpf[4];
  int i;

  _FXp_setw(xpf, 4, 1e9F);
  if (nlo == 0)
   _FXp_setw(xpx, 4, 0.0);
  else
   for (i = 1, _FXp_setn(xpx, 4, lo[1]); i < nlo; )
    {
    float xpa[4];
    float xpt[4 * 2];

    _FXp_mulx(xpx, 4, xpf, 4, xpt);
    _FXp_setn(xpa, 4, lo[++i]);
    _FXp_addx(xpx, 4, xpa, 4);
    }
  pten += lo[0];
  x = _FDtento(xpx, pten, perr);
  }
 else if (code == 2)
  {
  const int nlo = _Stoxflt(s0, s, endptr, lo, 1);
  float xpx[4], xpf[4];
  int i;

  _FXp_setw(xpf, 4, 268435456.0F);
  if (nlo == 0)
   _FXp_setw(xpx, 4, 0.0);
  else
   for (i = 1, _FXp_setn(xpx, 4, lo[1]); i < nlo; )
...
# 12 "/arm-libs/library-src/dinkum/source/./xstof.c" 2

float _Stof(const char *s, char **endptr, long pten)
 {
