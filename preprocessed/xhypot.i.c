# 1 "/arm-libs/library-src/dinkum/source/./xhypot.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xhypot.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xhypot.c" 2
...
double _Hypot(double x, double y, int *pexp)
 {
 short xexp, yexp;
 short errx = _Dunscale(&xexp, &x);
 short erry = _Dunscale(&yexp, &y);
 ...
 *pexp = 0;
 if (errx == 1 || erry == 1)
  ...
  else if (x == 0.0 || xexp < yexp || (xexp == yexp && x < y))
   {
   short texp = xexp;
   double tmp = x;
  ...
  if (z == x)
   ...
   {
   const double qv = ((x) / (y));
   z = x + ((y) / (qv + sqrt(qv * qv + 1.0)));
   }
  else
   {
   static const double root2 =
    1.4142135623730950488016887242096982;
   static const double oneplusroot2high = 2.4142;
   static const double oneplusroot2low =
    0.0000135623730950488016887242096980785698;
   const double qv = ((z) / (y));
   const double rv = (qv + 2.0) * qv;
   const double sv = ((rv) / (root2 + sqrt(rv + 2.0)))
...
# 4 "/arm-libs/library-src/dinkum/source/./xhypot.c" 2
