# 1 "/arm-libs/library-src/dinkum/source/./cpow.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cpow.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cpow.c" 2
...
_Dcomplex (cpow)(_Dcomplex x, _Dcomplex y)
 ...
 if (!(cimag(x) == 0.0))
  ...
  && !(cimag(y) == 0.0))
  return (cexp(((y) * (log(creal(x))))));
 else
  return (_Cbuild(pow(creal(x), creal(y)),
   0.0));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cpow.c" 2
