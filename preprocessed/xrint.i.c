# 1 "/arm-libs/library-src/dinkum/source/./xrint.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xrint.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xrint.c" 2
...
double _Rint(double xarg)
 ...
 case 0:
  ...
  case 2:
  if (_Dint(&x, 0) != 0 && !((*_Pmsw(&(x))) & ((unsigned short)0x8000)))
   x += 1.0;
  break;
 ...
 default:
  if (_Dint(&x, 1) == 0)
   ...
   if (_Dint(&x, 0) == 0
    || _Dint(&x, -1) == 0)
    ;
   else if (((*_Pmsw(&(x))) & ((unsigned short)0x8000)))
    x -= 2.0;
   else
    x += 2.0;
   }
...
# 4 "/arm-libs/library-src/dinkum/source/./xrint.c" 2
