# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/strcpy_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/strcpy_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/strcpy_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/strcpy_s.c" 2


errno_t (strcpy_s)(char * s1, rsize_t maxsize,
 const char * s2)
 {
 size_t n2;

 if (s1 == 0 || maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("strcpy_s: bad destination argument", sizeof ("strcpy_s: bad destination argument")));
 else if (s2 == 0
  || maxsize <= (n2 = strlen(s2))
  || (s1 < s2 + n2 && s2 < s1 + maxsize))
  {
  s1[0] = '\0';
  return (_Fail_s("strcpy_s: bad argument", sizeof ("strcpy_s: bad argument")));
  }
