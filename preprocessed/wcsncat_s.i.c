# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsncat_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsncat_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsncat_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/wcsncat_s.c" 2


errno_t (wcsncat_s)(wchar_t * s1, rsize_t maxsize,
 const wchar_t * s2, rsize_t n)
 {
 size_t n1, n2;

 if (s1 == 0 || maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("wcsncat_s: bad destination argument", sizeof ("wcsncat_s: bad destination argument")));
 else if (s2 == 0 || ((_Sizet)(-1) >> 1) < n
  || maxsize <= (n1 = wcsnlen_s(s1, maxsize)) + (n2 = wcsnlen_s(s2, n))
  || (s1 < s2 + n2 && s2 < s1 + n1))
  {
  s1[0] = L'\0';
  return (_Fail_s("wcsncat_s: bad argument", sizeof ("wcsncat_s: bad argument")));
  }
