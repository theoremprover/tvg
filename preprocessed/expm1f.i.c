# 1 "/arm-libs/library-src/dinkum/source/./expm1f.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./expm1f.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./expm1f.c" 2
...
# 12 "/arm-libs/library-src/dinkum/source/./xxexpm1.h"
...
static const float ln3_2 = 0.40546510810816438197801311546434914F;
static const float ln2 = 0.69314718055994530941723212145817657F;
static const float ln3 = 1.09861228866810969139524523692252573F;
static const float ln4 = 1.38629436111989061883446424291635315F;
static const float ln5 = 1.60943791243410037460075933322618765F;

static float _FExpm1_approx(float x)
 {
 return (((((s[0] * x + s[1]) * x + s[2]) * x) / ((((c[0] * x * x + c[2]) * x * x + c[4]) + (c[1] * x * x + c[3]) * x))));
 }
...
float (expm1f)(float x)
 ...
 case 1:
  return (((*_FPmsw(&(x))) & ((unsigned short)0x8000)) ? -1.0F : x);

 default:
  if (-_FEps._Float <= x && x <= _FEps._Float)
   return (x);
  else if (x < -ln2)
   return (expf(x) - 1.0F);
  else if (x < ln3_2)
   return (_FExpm1_approx(x));
  else if (x < ln3)
   return (2.0F * _FExpm1_approx(x - ln2) + 1.0F);
  else if (x < ln5)
   return (4.0F * _FExpm1_approx(x - ln4) + 3.0F);
  else
   return (expf(x) - 1.0F);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./expm1f.c" 2
