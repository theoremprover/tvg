# 1 "/arm-libs/library-src/dinkum/source/./xlprec.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlprec.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlprec.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xlprec.c" 2
...
long double _LXp_getw(const long double *p, int n)
 {
 if (n == 0)
  return (0.0L);
 else if (n == 1 || p[0] == 0.0L || p[1] == 0.0L)
  return (p[0]);
 else if (n == 2 || p[2] == 0.0L)
  return (p[0] + p[1]);
 else
  {
  long double p01 = p[0] + p[1];
  long double p2 = p[2];
...
long double *_LXp_addh(long double *p, int n, long double x0)
 ...
 if (n == 0)
  ...
  if (errx == 2 || (errx = _LDtest(&p[0])) <= 0)
   p[0] = x0;
  else if (errx == 2 || ((*_LPmsw(&(x0))) & ((unsigned short)0x8000)) == ((*_LPmsw(&(p[0]))) & ((unsigned short)0x8000)))
   ;
  else
   {
   _Feraise(0x01);
...
long double *_LXp_setn(long double *p, int n, long x)
 {

 _LXp_setw(p, n, (long double)x);
...
long double *_LXp_movx(long double *p, int n, const long double *q)
 {
 memcpy(p, q, n * sizeof (long double));
 return (p);
 }

long double *_LXp_addx(long double *p, int n,
 const long double *q, int m)
 {
 int k;
...
long double *_LXp_subx(long double *p, int n,
 const long double *q, int m)
 {
 int k;
...
long double *_LXp_mulx(long double *p, int n,
 const long double *q, int m,
 long double *ptemp2)
 {
 if (n == 0 || m == 0)
...
# 6 "/arm-libs/library-src/dinkum/source/./xlprec.c" 2
