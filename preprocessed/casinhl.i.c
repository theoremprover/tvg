# 1 "/arm-libs/library-src/dinkum/source/./casinhl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./casinhl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./casinhl.c" 2
...
_Lcomplex (casinhl)(_Lcomplex x)
 ...
 if (errx == 2)
  ...
  if (erry == 1)
   {
   u = re;
   v = im < 0.0L ? -piby4 : piby4;
   }
  else
   {
   u = re;
   v = ((*_LPmsw(&(im))) & ((unsigned short)0x8000)) ? -_LZero : 0.0L;
   }
 else if (erry == 1)
  {
  u = im;
...
# 4 "/arm-libs/library-src/dinkum/source/./casinhl.c" 2
