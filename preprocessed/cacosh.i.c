# 1 "/arm-libs/library-src/dinkum/source/./cacosh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cacosh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cacosh.c" 2
...
static const double pi3by4 = 2.3561944901923449288469825374596271;
...
_Dcomplex (cacosh)(_Dcomplex x)
 {
 double re = creal(x);
 double im = cimag(x);
 const short errx = _Dtest(&re);
 const short erry = _Dtest(&im);
 double u, v;
 ...
 if (errx == 2)
  ...
  if (erry == 1)
   ...
   if (re < 0.0)
    ...
    v = 0.0;
  if (((*_Pmsw(&(im))) & ((unsigned short)0x8000)))
   v = -v;
  }
 else if (erry == 1)
  {
  ...
  {
  const _Dcomplex w = csqrt(_Cbuild(re - 1.0, -im));
  const _Dcomplex z = csqrt(_Cbuild(re + 1.0, im));
  const double wr = creal(w);
  const double wi = cimag(w);
  const double zr = creal(z);
  const double zi = cimag(z);
  int neg;
  double alfa, beta;
  ...
  v = 2.0 * atan2(
   cimag(csqrt(_Cbuild(re - 1.0, im))), zr);
  if (neg)
   ((*_Pmsw(&(v))) ^= ((unsigned short)0x8000));
  }
 return (_Cbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cacosh.c" 2
