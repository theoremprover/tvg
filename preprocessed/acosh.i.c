# 1 "/arm-libs/library-src/dinkum/source/./acosh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./acosh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./acosh.c" 2
...
# 38 "/arm-libs/library-src/dinkum/source/./xxacosh.h"
  else if (x < ((2.0) / (_Eps._Double)))
   {
   double z = x - 1.0;

   return (log1p(z + sqrt(z * x + z)));
   }
  else
   return (log(x) + ln2);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./acosh.c" 2
