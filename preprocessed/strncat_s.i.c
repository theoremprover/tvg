# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/strncat_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/strncat_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/strncat_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/strncat_s.c" 2


errno_t (strncat_s)(char * s1, rsize_t maxsize,
 const char * s2, rsize_t n)
 {
 size_t n1, n2;

 if (s1 == 0 || maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("strncat_s: bad destination argument", sizeof ("strncat_s: bad destination argument")));
 else if (s2 == 0 || ((_Sizet)(-1) >> 1) < n
  || maxsize <= (n1 = strnlen_s(s1, maxsize)) + (n2 = strnlen_s(s2, n))
  || (s1 < s2 + n2 && s2 < s1 + n1))
  {
  s1[0] = '\0';
  return (_Fail_s("strncat_s: bad argument", sizeof ("strncat_s: bad argument")));
  }
