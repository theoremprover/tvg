# 1 "/arm-libs/library-src/dinkum/source/./fmin.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fmin.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fmin.c" 2
...
double (fmin)(double x, double y)
 ...
 if (_Dtest(&x) == 2)
  ...
  return (x);
 else if (x < y || (x == y && ((*_Pmsw(&(x))) & ((unsigned short)0x8000))))
  return (x);
 else
  return (y);
 }
# 4 "/arm-libs/library-src/dinkum/source/./fmin.c" 2
