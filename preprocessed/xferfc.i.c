# 1 "/arm-libs/library-src/dinkum/source/./xferfc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xferfc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xferfc.c" 2
...
const float _FErf_small = 1.523F;

float (_FErfc)(float x)
 {
 float z = -x * x;

 _FExp(&z, (((((s[0] * x + s[1]) * x + s[2]) * x + s[3])) / (((c[0] * x + c[1]) * x + c[2]))), 0);
 return (z);
 }
# 4 "/arm-libs/library-src/dinkum/source/./xferfc.c" 2
