# 1 "/arm-libs/library-src/dinkum/source/./casinhf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./casinhf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./casinhf.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcasinh.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxcasinh.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxcasinh.h" 2
...
# 21 "/arm-libs/library-src/dinkum/source/./xxcasinh.h"
...
_Fcomplex (casinhf)(_Fcomplex x)
 ...
 if (errx == 2)
  ...
  if (erry == 1)
   {
   u = re;
   v = im < 0.0F ? -piby4 : piby4;
   }
  else
   {
   u = re;
   v = ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -_FZero : 0.0F;
   }
 else if (erry == 1)
  {
  u = im;
  ...
  {
  const _Fcomplex w = csqrtf(_FCbuild(1.0F - im, re));
  const _Fcomplex z = csqrtf(_FCbuild(1.0F + im, -re));
  ...
  v = atan2f(im, crealf(((w) * (z))));
  }
 return (_FCbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./casinhf.c" 2
