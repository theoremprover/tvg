# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./c_ext1/wctomb_s.c" 2


static mbstate_t mbst = {0}; static int (*_Tls_setup_mbst)(void) = 0;
...
errno_t (wctomb_s)(int * pstat,
 ...
 if (pstat == 0
  || (s == 0 && 0 < maxsize)
  || (s != 0 && ((_Sizet)(-1) >> 1) < maxsize))
  return (_Fail_s("wctomb_s: bad argument", sizeof ("wctomb_s: bad argument")));

 if (s == 0 || _Getmbcurmax() <= maxsize)
  status = _Wctomb(s, wchar, ((_Tls_setup_mbst && _Tls_setup_mbst()), (&(mbst))));
 else
  {
  char buf[8];

  status = _Wctomb(buf, wchar, ((_Tls_setup_mbst && _Tls_setup_mbst()), (&(mbst))));
  if (status <= 0)
   ;
  else if (maxsize < (size_t)status)
   return (_Fail_s("wctomb_s: buffer too small", sizeof ("wctomb_s: buffer too small")));
  else
   memcpy(s, buf, status);
  }
