# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/gets_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/gets_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/gets_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/gets_s.c" 2
...
char *(gets_s)(char *buf, rsize_t n)
 ...
 if (n == 0)
  return (_Fail_s("gets_s: zero buffer size", sizeof ("gets_s: zero buffer size")), (char *)0);

 if (buf == 0 || ((_Sizet)(-1) >> 1) < n)
  {
  if (buf != 0)
   *buf = '\0';
  return (_Fail_s("gets_s: bad argument", sizeof ("gets_s: bad argument")), (char *)0);
  }
 --n;

 for (; (ch = (_Files[0]->_Next < _Files[0]->_Rend ? *_Files[0]->_Next++ : (fgetc)(_Files[0]))) != (-1) && ch != '\n' && 0 < n; --n)
  *s++ = (char)ch;

 if (ch == '\n' || (ch == (-1) && s != (unsigned char *)buf))
  {
  *s = '\0';
  return (buf);
  }
  ...
  {
  if (ch != (-1))
   _Fail_s("gets_s: short buffer", sizeof ("gets_s: short buffer"));
  for (; ch != (-1) && ch != '\n'; )
   ch = (_Files[0]->_Next < _Files[0]->_Rend ? *_Files[0]->_Next++ : (fgetc)(_Files[0]));
  *buf = '\0';
  return ((char *)0);
  }
 }
