# 1 "/arm-libs/library-src/dinkum/source/./xlrint.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlrint.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlrint.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxrint.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxrint.h" 2


long double _LRint(long double xarg)
 {
 long double x = xarg;
 ...
 case 0:
  ...
  case 2:
  if (_LDint(&x, 0) != 0 && !((*_LPmsw(&(x))) & ((unsigned short)0x8000)))
   x += 1.0L;
  break;
 ...
 default:
  if (_LDint(&x, 1) == 0)
   ...
   if (_LDint(&x, 0) == 0
    || _LDint(&x, -1) == 0)
    ;
   else if (((*_LPmsw(&(x))) & ((unsigned short)0x8000)))
    x -= 2.0L;
   else
    x += 2.0L;
   }
  else if (_LDint(&x, 0) == 0)
   ;
  else if (((*_LPmsw(&(x))) & ((unsigned short)0x8000)))
   x -= 1.0L;
  else
...
# 4 "/arm-libs/library-src/dinkum/source/./xlrint.c" 2
