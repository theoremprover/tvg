# 1 "/arm-libs/library-src/dinkum/source/./xscanf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xscanf.c" 2
...
int isblank(int);
# 3 "/arm-libs/library-src/dinkum/source/./xscanf.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xscanf.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xscanf.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./xscanf.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./xscanf.c" 2
...
int _Scanf(int (*pfn)(void *, int, int), void *arg,
 ...
 for (x.s = fmt; ; ++x.s)
  ...
  for (; ; )
   ...
   if ((n = _Mbtowc(&wc, x.s, 0x7fffffff, &mbst)) <= 0)
    n = *x.s == '\0' ? 0 : 1;
   if ((_Ctype[(int)(_Wctob(wc))] & (0x40|0x04|0x100)))
    {
    while ((_Ctype[(int)(ch = (++(&x)->nchar, (*(&x)->pfn)((&x)->arg, 0, 1)))] & (0x40|0x04|0x100)))
     ;
    do if ((int)(ch) != (-1)) (--(&x)->nchar, (*(&x)->pfn)((&x)->arg, ch, 0)); else --(&x)->nchar; while (0);
    }
   else
    {
    int m = wc == L'%' ? n - 1 : n;
    ...
    for (s = x.s; 0 <= --m; ++s)
     if ((ch = (++(&x)->nchar, (*(&x)->pfn)((&x)->arg, 0, 1))) != *s)
      {
      do if ((int)(ch) != (-1)) (--(&x)->nchar, (*(&x)->pfn)((&x)->arg, ch, 0)); else --(&x)->nchar; while (0);
  ...
  for (x.width = 0; (_Ctype[(int)((unsigned char)*x.s)] & 0x20); ++x.s)
  ...
  if (*x.s != '.')
   ...
   {
   x.prec = __builtin_va_arg(ap, size_t);
   ++x.s;
   }
  else
   {
   for (x.prec = 0; (_Ctype[(int)((unsigned char)*x.s)] & 0x20); ++x.s)
    if (x.prec <= (size_t)(-1) / 10 - ((unsigned char)*x.s - '0'))
