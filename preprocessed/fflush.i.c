# 1 "/arm-libs/library-src/dinkum/source/./fflush.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fflush.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fflush.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./fflush.c" 2
...
int (fflush)(FILE *str)
 ...
 if (str == 0)
  ...
  (void)0;
  for (stat = 0, nf = 0; nf < 20; ++nf)
   if (_Files[nf] != 0 && fflush(_Files[nf]) < 0)
    stat = (-1);
  (void)0;
  return (stat);
  }
 ...
 for (s = str->_Buf; s < str->_Next; s += n)
  {
  n = (int)write(((str)->_Handle), s, str->_Next - s);
  if (n <= 0)
   {
   str->_Next = str->_Buf;
