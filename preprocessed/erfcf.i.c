# 1 "/arm-libs/library-src/dinkum/source/./erfcf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./erfcf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./erfcf.c" 2
...
# 21 "/arm-libs/library-src/dinkum/source/./xxerfc.h"
...
float (erfcf)(float x)
 ...
 case 1:
  return (((*_FPmsw(&(x))) & ((unsigned short)0x8000)) ? 2.0F : 0.0F);
  ...
  if (x < erfc_half)
   ...
   for (i = 0; i < sizeof (approx0) / sizeof (approx0[0]); ++i)
    if (x < approx0[i].to)
     ...
     y = (1.0F / (x)) - approx0[i].mid;
     w = ((_FPoly(y, approx0[i].num, approx0[i].nsize - 1)) / (_FPoly(y, approx0[i].den, approx0[i].dsize - 1)));
...
# 4 "/arm-libs/library-src/dinkum/source/./erfcf.c" 2
