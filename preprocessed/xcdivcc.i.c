# 1 "/arm-libs/library-src/dinkum/source/./xcdivcc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xcdivcc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xcdivcc.c" 2
...
_Dcomplex (_Cdivcc)(_Dcomplex x, _Dcomplex y)
 {
 double xre = creal(x);
 double xim = cimag(x);
 double yre = creal(y);
 ...
 ((*_Pmsw(&(zre))) &= ~((unsigned short)0x8000));
 ((*_Pmsw(&(zim))) &= ~((unsigned short)0x8000));
 if (zim < zre)
  {
  double wr = ((yim) / (yre));
  double wd = yre + wr * yim;
  ...
  {
  double wr = ((yre) / (yim));
  double wd = yim + wr * yre;

  zre = ((xre * wr + xim) / (wd));
  zim = ((xim * wr - xre) / (wd));
  }
 return (_Cbuild(zre, zim));
 }
# 4 "/arm-libs/library-src/dinkum/source/./xcdivcc.c" 2
