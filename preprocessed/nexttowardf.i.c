# 1 "/arm-libs/library-src/dinkum/source/./nexttowardf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./nexttowardf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./nexttowardf.c" 2


float (nexttowardf)(float x, long double y)
 {
 _Fval *px = (_Fval *)(char *)&x;
 ...
 if (errx == 2)
  ...
  {
  px->_Sh[1] = (px->_Sh[1] & ((unsigned short)0x8000)) | (((unsigned short)((1 << (15 - 7)) - 1)) << 7) - 1;
  px->_Sh[0] = 0xffff;
  }
 else if (errx == 0)
  {
  px->_Sh[1] = x < y ? 0 : ((unsigned short)0x8000);
  px->_Sh[0] = 1;
  }
 else if ((x < y && 0.0F < x) || (y < x && x < 0.0F))
  {
  if ((++px->_Sh[0] & 0xffff) == 0
   && (++px->_Sh[1] & ~((unsigned short)0x8000)) == ((unsigned short)((1 << (15 - 7)) - 1)) << 7)
   _Feraise(0x04);
  }
 else if ((--px->_Sh[0] & 0xffff) == 0xffff)
  --px->_Sh[1];

 if ((px->_Sh[1] & ((unsigned short)(0x7fff & ~((unsigned short)((1 << 7) - 1))))) == 0)
  _Feraise(0x08);
 return (x);
 }
