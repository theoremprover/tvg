# 1 "/arm-libs/library-src/dinkum/source/./tgammal.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./tgammal.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./tgammal.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxtgamma.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxtgamma.h" 2


long double _LTgamma(long double *, short *);
extern const long double _LGamma_big;
# 24 "/arm-libs/library-src/dinkum/source/./xxtgamma.h"
...
long double (tgammal)(long double x)
 ...
 case 0:
  if (x == 0.0L)
   {
   _Feraise(0x02);
   return (((*_LPmsw(&(x))) & ((unsigned short)0x8000)) ? -_LInf._Long_double : _LInf._Long_double);
   }
  else if (x < 0.0L)
   {
   _Feraise(0x01);
 ...
 default:
  if (x <= -174.0L)
  ...
  else if (x < -_LGamma_big)
   {
   long double z = x;
   long double w = -x - 1.0L;
   short xexp;
   ...
   if (x - z < -1.0L)
    y = -y;
   z = ((pi) / ((-x * _LSinx(pi * y, 0, 0) * (-x - 1.0L))));
   z = ((z) / ((1.0L + _LTgamma(&w, &xexp))));
   z *= w;

   switch (_LDscale(&z, xexp))
    {
   case 0:
    _Feraise(0x08);
    break;
   ...
   {
   short xexp;
   long double y = _LTgamma(&x, &xexp);

   x = ((1.0L + y) / (x));
   switch (_LDscale(&x, -xexp))
    {
   case 0:
    _Feraise(0x08);
    break;
...
# 4 "/arm-libs/library-src/dinkum/source/./tgammal.c" 2
