# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpnam_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpnam_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpnam_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpnam_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/tmpnam_s.c" 2
...
errno_t (tmpnam_s)(char *s, rsize_t maxsize)
 ...
 if (s == 0 || ((_Sizet)(-1) >> 1) < maxsize)
  return (_Fail_s("tmpnam_s: bad argument", sizeof ("tmpnam_s: bad argument")));
 else
  {
  (void)0;
  for (i = 0; i < 32; ++i)
   {
   FILE *str = fopen((const char *)tmpnam(fn), "r");
  ...
  if (i < 32)
   return (strncpy_s(s, maxsize, fn, sizeof (fn)));
  else
   {
   if (0 < maxsize)
    s[0] = '\0';
   return (_Fail_s("tmpnam_s: can't create file", sizeof ("tmpnam_s: can't create file")));
   }
  }
 }
