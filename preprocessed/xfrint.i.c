# 1 "/arm-libs/library-src/dinkum/source/./xfrint.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfrint.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfrint.c" 2
...
float _FRint(float xarg)
 ...
 case 0:
  ...
  case 2:
  if (_FDint(&x, 0) != 0 && !((*_FPmsw(&(x))) & ((unsigned short)0x8000)))
   x += 1.0F;
  break;
 ...
 default:
  if (_FDint(&x, 1) == 0)
   ...
   if (_FDint(&x, 0) == 0
    || _FDint(&x, -1) == 0)
    ;
   else if (((*_FPmsw(&(x))) & ((unsigned short)0x8000)))
    x -= 2.0F;
   else
    x += 2.0F;
   }
  else if (_FDint(&x, 0) == 0)
   ;
  else if (((*_FPmsw(&(x))) & ((unsigned short)0x8000)))
   x -= 1.0F;
  else
...
# 4 "/arm-libs/library-src/dinkum/source/./xfrint.c" 2
