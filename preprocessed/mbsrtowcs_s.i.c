# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/mbsrtowcs_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/mbsrtowcs_s.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/mbsrtowcs_s.c" 2
# 1 "/toolchain/arm/include/limits.h" 1 3
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/mbsrtowcs_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/mbsrtowcs_s.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./c_ext1/mbsrtowcs_s.c" 2


errno_t (mbsrtowcs_s)(size_t * psize,
 wchar_t * wcs, rsize_t maxsize,
 const char ** ps, rsize_t n,
 mbstate_t * pst)
 {
 wchar_t *pwc;
 const char *s;
 ...
 if (psize == 0 || ps == 0 || *ps == 0 || pst == 0
  || (wcs == 0 && 0 < maxsize)
  || (wcs != 0
   && (maxsize == 0 || ((_Sizet)(-1) >> 1) < maxsize || ((_Sizet)(-1) >> 1) < n)))
  {
  if (wcs != 0 && 0 < maxsize && maxsize <= ((_Sizet)(-1) >> 1))
   wcs[0] = L'\0';
  if (psize != 0)
   *psize = (size_t)(-1);
  return (_Fail_s("mbsrtowcs_s: bad argument", sizeof ("mbsrtowcs_s: bad argument")));
  }
 ...
 if (wcs == 0)
  for (; ; ++nwc, s += i)
   ...
   if ((i = _Mbtowc(&wc, s, 0x7fffffff, pst)) < 0)
    {
    *psize = (size_t)-1;
    return (0x0058);
    }
 ...
 for (pwc = wcs; 0 < n && 0 < maxsize;
  ++nwc, s += i, ++pwc, --n, --maxsize)
  {
  if ((i = _Mbtowc(pwc, s, 0x7fffffff, pst)) < 0)
   {
   *ps = s;
   pwc[0] = L'\0';
   *psize = (size_t)-1;
 ...
 *ps = s;
  ...
  *psize = nwc;
  ...
  wcs[0] = L'\0';
  *psize = (size_t)-1;
  return (_Fail_s("mbsrtowcs_s: buffer too small", sizeof ("mbsrtowcs_s: buffer too small")));
  }
 }
