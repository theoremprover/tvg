# 1 "/arm-libs/library-src/dinkum/source/./xwstoflt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwstoflt.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xwstoflt.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xwstoflt.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xwstoflt.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./xwstoflt.c" 2
...
int _WStoflt(const wchar_t *s0, const wchar_t *s,
 ...
 for (seen = 0; *s == L'0'; ++s, seen = 1)
  ;
 for (nsig = 0; _Iswctype(*s, 4); ++s, seen = 1)
  if (nsig <= maxsig)
   buf[nsig++] = (char)(*s - L'0');
  else
   ++lo[0];

 if (*s == _Btowc(localeconv()->decimal_point[0]))
  ++s;
 if (nsig == 0)
  for (; *s == L'0'; ++s, seen = 1)
   --lo[0];
 for (; _Iswctype(*s, 4); ++s, seen = 1)
  if (nsig <= maxsig)
   {
   buf[nsig++] = (char)(*s - L'0');
   --lo[0];
 ...
 if (seen)
  {
  int bufidx = 0;
  int wordidx = 9 - nsig % 9;

  word = wordidx % 9 == 0 ? 0 : 1;
  for (; bufidx < nsig; ++wordidx, ++bufidx)
   if (wordidx % 9 == 0)
    lo[++word] = buf[bufidx];
   else
    lo[word] = lo[word] * 10 + buf[bufidx];
