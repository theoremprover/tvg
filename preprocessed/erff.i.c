# 1 "/arm-libs/library-src/dinkum/source/./erff.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./erff.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./erff.c" 2
...
float (_FErfc)(float x);
extern const float _FErf_one;
extern const float _FErf_small;
...
static const float twobyrootpi = 1.1283791670955125738961589031215451F;
...
float (erff)(float x)
 ...
 default:
  ...
  if (y < _FErf_small)
   ...
   return ((((((s[0] * y + s[1]) * y + s[2]) * y + s[3])) / ((((c[0] * y + c[1]) * y + c[2]) * y + c[3]))) * x);
   }
  else if (y < _FErf_one)
   y = 1.0F - _FErfc(y);
  else
...
# 4 "/arm-libs/library-src/dinkum/source/./erff.c" 2
