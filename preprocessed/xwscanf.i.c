# 1 "/arm-libs/library-src/dinkum/source/./xwscanf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwscanf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xwscanf.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 4 "/arm-libs/library-src/dinkum/source/./xwscanf.c" 2
...
int _WScanf(wint_t (*pfn)(void *, wint_t, int), void *arg,
 ...
 for (x.s = fmt; ; ++x.s)
  ...
  for (; *x.s != L'%'; ++x.s)
   ...
   else if (_Iswctype(*x.s, 9))
    {
    while (_Iswctype(ch = (++(&x)->nchar, (*(&x)->pfn)((&x)->arg, 0, 1)), 9))
     ;
    do if ((int)(ch) != (( wint_t)(-1))) (--(&x)->nchar, (*(&x)->pfn)((&x)->arg, ch, 0)); else --(&x)->nchar; while (0);
    }
   else if (*x.s != (ch = (++(&x)->nchar, (*(&x)->pfn)((&x)->arg, 0, 1))))
    {
    do if ((int)(ch) != (( wint_t)(-1))) (--(&x)->nchar, (*(&x)->pfn)((&x)->arg, ch, 0)); else --(&x)->nchar; while (0);
    return (nconv < 0 ? 0 : nconv);
    }
  ...
  for (x.width = 0; _Iswctype(*x.s, 4); ++x.s)
  ...
  if (*x.s != L'.')
   ...
   {
   for (x.prec = 0; _Iswctype(*x.s, 4); ++x.s)
    if (x.prec <= (size_t)(-1) / 10 - (*x.s - L'0'))
