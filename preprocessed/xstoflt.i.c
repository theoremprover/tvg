# 1 "/arm-libs/library-src/dinkum/source/./xstoflt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xstoflt.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xstoflt.c" 2
...
int isblank(int);
# 4 "/arm-libs/library-src/dinkum/source/./xstoflt.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xstoflt.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./xstoflt.c" 2
...
int _Stoflt(const char *s0, const char *s,
 ...
 for (seen = 0; *s == '0'; ++s, seen = 1)
  ;
 for (nsig = 0; (_Ctype[(int)((unsigned char)*s)] & 0x20); ++s, seen = 1)
  if (nsig < maxsig)
   buf[nsig++] = (char)(*s - '0');
  else
   {
 ...
 if (nsig == 0)
  for (; *s == '0'; ++s, seen = 1)
   --lo[0];
 for (; (_Ctype[(int)((unsigned char)*s)] & 0x20); ++s, seen = 1)
  if (nsig < maxsig)
   {
   buf[nsig++] = (char)(*s - '0');
   --lo[0];
 ...
 if (seen)
  ...
  if (*s == 'e' || *s == 'E')
   ...
   for (; (_Ctype[(int)((unsigned char)*s)] & 0x20); ++s, eseen = 1)
    if (lexp < 100000000)
