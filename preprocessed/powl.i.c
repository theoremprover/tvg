# 1 "/arm-libs/library-src/dinkum/source/./powl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./powl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./powl.c" 2
...
static const long double scale = (long double)(1 << 0);

static const long double lnbias[][4] = {
 {
...
long double (_LPow)(long double x, long double y, short *pex)
 ...
 if ((erry == 0 && y == 0.0L)
  ...
  if (errx == 2)
   ...
   if (!((*_LPmsw(&(x))) & ((unsigned short)0x8000)))
    return (((*_LPmsw(&(y))) & ((unsigned short)0x8000)) ? 0.0L : _LInf._Long_double);
   else if (!((*_LPmsw(&(y))) & ((unsigned short)0x8000)))
   ...
   if (!((*_LPmsw(&(y))) & ((unsigned short)0x8000)))
    return (xexp <= 0 ? 0.0L : _LInf._Long_double);
   else
    return (xexp <= 0 ? _LInf._Long_double : 0.0L);
  else
   if (!((*_LPmsw(&(y))) & ((unsigned short)0x8000)))
    return (erry == 0 && _LDint(&yi, -1) < 0 && ((*_LPmsw(&(x))) & ((unsigned short)0x8000))
     ? -_LZero : 0.0L);
   else
    {
    _Feraise(0x02);
 ...
 else if (((*_LPmsw(&(x))) & ((unsigned short)0x8000)) && erry < 0)
  {
  _Feraise(0x01);
 ...
 x -= 1.0L;
 x1 = z * (w * _LLogpoly(w) - x);

 yi = (long double)xexp + (x + x1) * invln2;
  }
...
# 198 "/arm-libs/library-src/dinkum/source/./xxpow.h"
 ...
 if (neg)
  ((*_LPmsw(&(z))) ^= ((unsigned short)0x8000));
 return (z);
 }
...
# 4 "/arm-libs/library-src/dinkum/source/./powl.c" 2
