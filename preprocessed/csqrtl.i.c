# 1 "/arm-libs/library-src/dinkum/source/./csqrtl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./csqrtl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./csqrtl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcsqrt.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxcsqrt.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxcsqrt.h" 2


_Lcomplex (csqrtl)(_Lcomplex x)
 {
 ...
 if (erry == 1)
  return (_LCbuild(_LInf._Long_double,
   im));
 else if (_LDtest(&re) != 1)
  ;
 else if (erry == 2)
  if (re < 0.0L)
   return (_LCbuild(im, re));
  else
   return (x);
 else if (re < 0.0L)
  return (_LCbuild(0.0L,
   ((*_LPmsw(&(im))) & ((unsigned short)0x8000)) ? -_LInf._Long_double
    : _LInf._Long_double));
 else
  return (_LCbuild(_LInf._Long_double,
   ((*_LPmsw(&(im))) & ((unsigned short)0x8000)) ? -_LZero
    : 0.0L));

 rho = _LHypot(re, im, &xexp);
 if (xexp == 0)
  return (_LCbuild(rho, im * rho));
 else
  {
  long double remag = re;

  ((*_LPmsw(&(remag))) &= ~((unsigned short)0x8000));
  remag = ldexpl(remag, -xexp);

  rho = ldexpl(sqrtl(2.0L * (remag + rho)),
   xexp / 2 - 1);
  if (0.0L <= re)
   return (_LCbuild(rho, ((im) / ((2.0L * rho)))));
  else if (((*_LPmsw(&(im))) & ((unsigned short)0x8000)))
   return (_LCbuild(((-im) / ((2.0L * rho))), -rho));
  else if (im == 0.0L)
   return (_LCbuild(0.0L, rho));
  else
   return (_LCbuild(((im) / ((2.0L * rho))), rho));
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./csqrtl.c" 2
