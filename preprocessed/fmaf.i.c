# 1 "/arm-libs/library-src/dinkum/source/./fmaf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./fmaf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./fmaf.c" 2
...
# 22 "/arm-libs/library-src/dinkum/source/./xxfma.h"
 if ((errx = _FDunscale(&expx, &xp)) == 2)
  ans = x;
 else if ((erry = _FDunscale(&expy, &yp)) == 2)
  ans = y;
 else if ((errz = _FDunscale(&expz, &zp)) == 2)
  ans = z;
 else if (errx == 1 || erry == 1)
  if (errx == 0 || erry == 0
   || (errz == 1 && (((*_FPmsw(&(x))) & ((unsigned short)0x8000)) ^ ((*_FPmsw(&(y))) & ((unsigned short)0x8000))) != ((*_FPmsw(&(z))) & ((unsigned short)0x8000))))
   {
   _Feraise(0x01);
   ans = _FNan._Float;
   }
   ...
   ans = z;
  else if ((((*_FPmsw(&(x))) & ((unsigned short)0x8000)) ^ ((*_FPmsw(&(y))) & ((unsigned short)0x8000))) == 0)
   ans = _FInf._Float;
  else
   ans = -_FInf._Float;
  ...
  if (2 * 24 < exp_diff)
   ...
   if (exp_diff < -2 * 24)
    ...
    _FDscale(&zp, exp_diff);
   _FXp_setw(xpz, 2, zp);

   _FXp_setw(xpx, 4, xp);
   _FXp_setw(xpy, 2, yp);
   _FXp_mulh(xpx, 4, xpy[0]);
   if (xpy[1] != 0.0F)
    {
    float xpw[4];

    _FXp_setw(xpw, 4, xp);
    _FXp_mulh(xpw, 4, xpy[1]);
    _FXp_addx(xpx, 4, xpw, 4);
    }
   }
  _FXp_addx(xpx, 4, xpz, 2);
  ans = _FXp_getw(xpx, 4);
  if (exp_prod != 0)
   _FDscale(&ans, exp_prod);

  if (_FDtest(&ans) != 1)
   ;
  else if (0.0F < ans)
   ans = 3.40282347e+38F + 3.40282347e+38F;
  else
   ans = -(3.40282347e+38F + 3.40282347e+38F);
  }
 return (ans);
 }
# 4 "/arm-libs/library-src/dinkum/source/./fmaf.c" 2
