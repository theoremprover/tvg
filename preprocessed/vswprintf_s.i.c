# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vswprintf_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/vswprintf_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/vswprintf_s.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/vswprintf_s.c" 2


typedef struct _Vswpstuff {
 wchar_t *s;
 size_t size;
 int fail;
 } _Vswpstuff;
...
int (vswprintf_s)(wchar_t * s, size_t size,
 ...
 if (s == 0 || size == 0 || ((_Sizet)(-1) >> 1) < size)
  return (_Fail_s("vswprintf_s: bad argument", sizeof ("vswprintf_s: bad argument")), 0);
 else if (fmt == 0)
  {
  *s = '\0';
  return (_Fail_s("vswprintf_s: bad format argument", sizeof ("vswprintf_s: bad format argument")), 0);
  }

 x.s = s;
 x.size = size - 1;
 x.fail = 0;
 ans = _WPrintf(&prout, &x, fmt, ap, 1);

 if (x.fail)
  return (_Fail_s("vswprintf_s: buffer overflow", sizeof ("vswprintf_s: buffer overflow")), 0);
 else if (ans < (-1))
  {
  *s = L'\0';
  return (0);
