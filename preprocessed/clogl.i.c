# 1 "/arm-libs/library-src/dinkum/source/./clogl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./clogl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./clogl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxclog.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxclog.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxclog.h" 2
...
_Lcomplex (clogl)(_Lcomplex x)
 ...
 if (errx == 2)
  {
  if (erry == 1)
   return (_LCbuild(_LInf._Long_double, re));
  else
   return (_LCbuild(re, re));
  }
 else if (erry == 2)
  {
  if (errx == 1)
   return (_LCbuild(_LInf._Long_double, im));
  else
   return (_LCbuild(im, im));
  }

 theta = atan2l(im, re);

 if (errx == 1 || erry == 1)
  return (_LCbuild(_LInf._Long_double, theta));
 else
  {
  ...
  if (re == 0.0L)
   ...
   _LDint(&re0, (53 - 1) / 2);
   _LDint(&im0, (53 - 1) / 2);
   norm1 = (re0 * re0 - 1.0L) + im0 * im0;
   norm1 += (re - re0) * (re + re0) + (im - im0) * (im + im0);
   return (_LCbuild(0.5L * log1pl(norm1), theta));
   }
  else
   {
   long double eps = ((im) / (re));

   return (_LCbuild(logl(re)
    + 0.5L * log1pl(eps * eps), theta));
   }
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./clogl.c" 2
