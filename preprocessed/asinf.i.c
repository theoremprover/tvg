# 1 "/arm-libs/library-src/dinkum/source/./asinf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./asinf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./asinf.c" 2
...
float (asinf)(float x)
 ...
 default:
  ...
  if (x < _FRteps._Float)
   ...
   x = ((x * ((((s[0] * xsq + s[1]) * xsq) + s[2]) * xsq + s[3])) / (((((c[0] * xsq + c[1]) * xsq) + c[2]) * xsq + c[3])));
   if (hex != 0)
    ((*_FPmsw(&(x))) ^= ((unsigned short)0x8000));
   return (x);
   }
   ...
   return (_FAtan(((x) / (sqrtf((1.0F - x) * (1.0F + x)))), hex));


  else if (x <= 1.0F)
   return (_FAtan(((sqrtf(1.0F - x) * sqrtf(1.0F + x)) / (x)), hex ^ 0x2));
...
# 4 "/arm-libs/library-src/dinkum/source/./asinf.c" 2
