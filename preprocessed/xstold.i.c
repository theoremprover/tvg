# 1 "/arm-libs/library-src/dinkum/source/./xstold.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xstold.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xstold.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xstold.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xstold.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxcctype.h" 1
# 6 "/arm-libs/library-src/dinkum/source/./xstold.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./xstold.c" 2


long double _Stoldx(const char *s, char **endptr, long pten,
 int *perr)
...
# 22 "/arm-libs/library-src/dinkum/source/./xxstod.h"
 ...
 long lo[3 + 1];
 const char *s0 = s;
 int code = _Stopfx(&s, endptr);
 const int neg = code & 8;

 extern long double _LDtento(long double *, long, int *);

 if (perr != 0)
  *perr = 0;
 if ((code &= ~8) == 1)
  {
  const int nlo = _Stoflt(s0, s, endptr, lo, 3);
  long double xpx[4], xpf[4];
  int i;

  _LXp_setw(xpf, 4, 1e9L);
  if (nlo == 0)
   _LXp_setw(xpx, 4, 0.0);
  else
   for (i = 1, _LXp_setn(xpx, 4, lo[1]); i < nlo; )
    {
    long double xpa[4];
    long double xpt[4 * 2];

    _LXp_mulx(xpx, 4, xpf, 4, xpt);
    _LXp_setn(xpa, 4, lo[++i]);
    _LXp_addx(xpx, 4, xpa, 4);
    }
  pten += lo[0];
  x = _LDtento(xpx, pten, perr);
  }
 else if (code == 2)
  {
  const int nlo = _Stoxflt(s0, s, endptr, lo, 3);
  long double xpx[4], xpf[4];
  int i;

  _LXp_setw(xpf, 4, 268435456.0L);
  if (nlo == 0)
   _LXp_setw(xpx, 4, 0.0);
  else
   for (i = 1, _LXp_setn(xpx, 4, lo[1]); i < nlo; )
  ...
  x = _LInf._Long_double;
 else if (code == 4)
  x = _LNan._Long_double;
 else
  x = 0.0L;
...
# 12 "/arm-libs/library-src/dinkum/source/./xstold.c" 2

long double _Stold(const char *s, char **endptr, long pten)
 {
