# 1 "/arm-libs/library-src/dinkum/source/./tanhf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./tanhf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./tanhf.c" 2
...
static const float halfpln2 = 8.32F;
...
static const float ln2by2 = 0.34657359027997265470861606072908827F;
static const float ln2 = 0.69314718055994530941723212145817657F;
# 98 "/arm-libs/library-src/dinkum/source/./xxtanh.h"
float (tanhf)(float x)
 {
 short neg;
 ...
 case 1:
  return (((*_FPmsw(&(x))) & ((unsigned short)0x8000)) ? -1.0F : 1.0F);
 ...
 default:
  ...
  if (x < _FRteps._Float)
   ...
   x *= ((((s[0] * xsq + s[1]) * xsq + s[2])) / (((c[0] * xsq + c[1]) * xsq + c[2])));
   }
# 144 "/arm-libs/library-src/dinkum/source/./xxtanh.h"
  else if (x < halfpln2)
   x = 1.0F - ((2.0F) / ((1.0F + expf(x + x))));
  else
   x = 1.0F;
...
# 4 "/arm-libs/library-src/dinkum/source/./tanhf.c" 2
