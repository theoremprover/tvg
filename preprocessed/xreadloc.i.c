# 1 "/arm-libs/library-src/dinkum/source/./xreadloc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xreadloc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xreadloc.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xreadloc.c" 2
...
const _Locitem *_Readloc(FILE *lf, char *buf, const char **ps)
 ...
 for (; ; )
  ...
  for (buf[0] = ' ', n = 1; ; n -= 2)
   if (fgets(buf + n, 256 - n, lf) == 0
    || buf[(n = (int)(n + strlen(buf + n))) - 1] != '\n')
    return (0);
   else if (n <= 1 || buf[n - 2] != '\\')
    break;
  buf[n - 1] = '\0';
