# 1 "/arm-libs/library-src/dinkum/source/./remquol.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./remquol.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./remquol.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxremquo.h" 1

# 1 "/toolchain/arm/include/limits.h" 1 3
# 3 "/arm-libs/library-src/dinkum/source/./xxremquo.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxremquo.h" 2


static void _L_Remquo_subtract(long double *px, long double *py)
 {
 short xexp, yexp;
...
long double (remquol)(long double x, long double y, int *pquo)
 ...
 if (0 <= errx || 0 <= erry)
  ...
  for (t = y, _LDunscale(&ychar, &t), n = 0, intpart = 0; ; )
   {
   short xchar;

   t = x;
   if (n < 0 || _LDunscale(&xchar, &t) == 0
   ...
   for (; 0 <= n; --n)
    ...
    if (t <= x)
     {
     _L_Remquo_subtract(&x, &t);
     if (n < 8 * sizeof (intpart))
      intpart += 1 << n;
     break;
     }
    }
  ...
  if (0.0L < x)
   {
   t = x, _LDscale(&t, 1);
   if (y < t || (y == t && (intpart & 1) != 0))
    {
    _L_Remquo_subtract(&x, &y);
    ++intpart;
    }
   }
  if (pquo != 0)
   {
   int ans = (int)(intpart & 0x7fffffff);
...
# 4 "/arm-libs/library-src/dinkum/source/./remquol.c" 2
