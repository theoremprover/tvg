# 1 "/arm-libs/library-src/dinkum/source/./xftgamma.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xftgamma.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xftgamma.c" 2
...
float _FPow(float, float, short *);
const float _FGamma_big = 28.0F;
...
# 142 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
typedef struct Approx_24 {
 float to, mid;
 int nsize, dsize;
 float num[8];
 float den[2];
...
static const Approx_24 approx0[] =
 {
 67.0F, 97.0F / 4020.0F,
 8, 2,
  {
...
static float _FTgamma_stirling(float *px, short *pex)
 {
 int i;
 float x = *px;
 float rootxx = _FPow(x, x - 0.5F, pex);
 ...
 *pex = -*pex;
 for (i = 0; i < sizeof (approx0) / sizeof (approx0[0]) - 1; ++i)
  ...
  {
 float y = (1.0F / (x)) - approx0[i].mid;

 *px = ((_FPoly(y, approx0[i].den, approx0[i].dsize - 1)) / (_FPoly(y, approx0[i].num, approx0[i].nsize - 1)));
...
float _FTgamma(float *px, short *pex)
 ...
 if (28.0F <= x)
  return (_FTgamma_stirling(px, pex));
 else
  {
  if (x < -0.5F)
   {
  ...
  else if (-_FEps._Float < x && x < _FEps._Float)
   {
   y = x;
   _FDunscale(pex, &y);
   }
   ...
   *pex = 0;
   ...
   {
   for (y = 1.0F; 2.5F < x; y = ((y) / (x)))
    x -= 1.0F;
   x -= 2.0F;
   _FDunscale(pex, &y);
   }
  *px = y;
  return ((((((s[0] * x + s[1]) * x + s[2]) * x + s[3])) / ((((c[0] * x + c[1]) * x + c[2]) * x + c[3]))) * x);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./xftgamma.c" 2
