# 1 "/arm-libs/library-src/dinkum/source/./csqrtf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./csqrtf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./csqrtf.c" 2
...
_Fcomplex (csqrtf)(_Fcomplex x)
 ...
 if (erry == 1)
  return (_FCbuild(_FInf._Float,
   im));
 else if (_FDtest(&re) != 1)
  ;
 else if (erry == 2)
  if (re < 0.0F)
   return (_FCbuild(im, re));
  else
   return (x);
 else if (re < 0.0F)
  return (_FCbuild(0.0F,
   ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -_FInf._Float
    : _FInf._Float));
 else
  return (_FCbuild(_FInf._Float,
   ((*_FPmsw(&(im))) & ((unsigned short)0x8000)) ? -_FZero
    : 0.0F));

 rho = _FHypot(re, im, &xexp);
 if (xexp == 0)
  return (_FCbuild(rho, im * rho));
 else
  {
  float remag = re;

  ((*_FPmsw(&(remag))) &= ~((unsigned short)0x8000));
  remag = ldexpf(remag, -xexp);

  rho = ldexpf(sqrtf(2.0F * (remag + rho)),
   xexp / 2 - 1);
  if (0.0F <= re)
   return (_FCbuild(rho, ((im) / ((2.0F * rho)))));
  else if (((*_FPmsw(&(im))) & ((unsigned short)0x8000)))
   return (_FCbuild(((-im) / ((2.0F * rho))), -rho));
  else if (im == 0.0F)
   return (_FCbuild(0.0F, rho));
  else
   return (_FCbuild(((im) / ((2.0F * rho))), rho));
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./csqrtf.c" 2
