# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/qsort_s.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./c_ext1/qsort_s.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./c_ext1/qsort_s.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./c_ext1/qsort_s.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./c_ext1/qsort_s.c" 2
...
static void med3(char *qf, char *qm, char *ql, size_t size, _Cmpfun_s *cmp,
 void *context)
 {
 if ((*cmp)(qm, qf, context) < 0)
  swap(qf, qm, size);
 if ((*cmp)(ql, qm, context) < 0)
  swap(qm, ql, size);
 if ((*cmp)(qm, qf, context) < 0)
  swap(qf, qm, size);
 }

static void median(char *qf, char *qm, char *ql, size_t size, _Cmpfun_s *cmp,
 void *context)
 {
 size_t nelem = (ql - qf) / size;

 if (nelem <= 40)
  med3(qf, qm, ql, size, cmp, context);
 else
  {
  ptrdiff_t step = (ptrdiff_t)((nelem / 8 + 1) * size);

  med3(qf, qf + step, qf + 2 * step, size, cmp, context);
  med3(qm - step, qm, qm + step, size, cmp, context);
  med3(ql - 2 * step, ql - step, ql, size, cmp, context);
  med3(qf + step, qm, ql - step, size, cmp, context);
  }
 }

static void adjust_heap(char *qb, size_t h, size_t n,
 size_t size, _Cmpfun_s *cmp, void *context)
 {
 size_t h0 = h;
 size_t k = 2 * h + 2;
 char *qh = qb + h * size;
 ...
 for (; k <= n; k = 2 * k + 2, qk = qb + k * size)
  {
  if (k == n || (*cmp)(qk, qk - size, context) < 0)
   --k, qk -= size;
  swap(qh, qk, size);
  h = k, qh = qk;
  }
...
static void intro_sort(char *qb, size_t n, size_t ideal,
 ...
 for (; 0 < ideal && 32 < n; )
  ...
  median(qf, qm, ql - size, size, cmp, context);
  while (qf < pf && (cmp)(pf - size, pf, context) == 0)
   pf -= size;
  while (pl < ql && (cmp)(pl, pf, context) == 0)
   pl += size;
  gf = pl;
  gl = pf;
  ...
  n = (ql - pl) / size;
  if (m <= n)
   intro_sort(qb, m, ideal, size, cmp, context), qb = pl;
  else
   intro_sort(pl, n, ideal, size, cmp, context), n = m;
  }
 ...
 if (32 < n)
  ...
  for (h = n / 2; 0 < h; )
   adjust_heap(qb, --h, n, size, cmp, context);
  for (qe = qb + n * size; 1 < n; )
   {
   swap(qb, qe -= size, size);
   adjust_heap(qb, 0, --n, size, cmp, context);
   }
  }
 else if (1 < n)
  {
  ...
  for (qm = qb; 0 < --n; )
   {
   qm += size;
   if ((*cmp)(qm, qb, context) < 0)
    rotate(qb, qm, size);
   else
    {
    char *qx = qm, *qx0 = qm;
    for (; (*cmp)(qm, qx0 -= size, context) < 0; qx = qx0)
     ;
    if (qx != qm)
     rotate(qx, qm, size);
    }
...
errno_t (qsort_s)(void *base,
 ...
 if (((_Sizet)(-1) >> 1) < nelem || ((_Sizet)(-1) >> 1) < size
  || (0 < nelem && (base == 0 || cmp == 0)))
  return (_Fail_s("qsort_s: bad argument", sizeof ("qsort_s: bad argument")));

 if (base != 0 && cmp != 0)
  intro_sort((char *)base, nelem, nelem, size, cmp, context);
 return (0);
 }
