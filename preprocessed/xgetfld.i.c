# 1 "/arm-libs/library-src/dinkum/source/./xgetfld.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xgetfld.c" 2
...
int isblank(int);
# 3 "/arm-libs/library-src/dinkum/source/./xgetfld.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xgetfld.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xgetfld.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./xgetfld.c" 2
# 15 "/arm-libs/library-src/dinkum/source/./xgetfld.c"
int _Getfld(_Sft *px)
 {
 px->stored = 0;
 switch (*px->s)
 ...
 case 'n':
  if (!px->noconv)
   switch (px->qual)
    {
   case 'b':
    {signed char *p = __builtin_va_arg(px->ap, signed char *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (signed char)px->nchar; }
    break;

   case 'q':
    {_Longlong *p = __builtin_va_arg(px->ap, _Longlong *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (_Longlong)px->nchar; }
    break;

   case 'j':
    {intmax_t *p = __builtin_va_arg(px->ap, intmax_t *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (intmax_t)px->nchar; }
    break;

   case 't':
    {ptrdiff_t *p = __builtin_va_arg(px->ap, ptrdiff_t *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (ptrdiff_t)px->nchar; }
    break;

   case 'z':
    {size_t *p = __builtin_va_arg(px->ap, size_t *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (size_t)px->nchar; }
    break;

   case 'h':
    {short *p = __builtin_va_arg(px->ap, short *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (short)px->nchar; }
    break;

   case 'l':
    {long *p = __builtin_va_arg(px->ap, long *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (long)px->nchar; }
    break;

   default:
    {int *p = __builtin_va_arg(px->ap, int *); if (p == 0) return ((-1) - _Fail_s("scanf_s: bad %n argument", sizeof ("scanf_s: bad %n argument"))); *p = (int)px->nchar; }
    }
  return (1);
 ...
 case '%':
  ...
  if ((ch = (++(px)->nchar, (*(px)->pfn)((px)->arg, 0, 1))) == '%')
   return (1);
