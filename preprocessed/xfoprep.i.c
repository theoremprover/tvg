# 1 "/arm-libs/library-src/dinkum/source/./xfoprep.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfoprep.c" 2
...
typedef int errno_t;
# 3 "/arm-libs/library-src/dinkum/source/./xfoprep.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xfoprep.c" 2


FILE *_Foprep(const _Sysch_t *name, const _Sysch_t *mods,
 FILE *str, signed char fd, int exc_code)
 {
 unsigned short alloced;
 unsigned char idx;
 static const FILE init = {0, 0, (-1)};
 if (str == 0)
  return (0);
 alloced = (unsigned short)(str->_Mode & 0x80);
 idx = str->_Idx;
 *str = init;
 ...
 if (name != 0)
  {
  str->_Handle = _Fopen(name,
   exc_code != 0 ? str->_Mode | 0x40 : str->_Mode,
   exc_code == 'U');
  if (!(0 <= (str->_Handle)))
   {
   int errno_save = ( _Errno);
   fclose(str);
   ( _Errno) = errno_save;
   return (0);
   }
