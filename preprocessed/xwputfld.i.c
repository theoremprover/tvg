# 1 "/arm-libs/library-src/dinkum/source/./xwputfld.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwputfld.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xwputfld.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xwputfld.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 5 "/arm-libs/library-src/dinkum/source/./xwputfld.c" 2
# 16 "/arm-libs/library-src/dinkum/source/./xwputfld.c"
int _WPutfld(_WPft *px, va_list *pap, wchar_t code,
 ...
 case L'c':
  ...
  if (px->qual == L'l')
   ...
   for (; 0 <= px->argno; --px->argno)
    ...
    ch = (wchar_t)__builtin_va_arg(*pap, wint_t);
 ...
 case L'd': case L'i':
  ...
  for (; 0 <= px->argno; --px->argno)
   px->v.li = px->qual == L'l' ? __builtin_va_arg(*pap, long)
    : px->qual == L'q' ? __builtin_va_arg(*pap, _Longlong)
    : px->qual == L'j' ? __builtin_va_arg(*pap, intmax_t)
    : px->qual == L't' ? __builtin_va_arg(*pap, ptrdiff_t)
    : px->qual == L'z' ? (ptrdiff_t)__builtin_va_arg(*pap, size_t)
 ...
 case L'x': case L'X':
  ...
  for (; 0 <= px->argno; --px->argno)
   px->v.uli = px->qual == L'l' ? __builtin_va_arg(*pap, unsigned long)
    : px->qual == L'q' ? __builtin_va_arg(*pap, _ULonglong)
    : px->qual == L'j' ? __builtin_va_arg(*pap, uintmax_t)
    : px->qual == L't' ? __builtin_va_arg(*pap, ptrdiff_t)
    : px->qual == L'z' ? __builtin_va_arg(*pap, size_t)
    : __builtin_va_arg(*pap, unsigned int);
  if (px->qual == L'h')
   px->v.uli = (unsigned short)px->v.uli;
  else if (px->qual == L'b')
   px->v.uli = (unsigned char)px->v.uli;
  else if (px->qual == L't' || px->qual == L'z')
 ...
 case L'n':
  if (px->secure)
   return ((-1) - _Fail_s("wprintf_s: %n disallowed", sizeof ("wprintf_s: %n disallowed")));

  switch (px->qual)
   {
   case L'b':
    {signed char *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, signed char *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (signed char)px->nchar; }
    break;

   case L'q':
    {_Longlong *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, _Longlong *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (_Longlong)px->nchar; }
    break;

   case L'j':
    {intmax_t *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, intmax_t *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (intmax_t)px->nchar; }
    break;

   case L't':
    {ptrdiff_t *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, ptrdiff_t *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (ptrdiff_t)px->nchar; }
    break;

   case L'z':
    {size_t *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, size_t *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (size_t)px->nchar; }
    break;

   case L'h':
    {short *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, short *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (short)px->nchar; }
    break;

   case L'l':
    {long *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, long *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (long)px->nchar; }
    break;

   default:
    {int *p = 0; for (; 0 <= px->argno; --px->argno) p = __builtin_va_arg(*pap, int *); if (p == 0) return ((-1) - _Fail_s("wprintf: bad %n argument", sizeof ("wprintf: bad %n argument"))); *p = (int)px->nchar; }
   }
  break;
 ...
 case L's':
  if (px->qual == L'l')
   ...
   for (; 0 <= px->argno; --px->argno)
    px->s = __builtin_va_arg(*pap, wchar_t *);
   if (px->s == 0)
    return ((-1) - _Fail_s("wprintf_s: bad %ls argument", sizeof ("wprintf_s: bad %ls argument")));

   px->n1 = (int)(px->prec < 0 ? wcslen(px->s)
   ...
   for (; 0 <= px->argno; --px->argno)
    s1 = __builtin_va_arg(*pap, const char *);
   if (s1 == 0)
    return ((-1) - _Fail_s("wprintf_s: bad %s argument", sizeof ("wprintf_s: bad %s argument")));
