# 1 "/arm-libs/library-src/dinkum/source/./ctanh.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./ctanh.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./ctanh.c" 2
...
_Dcomplex (ctanh)(_Dcomplex x)
 ...
 if (errx == 1)
  return (_Cbuild(re < 0.0 ? -1.0 : 1.0,
   ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -_Zero : 0.0));
 else if (erry == 2)
  ...
  {
  const double tv = tan(im);
  const double sv = sinh(re);
  const double bv = sv * (1.0 + tv * tv);
  const double dv = 1.0 + bv * sv;
  ...
  if (dv == _Inf._Double)
   ...
   return (_Cbuild(
    ((sqrt(1.0 + sv * sv) * bv) / (dv)),
    ((tv) / (dv))));
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./ctanh.c" 2
