# 1 "/arm-libs/library-src/dinkum/source/./xltgamma.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xltgamma.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xltgamma.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h" 2
# 21 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
long double _LPow(long double, long double, short *);
const long double _LGamma_big = 54.0L;
# 48 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
static const long double s[] = {
 0.00001192292740826975L,
 0.00011794182945081212L,
 0.00101959460145355905L,
 0.00756060680498438352L,
 0.01926527211745248008L,
 0.12554212337999338034L,
 0.54544333054825790736L,
 0.43251944860403390826L,
 };

static const long double c[] = {
 0.00002670630520841925L,
 -0.00142562700733949340L,
 0.01184309540813817347L,
 -0.00913626688892627011L,
 -0.18643116397504167435L,
 0.29357730424461011830L,
 1.02302619254637107038L,
 };
# 170 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
...
static const Approx_53 approx0[] =
 ...
 174.0L, 35.0L / 3132.0L,


 14, 2,
  {
 0.16614314518760217485e-1L,
 -0.13965707272626176936e-2L,
 -0.49824254383538848853e-2L,
 0.10583487432607028510e-2L,
 0.21489456425337538895e-2L,
 -0.95896447695592953043e-3L,
 -0.14455262959800639857e-2L,
 0.14401123536802324852e-2L,
 0.16474099822547439696e-2L,
 -0.51877751839995419056e-2L,
 -0.80784377037000113812e-3L,
 0.15488179113127575577L,
 1.96594700928271786568L,
 2.50896364305534390989L,
  },
  {
 0.70023705660120274726L,
 1.0L,
  },
 };
# 389 "/arm-libs/library-src/dinkum/source/./xxxtgamma.h"
static long double _LTgamma_stirling(long double *px, short *pex)
 {
 int i;
 long double x = *px;
 long double rootxx = _LPow(x, x - 0.5L, pex);
 ...
 *pex = -*pex;
 for (i = 0; i < sizeof (approx0) / sizeof (approx0[0]) - 1; ++i)
  ...
  {
 long double y = (1.0L / (x)) - approx0[i].mid;

 *px = ((_LPoly(y, approx0[i].den, approx0[i].dsize - 1)) / (_LPoly(y, approx0[i].num, approx0[i].nsize - 1)));
...
long double _LTgamma(long double *px, short *pex)
 ...
 if (54.0L <= x)
  return (_LTgamma_stirling(px, pex));
 else
  {
  if (x < -0.5L)
   {
  ...
  else if (-_LEps._Long_double < x && x < _LEps._Long_double)
   {
   y = x;
   _LDunscale(pex, &y);
   }
   ...
   *pex = 0;
   ...
   {
   for (y = 1.0L; 2.5L < x; y = ((y) / (x)))
    x -= 1.0L;
   x -= 2.0L;
   _LDunscale(pex, &y);
   }
  *px = y;
  return (((_LPoly(x, s, sizeof (s) / sizeof (s[0]) - 1)) / (_LPoly(x, c, sizeof (c) / sizeof (c[0]) - 1))) * x);
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./xltgamma.c" 2
