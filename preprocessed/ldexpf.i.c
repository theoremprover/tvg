# 1 "/arm-libs/library-src/dinkum/source/./ldexpf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./ldexpf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./ldexpf.c" 2
...
float (ldexpf)(float x, int xexp)
 {
 if (xexp != 0 && _FDtest(&x) < 0)
  switch (_FDscalex(&x, xexp, 1))
   {
  case 0:
   _Feraise(0x08);
   break;
...
# 4 "/arm-libs/library-src/dinkum/source/./ldexpf.c" 2
