# 1 "/arm-libs/library-src/dinkum/source/./xwprintf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xwprintf.c" 2
...
int _WScanf(wint_t (*)(void *, wint_t, int),
 void *, const wchar_t *, va_list, int);
# 3 "/arm-libs/library-src/dinkum/source/./xwprintf.c" 2


int _WPrintf(void *(*pfn)(void *, const wchar_t *, size_t),
 void *arg, const wchar_t *fmt, va_list ap_arg, int secure)
 {
 _WPft x;
 va_list ap;
 ...
 for (; ; )
  ...
  if ((n = (int)wcscspn(fmt, &percent[0])) <= 0)
   ;
  ...
  fmt += n + 1;
   ...
   {
  const wchar_t *fmt_sav = fmt;

  for (x.argno = 0;
   x.argno < 1000 && _Iswctype(*fmt, 4); ++fmt)
   x.argno = x.argno * 10 + (unsigned char)*fmt - L'0';
  if (*fmt++ != L'$' || x.argno <= 0 || 1000 <= x.argno)
   {
   x.argno = 0;
   fmt = fmt_sav;
   }
  ...
  for (x.flags = 0; (t = wmemchr(&fchar[0], *fmt, (sizeof (fbit) - 1) / sizeof (fbit[0]))) != 0;
   ++fmt)
  ...
  if (*fmt == L'*')
   ...
   for (x.width = 0; _Iswctype(*fmt, 4); ++fmt)
  ...
  if (*fmt != L'.')
   ...
   {
   wchar_t neg = (wchar_t)(*fmt == L'-'
    ? *fmt++ : 0);

   for (x.prec = 0; _Iswctype(*fmt, 4); ++fmt)
  ...
  if (0 < x.argno)
   ...
   --x.argno;
   _Vacopy(&ap2, ap);
   if ((ans = _WPutfld(&x, &ap2, *fmt++, ac)) != 0)
    return (ans);
   }
  else if ((ans = _WPutfld(&x, &ap, *fmt++, ac)) != 0)
   return (ans);
  if (_WPuttxt(&x, ac) < 0)
   return ((-1));
   }
