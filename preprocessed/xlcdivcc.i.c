# 1 "/arm-libs/library-src/dinkum/source/./xlcdivcc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlcdivcc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlcdivcc.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxcdivcc.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxcdivcc.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxxcdivcc.h" 2


_Lcomplex (_LCdivcc)(_Lcomplex x, _Lcomplex y)
 {
 long double xre = creall(x);
 long double xim = cimagl(x);
 long double yre = creall(y);
 long double yim = cimagl(y);
 long double zre = yre;
 long double zim = yim;

 ((*_LPmsw(&(zre))) &= ~((unsigned short)0x8000));
 ((*_LPmsw(&(zim))) &= ~((unsigned short)0x8000));
 if (zim < zre)
  {
  long double wr = ((yim) / (yre));
  long double wd = yre + wr * yim;
  ...
  {
  long double wr = ((yre) / (yim));
  long double wd = yim + wr * yre;

  zre = ((xre * wr + xim) / (wd));
  zim = ((xim * wr - xre) / (wd));
  }
 return (_LCbuild(zre, zim));
 }
# 4 "/arm-libs/library-src/dinkum/source/./xlcdivcc.c" 2
