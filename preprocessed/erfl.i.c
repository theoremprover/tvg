# 1 "/arm-libs/library-src/dinkum/source/./erfl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./erfl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./erfl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxerf.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxerf.h" 2


long double (_LErfc)(long double x);
extern const long double _LErf_one;
extern const long double _LErf_small;
# 35 "/arm-libs/library-src/dinkum/source/./xxerf.h"
static const long double s[] = {
 0.12040999373658753679e-6L,
 0.88784405615357064309e-5L,
 0.32806639713878541747e-3L,
 0.34814902101731196859e-2L,
 0.51728385731987527358e-1L,
 0.17644011324127378370L,
 1.12837916709551257390L,
 };

static const long double c[] = {
 0.14060567875468972603e-5L,
 0.58411032889811748820e-4L,
 0.11742377551152094336e-2L,
 0.14283713727543944095e-1L,
 0.10907619238514398035L,
 0.48969931241768294256L,
 1.0L,
 };
# 119 "/arm-libs/library-src/dinkum/source/./xxerf.h"
static const long double twobyrootpi = 1.1283791670955125738961589031215451L;
...
long double (erfl)(long double x)
 ...
 case 1:
  return (((*_LPmsw(&(x))) & ((unsigned short)0x8000)) ? -1.0L : 1.0L);
 ...
 default:
  y = x;
  ((*_LPmsw(&(y))) &= ~((unsigned short)0x8000));
  if (y < _LErf_small)
   {
   y = x * x;

   return (((((((((s[0] * y + s[1]) * y + s[2]) * y + s[3]) * y + s[4]) * y + s[5]) * y + s[6])) / (((((((c[0] * y + c[1]) * y + c[2]) * y + c[3]) * y + c[4]) * y + c[5]) * y + c[6]))) * x);
   }
  else if (y < _LErf_one)
   y = 1.0L - _LErfc(y);
  else
...
# 4 "/arm-libs/library-src/dinkum/source/./erfl.c" 2
