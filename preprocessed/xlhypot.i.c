# 1 "/arm-libs/library-src/dinkum/source/./xlhypot.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xlhypot.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xlhypot.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxxhypot.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxxhypot.h" 2


long double _LHypot(long double x, long double y, int *pexp)
 {
 short xexp, yexp;
 short errx = _LDunscale(&xexp, &x);
 short erry = _LDunscale(&yexp, &y);
 ...
 *pexp = 0;
 if (errx == 1 || erry == 1)
  ...
  else if (x == 0.0L || xexp < yexp || (xexp == yexp && x < y))
   {
   short texp = xexp;
   long double tmp = x;
  ...
  if ((*pexp = (xexp / 4) * 2) == 0)
   *pexp = 2;
  _LDscale(&x, xexp - *pexp);
  _LDscale(&y, yexp - *pexp);
  ...
  if (z == x)
   ...
   {
   const long double qv = ((x) / (y));
   z = x + ((y) / (qv + sqrtl(qv * qv + 1.0L)));
   }
  else
   {
   static const long double root2 =
    1.4142135623730950488016887242096982L;
   static const long double oneplusroot2high = 2.4142L;
   static const long double oneplusroot2low =
    0.0000135623730950488016887242096980785698L;
   const long double qv = ((z) / (y));
   const long double rv = (qv + 2.0L) * qv;
   const long double sv = ((rv) / (root2 + sqrtl(rv + 2.0L)))
...
# 4 "/arm-libs/library-src/dinkum/source/./xlhypot.c" 2
