# 1 "/arm-libs/library-src/dinkum/source/./xftan.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xftan.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xftan.c" 2
...
static float _FTan_approx(float x, unsigned int invert)
 {
 const float z = x * x;
 const float xd = 1.0F + z * (c[0] * z + c[1]);
...
static const float atan7_8 = 0.71882999962162450541701415152590464F;


float (_FTan)(float x, int retcode)
 {
 switch (_FDtest(&x))
  {
 case 2:
 ...
 default:
   {
  unsigned int invert = _FQuad(&x, retcode) & 0x1;
  ...
  if (x < _FRteps._Float)
   ...
   {
   float eps = x - atan7_8;
   float tanb = _FTan_approx(eps, 0);

   x = ((tanb + 0.875F) / (1.0F - 0.875F * tanb));
   }


  else
   {
   x = _FTan_approx(x, invert);
   invert = 0;
   }
...
# 4 "/arm-libs/library-src/dinkum/source/./xftan.c" 2
