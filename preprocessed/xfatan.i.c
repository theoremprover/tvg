# 1 "/arm-libs/library-src/dinkum/source/./xfatan.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfatan.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfatan.c" 2
...
static const float a[8] = {
 0.0F,
 0.52359877559829887307710723054658382F,
 1.57079632679489661923132169163975145F,
 1.04719755119659774615421446109316763F,
 1.57079632679489661923132169163975145F,
 2.09439510239319549230842892218633526F,
 3.14159265358979323846264338327950287F,
 2.61799387799149436538553615273291908F,
 };

static const float rt3 = 1.7320508075688772935274463415058725F;
static const float fold = 0.26794919243112270647255365849412763F;
static const float piby4 = 0.78539816339744830961566084581987572F;

float _FAtan(float x, int idx)
 {
 int idx7;
 ...
 if (x == 1.0F)
  ...
  x = piby4 + ((eps * ((s2[0] * eps + s2[1]) * eps + s2[2])) / (((c2[0] * eps + c2[1]) * eps + c2[2])));
  }
  ...
  if (x < -_FRteps._Float || _FRteps._Float < x)
   {
   const float z = x * x;
    const float eps = z * (c[0] * z + c[1]);
    const float denom = (1.0F / (1.0F + eps));
...
# 4 "/arm-libs/library-src/dinkum/source/./xfatan.c" 2
