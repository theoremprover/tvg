# 1 "/arm-libs/library-src/dinkum/source/./xfcdivcc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xfcdivcc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xfcdivcc.c" 2
...
_Fcomplex (_FCdivcc)(_Fcomplex x, _Fcomplex y)
 {
 float xre = crealf(x);
 float xim = cimagf(x);
 float yre = crealf(y);
 ...
 ((*_FPmsw(&(zre))) &= ~((unsigned short)0x8000));
 ((*_FPmsw(&(zim))) &= ~((unsigned short)0x8000));
 if (zim < zre)
  {
  float wr = ((yim) / (yre));
  float wd = yre + wr * yim;
 ...
 return (_FCbuild(zre, zim));
 }
# 4 "/arm-libs/library-src/dinkum/source/./xfcdivcc.c" 2
