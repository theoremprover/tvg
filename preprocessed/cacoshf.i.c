# 1 "/arm-libs/library-src/dinkum/source/./cacoshf.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cacoshf.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cacoshf.c" 2
...
static const float pi3by4 = 2.3561944901923449288469825374596271F;
...
_Fcomplex (cacoshf)(_Fcomplex x)
 {
 float re = crealf(x);
 float im = cimagf(x);
 const short errx = _FDtest(&re);
 const short erry = _FDtest(&im);
 float u, v;
 ...
 if (errx == 2)
  ...
  if (erry == 1)
   ...
   if (re < 0.0F)
    ...
    v = 0.0F;
  if (((*_FPmsw(&(im))) & ((unsigned short)0x8000)))
   v = -v;
  }
 else if (erry == 1)
  {
  ...
  {
  const _Fcomplex w = csqrtf(_FCbuild(re - 1.0F, -im));
  const _Fcomplex z = csqrtf(_FCbuild(re + 1.0F, im));
  const float wr = crealf(w);
  const float wi = cimagf(w);
  const float zr = crealf(z);
  const float zi = cimagf(z);
  int neg;
  float alfa, beta;
  ...
  v = 2.0F * atan2f(
   cimagf(csqrtf(_FCbuild(re - 1.0F, im))), zr);
  if (neg)
   ((*_FPmsw(&(v))) ^= ((unsigned short)0x8000));
  }
 return (_FCbuild(u, v));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cacoshf.c" 2
