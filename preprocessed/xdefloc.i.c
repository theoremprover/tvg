# 1 "/arm-libs/library-src/dinkum/source/./xdefloc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xdefloc.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./xdefloc.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xdefloc.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./xdefloc.c" 2
...
const char *_Defloc(int cat)
 ...
 if ((s = getenv("LC_ALL")) == 0)
  ...
  for (i = 0; catmask[i] != 0; ++i)
   if (cat == catmask[i])
    break;

  if ((catmask[i] == 0 || (s = getenv(catname[i])) == 0))
   s = getenv("LANG");
  }
 return (s);
 }
