# 1 "/arm-libs/library-src/dinkum/source/./cproj.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./cproj.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./cproj.c" 2
...
_Dcomplex (cproj)(_Dcomplex x)
 {
 double re = creal(x);
 double im = cimag(x);
 const short err_re = _Dtest(&re);
 const short err_im = _Dtest(&im);

 return (err_re != 1 && err_im != 1 ? x
  : _Cbuild(_Inf._Double, ((*_Pmsw(&(im))) & ((unsigned short)0x8000)) ? -_Zero : 0.0));
 }
# 4 "/arm-libs/library-src/dinkum/source/./cproj.c" 2
