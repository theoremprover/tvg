# 1 "/arm-libs/library-src/dinkum/source/./ctanhl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./ctanhl.c" 2
...
# 3 "/arm-libs/library-src/dinkum/source/./ctanhl.c" 2
# 1 "/arm-libs/library-src/dinkum/source/./xxctanh.h" 1
...
# 3 "/arm-libs/library-src/dinkum/source/./xxctanh.h" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xxctanh.h" 2


_Lcomplex (ctanhl)(_Lcomplex x)
 {
 ...
 if (errx == 1)
  return (_LCbuild(re < 0.0L ? -1.0L : 1.0L,
   ((*_LPmsw(&(im))) & ((unsigned short)0x8000)) ? -_LZero : 0.0L));
 else if (erry == 2)
  ...
  return (_LCbuild(_LNan._Long_double, _LNan._Long_double));
 else
  {
  const long double tv = tanl(im);
  const long double sv = sinhl(re);
  const long double bv = sv * (1.0L + tv * tv);
  const long double dv = 1.0L + bv * sv;

  if (dv == _LInf._Long_double)
   return (_LCbuild(
    sv < 0.0L ? -1.0L : 1.0L,
    tv * 0.0L));
  else
   return (_LCbuild(
    ((sqrtl(1.0L + sv * sv) * bv) / (dv)),
    ((tv) / (dv))));
  }
 }
# 4 "/arm-libs/library-src/dinkum/source/./ctanhl.c" 2
