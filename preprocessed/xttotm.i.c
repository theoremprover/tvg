# 1 "/arm-libs/library-src/dinkum/source/./xttotm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./xttotm.c" 2
...
time_t _Tzoff(void);
# 3 "/arm-libs/library-src/dinkum/source/./xttotm.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./xttotm.c" 2
...
static struct tm ts = {0}; static int (*_Tls_setup_ts)(void) = 0;
...
struct tm *_Ttotm(struct tm *t, time_t secsarg, int isdst)
 ...
 secsarg += ((70 * 365LU + 17) * 86400);
 if (t == 0)
  t = ((_Tls_setup_ts && _Tls_setup_ts()), (&(ts)));
 t->tm_isdst = isdst;
 for (secs = (unsigned long)secsarg; ;
  ...
  t->tm_yday = days;
   ...
   {
  int mon;
  const short *pm = ((year) & 03 || (year) == 0 ? mos : lmos);
