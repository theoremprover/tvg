# 1 "/arm-libs/library-src/dinkum/source/./setlocal.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
...
int isblank(int);
# 3 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
...
# 4 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
...
# 5 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
...
# 6 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
...
# 7 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
# 1 "/toolchain/arm/include/xstrxfrm.h" 1 3



# 1 "/toolchain/arm/include/string.h" 1 3
# 5 "/toolchain/arm/include/xstrxfrm.h" 2 3
...
# 8 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
...
# 9 "/arm-libs/library-src/dinkum/source/./setlocal.c" 2
# 19 "/arm-libs/library-src/dinkum/source/./setlocal.c"
struct _Locprofile {
 _Linfo *_Array[7];
 };
...
static name_t curname = "C"; static int (*_Tls_setup_curname)(void) = 0;
static name_t namalloc = 0; static _Once_t _Tls_once_namalloc = 0; static void _Tls_dtor_namalloc(void) { free_tls(&(namalloc)); } static void _Tls_reg_namalloc(void) { _Atexit(_Tls_dtor_namalloc); } static int _Tls_setupx_namalloc(void) { if (*(&_Tls_once_namalloc) == 0) (_Tls_reg_namalloc)(), *(&_Tls_once_namalloc) = 2; return (1); } static int (*_Tls_setup_namalloc)(void) = _Tls_setupx_namalloc;
_Linfo _Clocale = {"C"};
static const char *const nmcats[7] = {
...
static int (*_Tls_setup_profile)(void) = 0;
static _Once_t cloc_o = 0;
...
int _Getnloc(_Linfo **pc, int cat, const char *locname)
 ...
 if ((cat & (((1 << (7)) >> 1) - 1)) != cat)
  return (-1);
 if (strcmp(locname, "POSIX") == 0)
  locname = "C";
 for (i = 0; ++i < 7; )
  if ((((1 << (i)) >> 1) & cat) != 0)
   {
   const char *catname = locname;

   if (locname[0] == '\0')
    catname = _Defloc(((1 << (i)) >> 1) & cat);
   if (catname != 0 && (p = _Getloc(nmcats[i], catname)) != 0)
    ;
   else if (locname[0] == '\0')
    p = _Getloc(nmcats[i], "C");
   else
    return (-1);
   if (p != pc[i])
    changed = 1;
 ...
 if (cat == (((1 << (7)) >> 1) - 1))
  {
  if (locname[0] == '\0')
   locname = _Defloc((((1 << (7)) >> 1) - 1));
  if (locname != 0 && (p = _Getloc("", locname)) != 0 && p != pc[0])
   {
   pc[0] = p;
   changed = 1;
   }
...
char *(setlocale)(int cat, const char *locname)
 {
 name_t *pcurname = ((_Tls_setup_curname && _Tls_setup_curname()), (&(curname)));

 if (locname != 0)
  {
  struct _Locprofile prof = *((_Tls_setup_profile && _Tls_setup_profile()), (&(profile)));
  int changed;
  ...
  if (changed < 0)
   ...
   {
   char *name = _Getlname(prof._Array, (((1 << (7)) >> 1) - 1),
    (const char **)((_Tls_setup_namalloc && _Tls_setup_namalloc()), (&(namalloc))));
   size_t i;
   ...
   if (name == 0)
    ...
    {
    struct _Locprofile *pprof = ((_Tls_setup_profile && _Tls_setup_profile()), (&(profile)));
